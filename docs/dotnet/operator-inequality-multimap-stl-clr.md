...
## Notes  
 La fonction d'opérateur retourne `!(``left` `==` `right``)`.  Vous l'utilisez pour tester si `left` est ordonné comme `right` lorsque les deux multimaps sont comparés élément par élément.  
