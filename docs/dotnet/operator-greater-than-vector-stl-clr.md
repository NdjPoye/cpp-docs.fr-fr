...
Vecteur plus grand que comparaison.  
...
## Notes  
 La fonction d'opérateur retourne `right` `<` `left`.  Vous l'utilisez pour tester si `left` est ordonné d'après `right` lorsque les deux deques d'éléments sont comparées élément par élément.  
