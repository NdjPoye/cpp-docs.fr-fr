...
## Notes  
 La fonction d'opérateur retourne `!(``right` `<` `left``)`.  Vous l'utilisez pour tester si `left` n'est pas ordonné d'après `right` lorsque les deux paires d'éléments sont comparées élément par élément.  
