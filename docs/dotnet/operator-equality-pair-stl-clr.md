...
## Notes  
 La fonction d'opérateur retourne `left``.first ==` `right``.first &&` `left``.second ==` `right``.second`.  Vous l'utilisez pour tester si `left` est commandé comme `right` lorsque les deux paires sont comparées élément par élément.  
