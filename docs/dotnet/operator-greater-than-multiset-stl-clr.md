...
Liste de comparaison de supériorité.  
...
## Notes  
 La fonction d'opérateur retourne `right` `<` `left`.  Vous l'utilisez pour tester si `left` n'est pas ordonné après `right` lorsque les deux multisets sont comparées élément par élément.  
