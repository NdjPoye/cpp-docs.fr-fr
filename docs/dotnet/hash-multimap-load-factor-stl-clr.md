...
## Notes  
 La méthode retourne `(float)`[hash\_multimap::size](../dotnet/hash-multimap-size-stl-clr.md)`() /` [hash\_multimap::bucket\_count](../dotnet/hash-multimap-bucket-count-stl-clr.md)`()`.  A utiliser pour déterminer la taille moyenne d'un compartiment.  
