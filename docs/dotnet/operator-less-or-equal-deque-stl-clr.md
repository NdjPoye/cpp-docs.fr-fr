...
Comparaison d'infériorité ou d'égalité de pile.  
...
## Notes  
 La fonction d'opérateur retourne `!(``right` `<` `left``)`.  Vous l'utilisez pour tester si `left` n'est pas ordonné d'après `right` lorsque les deux piles d'éléments sont comparées élément par élément.  
