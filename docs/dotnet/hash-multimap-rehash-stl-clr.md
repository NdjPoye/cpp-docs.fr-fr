...
## Notes  
 La méthode reconstruit la table de hachage, ce qui garantit [hash\_multimap::load\_factor](../dotnet/hash-multimap-load-factor-stl-clr.md)`() <=` [hash\_multimap::max\_load\_factor](../dotnet/hash-multimap-max-load-factor-stl-clr.md).  Sinon, la table de hachage augmente en taille seulement après une insertion, si nécessaire. \(Elle ne diminue jamais automatiquement de taille.\) Vous l'utilisez pour ajuster la taille de la table de hachage.  
