...
## Notes  
 L'opérateur renvoie true uniquement si les séquences contrôlées par `left` et `right` ont la même longueur et, pour chaque position `i`, `left``[i] ==` `right``[i]`.  Vous l'utilisez pour tester si `left` est ordonné comme `right` lorsque les deux multimaps sont comparées élément par élément.  
