...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "assemblys mixtes (C++), verrouillage du chargeur"
  - "verrouillage du chargeur (C++)"
  - "initialisation d'assemblys mixtes"
  - "interblocages (C++)"
  - ".cctor (C++)"
  - "paramètres régionaux personnalisés (C++)"
  - "assemblys mixtes (C++), initialisation"
...
Dans Visual C\+\+ .NET et Visual C\+\+ 2003, les DLL compilées avec l’option du compilateur **\/clr** pouvaient faire l’objet d’un interblocage non déterministe pendant le chargement. On parlait dans ce cas de « problème de chargement de DLL mixtes » ou de « problème de verrouillage du chargeur ». Le non\-déterminisme a été pratiquement supprimé du processus de chargement de DLL mixtes. Toutefois, il reste quelques scénarios dans lesquels le verrouillage du chargeur peut se produire \(de façon déterministe\). Pour plus d’informations sur ce problème, consultez « Problème de chargement de DLL mixtes » dans [MSDN Library](http://go.microsoft.com/fwlink/?linkid=556).  
...
### Fonctions fournies par l’utilisateur affectant le démarrage  
 Il existe plusieurs fonctions fournies par l’utilisateur dont dépendent les bibliothèques pour l’initialisation au démarrage. Par exemple, durant la surcharge globale d’opérateurs en C\+\+, comme les opérateurs `new` et `delete`, les versions fournies par l’utilisateur sont utilisées partout, notamment pour l’initialisation et la destruction STL. STL et les initialiseurs statiques fournis par l’utilisateur appellent donc les versions fournies par l’utilisateur de ces opérateurs.  
...
### Implémentation dans les en\-têtes  
 ...
 Dans Visual C\+\+ .NET et Visual C\+\+ 2003, l’éditeur de liens choisit simplement la plus importante de ces définitions sémantiquement équivalentes, et ce pour tenir compte des déclarations anticipées et des scénarios quand différentes options d’optimisation sont utilisées pour des fichiers sources différents. Cela crée un problème pour les DLL natives\/.NET mixtes.  
...
-   Avec la compilation ciblée Itanium, il existe un bogue dans l’implémentation de tous les pointeurs de fonction.  Dans l’exemple précédent, si myObject\_p était défini localement à l’intérieur de during\_loaderlock\(\), l’appel peut également être résolu en une implémentation managée.  
...
5.  Ouvrez la fenêtre Pile des appels. \(Dans le menu Déboguer, cliquez sur Windows, puis sur Pile des appels.\) La fonction `DllMain` incriminée ou l’initialiseur statique est identifié avec une flèche verte. Si la fonction incriminée n’est pas identifiée, vous devez effectuer les étapes suivantes pour la rechercher.  
