...
Comparaison de vecteurs non égaux.  
...
## Notes  
 La fonction d'opérateur retourne `!(``left` `==` `right``)`.  Vous l'utilisez pour tester si `left` n'est pas ordonné de la même façon que `right` lorsque les deux vecteurs sont comparées élément par élément.  
