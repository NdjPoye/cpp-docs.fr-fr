...
## Notes  
 L'opérateur renvoie vrai uniquement si les séquences contrôlées par `left` et `right` ont la même longueur et, pour chaque position `i`, `left``[i] ==` `right``[i]`.  Vous l'utilisez pour tester si `left` est ordonné comme `right` lorsque les deux ensembles sont comparées élément par élément.  
