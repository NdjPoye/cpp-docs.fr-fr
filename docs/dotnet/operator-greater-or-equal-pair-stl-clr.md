...
Comparaison « supérieur ou égal à » paire.  
...
## Notes  
 La fonction d'opérateur retourne `!(``left` `<` `right``)`.  Vous l'utilisez pour tester si `left` n'est pas ordonné avant `right` lorsque les deux paires sont comparées élément par élément.  
