...
Impossible de définir\/annuler la définition des symboles de préprocesseur à la suite du premier jeton du fichier  
 ...
 Les [directives de préprocesseur](../Topic/C%23%20Preprocessor%20Directives.md)`#define` et `#undef` doivent être utilisées au début d’un programme, avant les autres mots clés, notamment ceux utilisés dans la déclaration d’espace de noms.  
