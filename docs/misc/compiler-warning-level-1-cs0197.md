...
Le passage de 'argument' comme ref ou out ou la prise de son adresse peut provoquer une exception runtime, car il s’agit d’un champ d’une classe marshalée par référence  
 ...
 Toute classe qui dérive directement ou indirectement de <xref:System.MarshalByRefObject> est une classe marshalée par référence. Une telle classe peut être marshalée par référence au\-delà des limites du processus et de l’ordinateur. Par conséquent, les instances de cette classe peut être des proxys vers des objets distants. Vous ne pouvez pas passer un champ d’un objet proxy en tant que [ref](../Topic/ref%20\(C%23%20Reference\).md) ou [out](../Topic/out%20\(C%23%20Reference\).md). Vous ne pouvez donc pas passer les champs d’une telle classe en tant que `ref` ou `out`, sauf si l’instance est [this](../Topic/this%20\(C%23%20Reference\).md), qui ne peut pas être un objet proxy.  
