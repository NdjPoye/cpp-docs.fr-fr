...
'interface' est déjà énuméré dans la liste des interfaces  
 ...
 Une liste d’héritage d’interface comprend un doublon. Une [interface](../Topic/interface%20\(C%23%20Reference\).md) ne peut être spécifiée qu’une seule fois dans la liste d’héritage.  
