...
Le premier opérande d’un opérateur « is » ou « as » ne peut pas être une expression lambda ou une méthode anonyme.  
 ...
 Les expressions lambda et les méthodes anonymes ne peuvent pas être utilisées du côté gauche d’un opérateur [is](../Topic/is%20\(C%23%20Reference\).md) ou [as](../Topic/as%20\(C%23%20Reference\).md).  
