...
Le système de projet n’a pas pu copier un ou plusieurs fichiers de sortie de la build \(par exemple, la DLL, le fichier PDB ou des assemblys satellites\) sur le serveur web.  
 ...
 Elle se produit généralement si un fichier de sortie de la build est verrouillé ou en lecture seule sur le serveur. Si un fichier est verrouillé sur le serveur, cela indique que le runtime [!INCLUDE[vstecasp](../misc/includes/vstecasp_md.md)] n’a pas copié correctement les assemblys, les satellites ou les symboles de débogage qui sont actuellement sur le serveur.  
  
 Cela peut également indiquer que le serveur ne dispose pas assez d’espace disque ou que des problèmes réseau empêchent [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] de charger les fichiers sur le web.  
  
### Pour corriger cette erreur  
  
1.  Vérifiez l’espace disque disponible.  
  
2.  Si un ou plusieurs fichiers sont verrouillés, redémarrez le serveur web pour libérer le verrou [!INCLUDE[vstecasp](../misc/includes/vstecasp_md.md)] mis sur ces fichiers.  
