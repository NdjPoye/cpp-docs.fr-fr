...
Une classe ou une structure apparaît dans plusieurs définitions du même type conteneur, et plusieurs définitions ne sont pas marquées comme `Partial`.  
 ...
 Vous devez utiliser le mot clé [Partial](../Topic/Partial%20\(Visual%20Basic\).md) sur au moins l’une des nombreuses définitions d’une classe ou d’une structure, mais il est recommandé de l’utiliser sur toutes les définitions partielles.  
...
-   Utilisez le mot clé [Partial](../Topic/Partial%20\(Visual%20Basic\).md) sur chaque définition partielle de la classe ou de la structure.