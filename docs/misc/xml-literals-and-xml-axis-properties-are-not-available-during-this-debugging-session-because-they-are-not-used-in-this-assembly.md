...
Un littéral XML ou une propriété d’axe XML ont été référencés dans la fenêtre **Espion** ou **Exécution** pendant une session de débogage dans laquelle le langage XML n’est pas disponible dans les fonctionnalités [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)]. C’est le cas pour un assembly qui n’utilise pas le langage XML dans les fonctionnalités [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] ou qui n’est pas une version Release.  
