...
Cette procédure pas à pas montre comment utiliser l’automatisation pour créer un VSPackage managé qui manipule l’environnement de développement intégré \(IDE\) de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)]. Dans cette procédure, vous créez un exemple de VSPackage géré, puis utilisez des méthodes d’automatisation dans le VSPackage résultant pour afficher les propriétés [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] dans la fenêtre **Sortie**.  
...
1.  Créez un projet [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] Package nommé `Auto`.  
...
2.  Dans la page **Sélectionner un langage de programmation**, définissez la langue sur [!INCLUDE[csprcs](../ide/includes/csprcs_md.md)].  
...
4.  Dans le fichier AutoPackage, remplacez le corps de la méthode `MenuItemCallback` par les lignes suivantes :  
 ...
 Ce code appelle <xref:Microsoft.VisualStudio.Shell.Package.GetService%2A> pour obtenir un objet Automation <xref:EnvDTE.DTE> qui représente l’IDE de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] Le code d’automatisation de `MenuItemCallback` crée un volet dans la fenêtre **Sortie** nommé **Test**. Le nom et la version de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] sont ensuite écrits dans le nouveau volet **Sortie**.  
  
1.  Créez et démarrez le projet Auto en mode débogage en appuyant sur F5.  
  
     La build expérimentale [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] \([!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] Exp\) est alors démarrée.  
  
    > [!NOTE]
    >  Les deux versions de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] sont ouvertes à ce stade.  
  
2.  Dans [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] Exp, dans le menu **Outils**, cliquez sur **Auto**.  
    ...
    ```  
     ...
     Où x.xx correspond au numéro de version de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] le plus récent.  
