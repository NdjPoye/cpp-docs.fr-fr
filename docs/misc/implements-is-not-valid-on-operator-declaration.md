...
Une [Operator Statement](../Topic/Operator%20Statement.md) spécifie le mot clé [Implements](../Topic/Implements%20Clause%20\(Visual%20Basic\).md).  
 ...
 Une procédure `Operator` exige les deux mots clés `Public` et `Shared`, et un opérateur de conversion nécessite le mot clé `Widening` ou `Narrowing`. Pour plus d'informations, consultez [Operator Procedures](../Topic/Operator%20Procedures%20\(Visual%20Basic\).md).  
