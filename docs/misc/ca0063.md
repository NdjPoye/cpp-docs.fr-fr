...
```  
 ...
 Dans certains cas, l'emplacement d'un ruleset peut dépendre d'une propriété de [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)].  les propriétés [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)] ne peuvent pas être référencées à partir d'un ruleset.  Pour contourner ce problème, spécifiez les chemins de recherche supplémentaires dans la propriété **CodeAnalysisRuleSetDirectories** de [!INCLUDE[vstecmsbuild](../build/includes/vstecmsbuild_md.md)].  Dans ce scénario, spécifiez uniquement le nom du ruleset dans l'attribut de **Path** de l'élément **Include**.  
