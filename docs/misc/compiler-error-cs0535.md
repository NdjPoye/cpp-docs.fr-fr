...
'class' n’implémente pas le membre d’interface 'member'  
 ...
 Une [classe](../Topic/class%20\(C%23%20Reference\).md) dérivée d’une [interface](../Topic/interface%20\(C%23%20Reference\).md) n’implémente pas un ou plusieurs membres de l’interface. Une classe doit implémenter tous les membres des interfaces dont elle est dérivée ; sinon, elle doit être déclarée `abstract`.  
