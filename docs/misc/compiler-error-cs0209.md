...
Le type des variables locales déclaré dans une instruction fixed doit être un type pointeur  
 ...
 La variable que vous déclarez dans une [instruction fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md) doit être un pointeur. Pour plus d’informations, consultez [Pointeurs et code unsafe](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
