...
Cette procédure pas à pas montre comment créer un complément Visual Studio qui consomme un service Visual Studio. Le complément que vous créez obtient un fournisseur de services et l’utilise pour obtenir un service. Vous pouvez utiliser cette même technique pour obtenir un service Visual Studio proposé. Pour plus d’informations sur les services et fournisseurs de services, consultez [À l'aide et fournir des Services](../Topic/Using%20and%20Providing%20Services.md). Les procédures ci\-après montrent comment créer un complément et obtenir un service à partir du complément.  
...
3.  Créez un projet de complément nommé `Addin`.  
     ...
     Dans l’Assistant Complément [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)], cliquez sur **Suivant**.  
...
4.  Créez et démarrez le projet de complément en mode débogage en appuyant sur F5.  
     ...
     Une autre instance de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] démarre.  
  
5.  Dans la nouvelle instance [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)], dans le menu **Outils**, cliquez sur **Complément**. La boîte de message indique :  
