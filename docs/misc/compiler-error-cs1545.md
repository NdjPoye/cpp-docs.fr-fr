...
La propriété, l’indexeur ou l’événement 'property' n’est pas pris en charge par le langage ; essayez d’appeler directement les méthodes d’accesseur 'set accessor' ou 'get accessor'  
 ...
 Le code utilise un objet ayant un [indexeur](../Topic/Indexers%20\(C%23%20Programming%20Guide\).md) autre que celui par défaut et a tenté d’utiliser la syntaxe indexée. Pour résoudre cette erreur, appelez la méthode d’accesseur `get` ou `set` de la propriété.  
