...
Un événement dans une interface ne peut pas avoir d’accesseurs add ou remove  
 ...
 Vous ne pouvez pas définir les fonctions d’accesseur d’un événement dans une [interface](../Topic/interface%20\(C%23%20Reference\).md). Pour plus d’informations, consultez [Événements](../Topic/Events%20\(C%23%20Programming%20Guide\).md) et [Interfaces](../Topic/Interfaces%20\(C%23%20Programming%20Guide\).md).  
