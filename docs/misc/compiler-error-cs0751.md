...
Une méthode partielle doit être déclarée au sein d’une classe partielle ou d’une structure partielle  
 ...
 Il n’est pas possible de déclarer une méthode [partielle](../Topic/partial%20\(Method\)%20\(C%23%20Reference\).md), sauf si elle est encapsulée dans une classe partielle ou un struct partiel.  
