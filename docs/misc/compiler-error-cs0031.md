...
Impossible de convertir la valeur de constante 'value' en 'type'. \(utilisez la syntaxe 'unchecked'\)  
 ...
 Une tentative d’assignation d’une valeur à une variable dont le type ne peut pas stocker la valeur a été effectuée. Pour plus d'informations, consultez [Types](../Topic/Types%20\(C%23%20Programming%20Guide\).md).  
