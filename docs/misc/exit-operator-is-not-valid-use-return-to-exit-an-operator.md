...
Une instruction `Exit Operator` apparaît dans une procédure `Operator`.  
 ...
 Vous devez utiliser un [Return Statement](../Topic/Return%20Statement%20\(Visual%20Basic\).md) pour quitter une procédure `Operator`.[Exit Statement](../Topic/Exit%20Statement%20\(Visual%20Basic\).md) n’accepte pas le mot clé `Operator` et l’instruction `End Operator` ne retourne pas le contrôle au code appelant.  
