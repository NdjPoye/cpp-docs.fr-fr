...
'membre' dans une déclaration d’interface explicite n’est pas un membre d’interface  
 ...
 Une tentative a été effectuée pour déclarer explicitement un membre d’[interface](../Topic/interface%20\(C%23%20Reference\).md) qui n’existe pas. Vous devez supprimer la déclaration ou la modifier pour qu’elle fasse référence à un membre d’interface valide.  
