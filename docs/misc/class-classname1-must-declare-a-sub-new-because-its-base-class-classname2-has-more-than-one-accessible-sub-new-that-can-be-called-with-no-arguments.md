...
Une classe dérivée ne déclare pas de constructeur et [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] ne peut pas en générer un car il ne peut pas déterminer le constructeur de classe de base à appeler.  
  
 Quand une classe dérivée ne déclare pas de constructeur, [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] tente de générer un constructeur sans paramètre implicite qui appelle `MyBase.New()`. S’il n’existe aucun constructeur accessible dans la classe de base pouvant être appelé sans arguments, ou s’il en existe plusieurs, [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] ne peut pas générer de constructeur implicite.  
