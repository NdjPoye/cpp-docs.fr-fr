...
1.  Créez le volet de fenêtre Outil, le frame et le package Visual Studio qui les implémente. Pour plus d'informations, consultez [Ajout d’une fenêtre outil](../Topic/Adding%20a%20Tool%20Window.md).  
...
2.  Ajoutez le <xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute> au package Visual Studio qui le fournit.  
    ...
    ```  
     ...
     Cela inscrit le PersistedWindowPane de la fenêtre Outil à ouvrir comme ancré à l’**Explorateur de solutions**. Pour plus d'informations, consultez [L’inscription d’une fenêtre outil](../Topic/Registering%20a%20Tool%20Window.md).  
