...
Une référence à l’assembly '\<`assemblyname`\>' contenant la définition de l’événement '\<`eventname`\>' est requise Ajoutez une référence à votre projet.  
 ...
 L’événement est défini dans une bibliothèque de liens dynamiques \(DLL\) ou un assembly qui n’est pas directement référencé dans votre projet. Le compilateur [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] nécessite une référence afin d’éviter toute ambiguïté au cas où l’événement serait défini dans plusieurs DLL ou assemblys.  
