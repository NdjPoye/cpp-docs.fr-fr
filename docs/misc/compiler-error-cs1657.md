...
Impossible de passer 'parameter' en tant qu’argument ref ou out car 'reason'  
 ...
 Cette erreur se produit quand une variable est passée en tant qu’argument [ref](../Topic/ref%20\(C%23%20Reference\).md) ou [out](../Topic/out%20\(C%23%20Reference\).md) dans un contexte dans lequel cette variable est en lecture seule. Les contextes en lecture seule incluent les variables d’itération [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md), les variables [using](../Topic/using%20Statement%20\(C%23%20Reference\).md) et les variables `fixed`. Pour résoudre cette erreur, n’appelez pas les fonctions qui acceptent la variable `foreach`, `using` ou `fixed` en tant que paramètre `ref` ou `out` dans des blocs `using`, des instructions `foreach` et des instructions `fixed`.  
