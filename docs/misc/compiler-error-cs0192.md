...
Impossible de passer les champs d’un champ readonly statique 'nom' en ref ou out \(sauf s’ils appartiennent à un constructeur statique\)  
 ...
 Un champ \(variable\) marqué avec le mot clé [readonly](../Topic/readonly%20\(C%23%20Reference\).md) ne peut pas être passé à un paramètre [ref](../Topic/ref%20\(C%23%20Reference\).md) ou [out](../Topic/out%20\(C%23%20Reference\).md), sauf à l’intérieur d’un constructeur. Pour plus d'informations, consultez [Champs](../Topic/Fields%20\(C%23%20Programming%20Guide\).md).  
  
 L’erreur CS0192 se produit également si le champ `readonly` est [static](../Topic/static%20\(C%23%20Reference\).md) et que le constructeur n’est pas marqué `static`.  
