...
-   Aucun autre fichier devant être copié localement n’a pu être copié dans le répertoire à partir duquel le projet est exécuté.  
 ...
 En règle générale, la `reason` citée à la fin du message d’erreur signale que le fichier est utilisé par un autre processus. Le fichier est probablement verrouillé par un autre processus dans [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)].  
