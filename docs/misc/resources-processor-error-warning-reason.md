...
Un message d'erreur ou d'avertissement s'affiche si un outil retourne une erreur ou un avertissement pendant le traitement d'un fichier .resx.  Dans le cadre du processus de génération, le système de projet transforme chaque fichier .resx en fichier binaire que le gestionnaire de ressources [!INCLUDE[dnprdnshort](../error-messages/tool-errors/includes/dnprdnshort_md.md)] comprend.  Cette transformation est réalisée à l'aide d'outils in\-process.  
