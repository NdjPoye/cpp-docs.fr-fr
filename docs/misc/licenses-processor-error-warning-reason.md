...
Un message d'erreur ou d'avertissement s'affiche si un outil retourne une erreur ou un avertissement pendant le traitement d'un fichier .licx.  Dans le cadre du processus de génération, le système de projet transforme le fichier Licenses.licx \(s'il est présent\) en fichier binaire que le gestionnaire de licences [!INCLUDE[dnprdnshort](../error-messages/tool-errors/includes/dnprdnshort_md.md)] comprend.  Cette transformation est réalisée à l'aide d'outils in\-process.  
  
 La cause la plus probable de cette erreur ou de cet avertissement est un fichier .licx incorrect.  Un fichier .licx peut être endommagé s'il est modifié à l'extérieur de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] ou dans [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] mais avec un éditeur de texte.  
