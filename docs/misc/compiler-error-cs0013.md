...
Erreur inattendue lors de l’écriture des métadonnées dans le fichier 'ModelStore.dll' \-\- 'Pas d’espace logique restant pour la création de chaînes utilisateur supplémentaires.  
 ...
 Le CLR \(Common Language Runtime\) .NET Framework n’a pas émis de métadonnées. Vérifiez que le chemin est correct et que le disque n’est pas saturé. Si le problème persiste, vous devrez peut\-être réparer ou réinstaller [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] et\/ou le .NET Framework.