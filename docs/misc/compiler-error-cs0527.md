...
Le type 'type' qui figure dans la liste des interfaces n’est pas une interface  
 ...
 Il est possible pour un [struct](../Topic/struct%20\(C%23%20Reference\).md) ou une [interface](../Topic/interface%20\(C%23%20Reference\).md) d’hériter d’une autre interface, mais pas à partir de n’importe quel autre type.  
