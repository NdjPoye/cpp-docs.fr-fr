...
Pour afficher dans la boîte de dialogue de **À propos de** et l'écran de démarrage, VSPackages doit implémenter l'interface d' <xref:Microsoft.VisualStudio.Shell.Interop.IVsInstalledProduct> .  Cela fournit les informations suivantes dans [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)]:  
...
3.  Implémentez <xref:Microsoft.VisualStudio.Shell.Interop.IVsInstalledProduct> à l'aide de le code suivant.  
     ...
     [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] appelle ces méthodes pour obtenir des informations pour stigmatiser le VSPackage.  la méthode de GetResourceString est utilisée pour localiser ces informations.  
