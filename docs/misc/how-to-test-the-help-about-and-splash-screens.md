...
Une fois que vous avez implémenté **À propos de** et la prise en charge de l'écran de démarrage, vous pouvez tester votre implémentation dans [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)].  
  
### Pour tester l'aide concernant la boîte de dialogue  
  
1.  À partir de l'invite de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] , exécutez devenv.exe avec le commutateur d' **\/setup** .  Pour exécuter dans l'environnement instance expérimentale de, tapez :  
...
2.  [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] exécuté dans la même racine de Registre qui a été mentionné précédemment, mais sans commutateur d' **\/setup** :  
...
2.  Exécutez [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] dans la même racine de Registre qui a été mentionné précédemment, mais avec le commutateur d' **\/splash** au lieu du commutateur d' **\/setup** .  
...
## Voir aussi  
 [Personnalisation de VSPackage](../misc/vspackage-branding.md)