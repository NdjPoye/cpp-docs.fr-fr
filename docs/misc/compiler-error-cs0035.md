...
L’opérateur 'opérateur' est ambigu pour un opérande de type 'type'  
 ...
 Plusieurs conversions sont disponibles et le compilateur ne sait pas laquelle choisir avant d’appliquer l’opérateur. Pour plus d’informations, consultez [Conversions définies par l'utilisateur basées sur un modèle](../misc/templated-user-defined-conversions.md) et [Conversion, opérateurs](../Topic/Conversion%20Operators%20\(C%23%20Programming%20Guide\).md).  
