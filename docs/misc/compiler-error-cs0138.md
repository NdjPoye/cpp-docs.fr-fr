...
Une directive d’espace de noms using ne peut être appliquée qu’aux espaces de noms ; 'type' est un type, pas un espace de noms  
 ...
 Une directive [using](../Topic/using%20\(C%23%20Reference\).md) accepte uniquement le nom d’un espace de noms en tant que paramètre. Pour plus d'informations, consultez [Espaces de noms](../Topic/Namespaces%20\(C%23%20Programming%20Guide\).md).  
