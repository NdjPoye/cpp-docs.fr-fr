---
title: "Ponctuation et caract&#232;res sp&#233;ciaux | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "C++"
  - "C"
helpviewer_keywords: 
  - "# (opérateur de préprocesseur)"
  - "caractères, spécial"
  - "signe dièse"
  - "ponctuation"
  - "caractères spéciaux"
ms.assetid: e239633e-36f6-4967-939c-bf2799323778
caps.latest.revision: 7
caps.handback.revision: 7
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
---
# Ponctuation et caract&#232;res sp&#233;ciaux
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

La ponctuation et les caractères spéciaux du jeu de caractères C ont plusieurs utilisations, qui varient de l'organisation du texte de programme à la définition des tâches effectuées par le compilateur ou le programme compilé.  Ils ne spécifient pas d'opération à effectuer.  Certains symboles de ponctuation sont également des opérateurs \(consultez [Opérateurs](../c-language/c-operators.md)\).  Le compilateur détermine leur utilisation à partir du contexte.  
  
## Syntaxe  
 `punctuator` : l'un des éléments suivants :  
 **\[ \]   \( \)   { }   \*   ,   :   \=   ;   ... \#**  
  
 Ces caractères ont des significations spéciales en C.  Leurs utilisations sont décrites dans les différentes sections de cette documentation.  Le signe dièse \(**\#**\) peut uniquement figurer dans les [directives de prétraitement](../preprocessor/preprocessor-directives.md).  
  
## Voir aussi  
 [Éléments de C](../c-language/elements-of-c.md)