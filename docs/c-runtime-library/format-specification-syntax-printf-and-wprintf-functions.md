...
## <a name="type"></a> Spécificateur de conversion de type
...
## <a name="flags"></a> Directives d’indicateur
...
## <a name="width"></a> Spécification de largeur
...
## <a name="precision"></a> Spécification de précision
...
## <a name="size"></a> Spécification de taille d’argument
...
|`intmax_t`<br />`uintmax_t`|**j** ou **I** (i majuscule)|**d**, **i**, **o**, **u**, **x** ou **X**|  
...
Les types `intmax_t`, `uintmax_t`, `ptrdiff_t` et `size_t` sont `__int32` ou `unsigned __int32` sur les plateformes 32 bits, et `__int64` ou `unsigned __int64` sur les plateformes 64 bits. Les préfixes de taille **I** (i majuscule), **j**, **t** et **z** acceptent la largeur correcte des arguments pour la plateforme.  
...
> [!NOTE]
>  Les préfixes de modificateur de taille d’argument **I** (i majuscule), **I32**, **I64** et **w** sont des extensions Microsoft et ne sont pas compatibles avec la norme ISO C. Le préfixe **h** (utilisé avec des données de type `char`) et le préfixe**l** (utilisé avec des données de type `double`) sont des extensions Microsoft.  
