...
|`_ismbcl2_l`|JIS niveau 2 : 0x989F<=`c`<=0xEAA4.|  
 ...
 Les fonctions vérifient que la valeur spécifiée `c` satisfait aux conditions de test décrites ci-dessus, mais ne vérifient pas que `c` est un caractère multioctet valide. Si l’octet de poids faible est compris dans les plages 0x00-0x3F, 0x7F ou 0xFD-0xFF, ces fonctions retournent une valeur différente de zéro, indiquant que le caractère satisfait à la condition de test. Utilisez [_ismbbtrail](../../c-runtime-library/reference/ismbbtrail-ismbbtrail-l.md) pour tester si le caractère multioctet est défini.  
