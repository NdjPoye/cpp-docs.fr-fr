...
dev_langs: 
  ...
  - "C"
helpviewer_keywords: 
  - "_tsetlocale (fonction)"
  - "_wsetlocale (fonction)"
  - "définir les paramètres régionaux"
  - "paramètres régionaux, définir"
  - "setlocale (fonction)"
  - "tsetlocale (fonction)"
  - "wsetlocale (fonction)"
...
|`_tsetlocale`|`setlocale`|`setlocale`|`_wsetlocale`|  
 ...
 `LC_NUMERIC`  
 Le caractère de virgule décimale pour les routines de sortie mise en forme \(comme `printf`\), pour les routines de conversion de données, et pour les informations de mise en forme non monétaire retournées par `localeconv`.  Outre le caractère de virgule décimale, `LC_NUMERIC` définit également le séparateur de milliers et la chaîne de contrôle de regroupement retournés par [localeconv](../../c-runtime-library/reference/localeconv.md).  
  
 `LC_TIME`  
 Les fonctions `strftime` et `wcsftime`.  
  
 Cette fonction valide le paramètre de catégorie.  Si le paramètre de catégorie ne fait pas partie des valeurs fournies dans le tableau précédent, il est fait appel au gestionnaire de paramètres invalides, comme décrit dans [Validation de paramètre](../../c-runtime-library/parameter-validation.md).  Si l'exécution est autorisée à se poursuivre, la fonction définit `errno` avec la valeur `EINVAL` et retourne `NULL`.  
 ...
 Définit les paramètres régionaux au nom des paramètres régionaux spécifiés par *\<localename\>*.  
  
 `setlocale( LC_ALL, "<language>_<country>" );`  
 Définit les paramètres régionaux à la langue et au pays\/région indiqués par *\<language\>* et *\<country\>*, en utilisant la page de codes par défaut fournie par le système d'exploitation hôte.  
  
 `setlocale( LC_ALL, "<language>_<country>.<code_page>" );`  
 Définit les paramètres régionaux à la langue, pays\/région, et page de codes indiqués par *\<language\>*, *\<country\>*, et les chaînes *\<code\_page\>*.  Vous pouvez utiliser différentes combinaisons de langues, pays\/région et page de codes.  Par exemple, cet appel définit les paramètres régionaux à français du Canada avec la page de codes 1252 :  
 ...
 Définit les paramètres régionaux à la langue donnée par *\<language\>*, et utilise le pays ou la région par défaut pour la langue spécifiée et la page de codes utilisateur par défaut ANSI du pays ou de la région obtenue du système d'exploitation hôte.  Par exemple, les appels suivants à `setlocale` sont fonctionnellement équivalents :  
 ...
 Définit la page de codes à la valeur indiquée par *\<code\_page\>*, avec les pays\/régions et langues par défaut \(comme défini par le système d'exploitation hôte\) pour la page de codes spécifiée.  
...
```  
  ...
  **Les paramètres régionaux du thread sont maintenant définis à en\-US.**  
**L'heure dans les paramètres régionaux en\-US est : « Wednesday, May 12, 2004 »**  
**Les paramètres régionaux du thread sont maintenant définis à de\-DE.**  
**L'heure dans les paramètres régionaux de\-DE est : 'Mittwoch, 12.  Mai 2004 '**    
