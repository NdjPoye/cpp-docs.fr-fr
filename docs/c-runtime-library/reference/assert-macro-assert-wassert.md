...
## Notes  
 ...
 La macro `assert` est activée dans les versions Release et Debug des bibliothèques Runtime C lorsque `NDEBUG` n’est pas défini. Lorsque `NDEBUG` est défini, la macro est disponible mais n’évalue pas son argument et n’a aucun effet. Lorsqu’elle est activée, la macro `assert` appelle `_wassert` pour son implémentation. D’autres macros d’assertion, [\_ASSERT](../../c-runtime-library/reference/assert-asserte-assert-expr-macros.md), [\_ASSERTE](../../c-runtime-library/reference/assert-asserte-assert-expr-macros.md) et [\_ASSERT\_EXPR](../../c-runtime-library/reference/assert-asserte-assert-expr-macros.md), sont également disponibles, mais elles évaluent seulement les expressions qui leur sont passées lorsque la macro [\_DEBUG](../../c-runtime-library/debug.md) a été définie et lorsqu’elles sont dans du code lié à la version Debug des bibliothèques Runtime C.  
