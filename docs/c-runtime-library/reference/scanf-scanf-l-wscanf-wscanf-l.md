...
Lit les données mises en forme du flux d'entrée standard. Il existe des versions plus sécurisées de ces fonctions. Consultez [scanf_s, _scanf_s_l, wscanf_s, _wscanf_s_l](../../c-runtime-library/reference/scanf-s-scanf-s-l-wscanf-s-wscanf-s-l.md).  
...
> [!IMPORTANT]
>  Quand il s’agit de lire une chaîne avec `scanf`, spécifiez toujours la largeur du format `%s` (par exemple, `"%32s"` au lieu de `"%s"`) ; une entrée mal formatée peut en effet facilement entraîner un dépassement de mémoire tampon. Vous pouvez aussi envisager d’utiliser [scanf_s, _scanf_s_l, wscanf_s, _wscanf_s_l](../../c-runtime-library/reference/scanf-s-scanf-s-l-wscanf-s-wscanf-s-l.md) ou [fgets](../../c-runtime-library/reference/fgets-fgetws.md).  
