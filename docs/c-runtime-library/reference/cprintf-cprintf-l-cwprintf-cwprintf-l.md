...
Met en forme et affiche les informations dans la console. Des versions plus sécurisées sont disponibles. Consultez [_cprintf_s, _cprintf_s_l, _cwprintf_s, _cwprintf_s_l](../../c-runtime-library/reference/cprintf-s-cprintf-s-l-cwprintf-s-cwprintf-s-l.md).  
...
## <a name="remarks"></a>Notes  
 ...
 `_cprintf` valide le paramètre `format`. Si `format` est un pointeur null, la fonction appelle le gestionnaire de paramètres non valides, comme décrit dans [Validation de paramètre](../../c-runtime-library/parameter-validation.md). Si l'exécution est autorisée à se poursuivre, la fonction retourne -1 et définit `errno` avec la valeur `EINVAL`.  
