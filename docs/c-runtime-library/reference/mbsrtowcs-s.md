...
Convertir une chaîne de caractères multioctets correspondant aux paramètres régionaux actuels en sa représentation sous forme de chaîne de caractères larges.  Version de [mbsrtowcs](../../c-runtime-library/reference/mbsrtowcs.md) qui intègre des améliorations de sécurité, comme décrit dans [Fonctionnalités de sécurité dans le CRT](../../c-runtime-library/security-features-in-the-crt.md).  
...
-   Le nombre caractères larges stockés dans la mémoire tampon de `wcstr` est égal à `count`.  
 ...
 Si les séquences pointées par `mbstr` et `wcstr` se chevauchent, le comportement de `mbsrtowcs_s` n'est pas défini.  `mbsrtowcs_s` est affecté par la catégorie LC\_TYPE des paramètres régionaux actuels.  
