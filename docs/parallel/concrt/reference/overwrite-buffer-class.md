...
|Nom|Description|  
|----------|-----------------|  
|[has_value](#has_value)|Vérifie si cette `overwrite_buffer` bloc de messagerie a déjà une valeur.|  
...
|Nom|Description|  
|----------|-----------------|  
|[accept_message](#accept_message)|Accepte un message qui a été transmis par ce `overwrite_buffer` messagerie bloc, en retournant une copie du message à l’appelant.|  
...
|[propagate_message](#propagate_message)|Passe de façon asynchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Il est appelé par le `propagate` la méthode appelée par un bloc source.|  
...
|[send_message](#send_message)|Passe de façon synchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Il est appelé par le `send` la méthode appelée par un bloc source.|  
