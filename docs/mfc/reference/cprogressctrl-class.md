...
|Nom|Description|  
|----------|-----------------|  
|[CProgressCtrl::Create](#create)|Crée un contrôle de barre de progression et l’attache à une `CProgressCtrl` objet.|  
|[CProgressCtrl::CreateEx](#createex)|Crée un contrôle de progression avec les styles étendus Windows spécifiés et l’attache à une `CProgressCtrl` objet.|  
...
|[CProgressCtrl::SetMarquee](#setmarquee)|Active le mode de sélection ou désactiver pour le contrôle de barre de progression actuel.|  
...
|[CProgressCtrl::StepIt](#stepit)|Avance la position actuelle d’un contrôle de barre de progression de l’incrément de l’étape (voir [SetStep](#setstep)) et redessine la barre pour refléter la nouvelle position.|  
  
## <a name="remarks"></a>Remarques  
 Un contrôle de barre de progression est une fenêtre d’une application peut utiliser pour indiquer la progression d’une opération longue. Il se compose d’un rectangle qui se remplit progressivement, de gauche à droite, avec le système de couleur de surbrillance que l’opération progresse.  
  
 Un contrôle de barre de progression a une plage et une position actuelle. La plage représente la durée totale de l’opération, et la position actuelle représente la progression de que l’application a effectué l’opération. La procédure de fenêtre utilise la plage et la position actuelle pour déterminer le pourcentage de la barre de progression à remplir avec la couleur de surbrillance. Étant donné que la plage et les valeurs de position actuelle sont exprimées sous forme d’entiers signés, la plage de valeurs de position actuelles possible provient â « 2 147 483 648 à 2 147 483 647 inclus.  
  
 Pour plus d’informations sur l’utilisation de `CProgressCtrl`, consultez [contrôles](../../mfc/controls-mfc.md) et [l’utilisation de CProgressCtrl](../../mfc/using-cprogressctrl.md).  
...
##  <a name="create"></a>CProgressCtrl::Create  
 Crée un contrôle de barre de progression et l’attache à une `CProgressCtrl` objet.  
...
### <a name="parameters"></a>Paramètres  
 `dwStyle`  
 Spécifie le style du contrôle de barre de progression. Appliquer n’importe quelle combinaison de stylesdescribed fenêtre dans [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)], en plus de la barre des styles de contrôle, au contrôle de progression suivante :  
...
- `PBS_SMOOTH`Affiche progressive et sans heurts de remplissage dans le contrôle de barre de progression. Sans cet indicateur, le contrôle remplit avec des blocs.  
  
 `rect`  
 Spécifie la taille et la position du contrôle de barre de progression. Il peut être soit un [CRect](../../atl-mfc-shared/reference/crect-class.md) objet ou un [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) structure. Étant donné que le contrôle doit être une fenêtre enfant, sont les coordonnées spécifiées par rapport à la zone cliente de la `pParentWnd`.  
  
 `pParentWnd`  
 Indique la progression de la fenêtre parent du contrôle barre généralement un `CDialog`. Il ne doit pas être **NULL.**  
  
 `nID`  
 Spécifie l’ID. du contrôle de barre de progression  
  
### <a name="return-value"></a>Valeur de retour  
 **TRUE** si le `CProgressCtrl` objet a été créé ; sinon **FALSE**.  
  
### <a name="remarks"></a>Remarques  
 Vous construisez un `CProgressCtrl` objet en deux étapes. Tout d’abord, appelez le constructeur, qui crée la `CProgressCtrl` de l’objet, puis appelez **créer**, ce qui crée le contrôle de barre de progression.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CProgressCtrl n °&2;](../../mfc/reference/codesnippet/cpp/cprogressctrl-class_2.cpp)]  
  
##  <a name="createex"></a>CProgressCtrl::CreateEx  
 Crée un contrôle (une fenêtre enfant) et l’associe à la `CProgressCtrl` objet.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `dwStyle`  
 Spécifie le style du contrôle de barre de progression. Appliquer n’importe quelle combinaison de styles de fenêtre décrit dans [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valeur de retour  
 La couleur de la barre de progression active, représentée comme un [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) valeur, ou `CLR_DEFAULT` si la couleur de barre de progression indicateur est la couleur par défaut.  
  
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [PBM_GETBARCOLOR](http://msdn.microsoft.com/library/windows/desktop/bb760826) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valeur de retour  
 La couleur d’arrière-plan de la barre de progression active, représentée comme un [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) valeur.  
  
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [PBM_GETBKCOLOR](http://msdn.microsoft.com/library/windows/desktop/bb760828) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Notes  
 La position du contrôle de barre de progression n’est pas l’emplacement physique de l’écran, mais plutôt est entre la limite supérieure et inférieure plage indiquée dans [SetRange](#setrange).  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CProgressCtrl n °&3;](../../mfc/reference/codesnippet/cpp/cprogressctrl-class_3.cpp)]  
  
##  <a name="getrange"></a>CProgressCtrl::GetRange  
 Obtient l’actuels limites inférieure et supérieure, ou une plage de contrôle de barre de progression.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [PBM_GETSTATE](http://msdn.microsoft.com/library/windows/desktop/bb760834) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Remarques  
 ...
 Cette méthode envoie le [PBM_GETSTEP](http://msdn.microsoft.com/library/windows/desktop/bb760836) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="offsetpos"></a>CProgressCtrl::OffsetPos  
 Avance la progression de la position actuelle du contrôle de la barre de l’incrément spécifié par `nPos` et redessine la barre pour refléter la nouvelle position.  
...
### <a name="parameters"></a>Paramètres  
 `nPos`  
 Quantité de passer à la position.  
...
### <a name="return-value"></a>Valeur de retour  
 La couleur précédente de la barre de progression, représentée comme un [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) valeur, ou `CLR_DEFAULT` si la couleur de la barre de progression est la couleur par défaut.  
  
### <a name="remarks"></a>Remarques  
 Le `SetBarColor` méthode définit la progression de la barre uniquement si de couleur un [!INCLUDE[windowsver](../../build/reference/includes/windowsver_md.md)] [thème](https://msdn.microsoft.com/library/windows/desktop/hh270423.aspx) n’est pas activée.  
  
 Cette méthode envoie le [PBM_SETBARCOLOR](http://msdn.microsoft.com/library/windows/desktop/bb760838) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Paramètres  
 `clrNew`  
 A **COLORREF** valeur qui spécifie la couleur d’arrière-plan. Spécifier la `CLR_DEFAULT` valeur à utiliser la couleur d’arrière-plan par défaut pour la barre de progression.  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CProgressCtrl n °&6;](../../mfc/reference/codesnippet/cpp/cprogressctrl-class_10.cpp)]  
  
##  <a name="setmarquee"></a>CProgressCtrl::SetMarquee  
 Active le mode de sélection ou désactiver pour le contrôle de barre de progression actuel.  
...
|[in] `nInterval`|Durée en millisecondes entre les mises à jour de l’animation de texte défilant.|  
...
### <a name="remarks"></a>Notes  
 ...
 Cette méthode envoie le [PBM_SETMARQUEE](http://msdn.microsoft.com/library/windows/desktop/bb760842) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="setpos"></a>CProgressCtrl::SetPos  
 Définit la progression de la position actuelle du contrôle comme spécifié par la barre `nPos` et redessine la barre pour refléter la nouvelle position.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [PBM_SETSTATE](http://msdn.microsoft.com/library/windows/desktop/bb760850) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Paramètres  
 *nEtape*  
 Incrément de la nouvelle étape.  
  
### <a name="return-value"></a>Valeur de retour  
 L’incrément d’étape précédente.  
