...
|`CMFCVisualManagerOffice2007::GetStyle`|Retourne le modèle de couleurs actuelle d' `CMFCVisualManagerOffice2007` interface GUI qui, ensuite, duplique Microsoft Office 2007 interface GUI.|  
...
|`CMFCVisualManagerOffice2007::SetStyle`|Définit le modèle de couleurs d' `CMFCVisualManagerOffice2007` interface GUI.|  
  
## Notes  
 Utilisez `CMFCVisualManagerOffice2007` pour modifier l'apparence visuelle de votre application pour ressembler à celle de Microsoft Office 2007.  Ce gestionnaire visuel requiert que vous définissez le style avant de l'utiliser.  Avant de passer ce gestionnaire visuel à `CMFCVisualManager::SetDefaultManager`, vous devez appeler la fonction statique `CMFCVisualManagerOffice2007::SetStyle`.  
  
## Exemple  
 L'exemple suivant montre comment utiliser le bureau du ouvrez visual 2007.  Cet extrait de code fait partie d' [Exemple d'alerte de démonstration de Bureau](../../top/visual-cpp-samples.md).  
