...
|Nom|Description|  
|----------|-----------------|  
|[CTreeCtrl::Create](#create)|Crée un contrôle d’arborescence et l’attache à une `CTreeCtrl` objet.|  
|[CTreeCtrl::CreateDragImage](#createdragimage)|Crée une image bitmap de glissement pour l’élément d’arborescence spécifié.|  
|[CTreeCtrl::CreateEx](#createex)|Crée un contrôle d’arborescence avec les styles étendus Windows spécifiés et l’attache à une `CTreeCtrl` objet.|  
|[CTreeCtrl::DeleteAllItems](#deleteallitems)|Supprime tous les éléments dans un contrôle d’arborescence.|  
|[CTreeCtrl::DeleteItem](#deleteitem)|Supprime un élément dans un contrôle d’arborescence.|  
...
|[CTreeCtrl::EnsureVisible](#ensurevisible)|Garantit qu’un élément d’arborescence est visible dans le contrôle arborescence.|  
...
|[CTreeCtrl::GetCheck](#getcheck)|Récupère l’état d’activation d’un élément de l’arborescence.|  
|[CTreeCtrl::GetChildItem](#getchilditem)|Récupère le contrôle enfant d’un élément d’arborescence spécifié.|  
...
|[CTreeCtrl::GetImageList](#getimagelist)|Récupère le handle de la liste d’images associée à un contrôle d’arborescence.|  
|[CTreeCtrl::GetIndent](#getindent)|Récupère le décalage (en pixels) d’un élément d’arborescence à partir de son parent.|  
|[CTreeCtrl::GetInsertMarkColor](#getinsertmarkcolor)|Récupère la couleur utilisée pour dessiner la marque d’insertion de l’arborescence.|  
|[CTreeCtrl::GetItem](#getitem)|Récupère les attributs d’un élément d’arborescence spécifié.|  
|[CTreeCtrl::GetItemData](#getitemdata)|Retourne la valeur de spécifiques aux applications 32 bits associés liée un élément.|  
|[CTreeCtrl::GetItemExpandedImageIndex](#getitemexpandedimageindex)|Récupère l’index de l’image à afficher lorsque l’élément spécifié du contrôle de l’arborescence en cours est dans l’état développé.|  
|[CTreeCtrl::GetItemHeight](#getitemheight)|Récupère la hauteur actuelle de l’affichage des éléments d’arborescence.|  
...
|[CTreeCtrl::GetScrollTime](#getscrolltime)|Récupère l’heure de défilement maximale pour le contrôle TreeView.|  
...
|[CTreeCtrl::MapAccIdToItem](#mapaccidtoitem)|Mappe l’identificateur spécifié d’accessibilité pour le handle à un élément d’arborescence dans le contrôle d’arborescence en cours.|  
...
|[CTreeCtrl::Select](#select)|Sélectionne, affiche ou redessine un élément d’arborescence spécifié.|  
|[CTreeCtrl::SelectDropTarget](#selectdroptarget)|La cible d’une opération de glisser-déplacer pour redessiner l’élément d’arborescence.|  
...
|[CTreeCtrl::SetAutoscrollInfo](#setautoscrollinfo)|Définit la vitesse de défilement automatique du contrôle de l’arborescence en cours.|  
|[CTreeCtrl::SetBkColor](#setbkcolor)|Définit la couleur d’arrière-plan du contrôle.|  
|[CTreeCtrl::SetCheck](#setcheck)|Définit l’état d’activation d’un élément de l’arborescence.|  
|[CTreeCtrl::SetExtendedStyle](#setextendedstyle)|Définit les styles étendus pour le contrôle d’arborescence actuel.|  
|[CTreeCtrl::SetImageList](#setimagelist)|Définit le handle de la liste d’images associée à un contrôle d’arborescence.|  
...
|[CTreeCtrl::SetInsertMarkColor](#setinsertmarkcolor)|Définit la couleur utilisée pour dessiner la marque d’insertion de l’arborescence.|  
|[CTreeCtrl::SetItem](#setitem)|Définit les attributs d’un élément d’arborescence spécifié.|  
|[CTreeCtrl::SetItemData](#setitemdata)|Définit la valeur de spécifiques aux applications 32 bits associés liée un élément.|  
|[CTreeCtrl::SetItemExpandedImageIndex](#setitemexpandedimageindex)|Définit l’index de l’image à afficher lorsque l’élément spécifié du contrôle de l’arborescence en cours est dans l’état développé.|  
...
|[CTreeCtrl::SetLineColor](#setlinecolor)|Définit la couleur de la ligne actuelle pour le contrôle TreeView.|  
...
## <a name="remarks"></a>Notes  
 ...
 Pour plus d’informations sur l’utilisation de `CTreeCtrl`, voir :  
  
- [Contrôles](../../mfc/controls-mfc.md)  
  
- [Utilisation de CTreeCtrl](../../mfc/using-ctreectrl.md)  
  
- [Référence du contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb759988) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
-   L’article de la Base de connaissances Q222905 : comment faire : afficher un Menu contextuel pour CTreeCtrl  
...
### <a name="parameters"></a>Paramètres  
 `dwStyle`  
 Spécifie le style du contrôle arborescence de la vue. Appliquer des styles de fenêtre, décrits dans [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679)et toute combinaison de [styles de contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb760013) comme décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
 `rect`  
 Spécifie la taille et la position du contrôle tree view. Il peut être soit un [CRect](../../atl-mfc-shared/reference/crect-class.md) objet ou un [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) structure.  
  
 `pParentWnd`  
 Spécifie la vue fenêtre du contrôle parent, généralement un `CDialog`. Il ne doit pas être **NULL**.  
  
 `nID`  
 Spécifie ID. du contrôle arborescence  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si l’initialisation a réussi ; sinon 0.  
  
### <a name="remarks"></a>Remarques  
 Si vous souhaitez créer le contrôle d’arborescence en tant que fenêtre enfant d’une autre fenêtre, utilisez la **créer** fonction membre. Si vous créez le contrôle d’arborescence à l’aide **créer**, vous devez lui transmettre **WS_VISIBLE**, en plus des autres styles d’arborescence.  
  
 Vous construisez un `CTreeCtrl` en deux étapes. Premier appel du constructeur, puis appelez **créer**, qui crée le contrôle TreeView et l’attache à le `CTreeCtrl` objet.  
...
##  <a name="a-namecreateexa--ctreectrlcreateex"></a><a name="createex"></a>CTreeCtrl::CreateEx  
 Appelez cette fonction pour créer un contrôle (une fenêtre enfant) et y associer le `CTreeCtrl` objet.  
...
##  <a name="a-namecreatedragimagea--ctreectrlcreatedragimage"></a><a name="createdragimage"></a>CTreeCtrl::CreateDragImage  
 Appelez cette fonction pour créer une image bitmap de glissement pour l’élément donné dans un contrôle d’arborescence, créer une liste d’images pour l’image bitmap et l’ajouter à la liste d’images.  
...
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers la liste d’images à laquelle l’image bitmap de glissement a été ajouté, si l’opération a réussi ; dans le cas contraire **NULL**.  
...
### <a name="parameters"></a>Paramètres  
 `hItem`  
 Handle de l’élément de l’arborescence doit être supprimé. Si *hitem* a le **TVI_ROOT** valeur, tous les éléments sont supprimés dans le contrôle arborescence.  
...
|---------------|-----------------|  
|[in] `fCancelWithoutSave`|`true`pour ignorer les modifications à l’élément de l’arborescence avant de conclure l’opération de modification, ou `false` pour enregistrer les modifications à l’élément de l’arborescence avant de conclure l’opération.|  
  
### <a name="return-value"></a>Valeur de retour  
 `true`Si cette méthode a réussi ; dans le cas contraire, `false`.  
  
### <a name="remarks"></a>Notes  
 Cette méthode envoie le [TVM_ENDEDITLABELNOW](http://msdn.microsoft.com/library/windows/desktop/bb773564) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne **TRUE** si le système qui la font défiler les éléments dans le contrôle d’arborescence pour vous assurer que l’élément spécifié est visible. Sinon, la valeur de retour est **FALSE**.  
...
##  <a name="a-nameexpanda--ctreectrlexpand"></a><a name="expand"></a>CTreeCtrl::Expand  
 Appelez cette fonction pour développer ou réduire la liste des éléments enfants, si éventuellement associé à l’élément parent donné.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `nCode`  
 Indicateur spécifiant le type d’action à entreprendre. Cet indicateur peut avoir une des valeurs suivantes :  
...
### <a name="return-value"></a>Valeur de retour  
 A **COLORREF** valeur qui représente la couleur d’arrière-plan de fenêtre en cours pour le contrôle. Si cette valeur est -1, le contrôle utilise la couleur de la fenêtre. Dans ce cas, vous pouvez utiliser `::GetSysColor(COLOR_WINDOW)` pour obtenir la couleur système actuelle qui utilise le contrôle.  
...
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si l’élément de l’arborescence est activé ; sinon 0.  
...
### <a name="parameters"></a>Paramètres  
 `hItem`  
 Handle d’un élément de l’arborescence.  
...
### <a name="return-value"></a>Valeur de retour  
 Une valeur qui contient une combinaison d’opérations de bits (OR) du contrôle de l’arborescence en cours de l’étendue de styles. Pour plus d’informations, consultez [Styles étendus de contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb759981).  
  
### <a name="remarks"></a>Notes  
 Cette méthode envoie le [TVM_GETEXTENDEDSTYLE](http://msdn.microsoft.com/library/windows/desktop/bb773580) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="a-namegetimagelista--ctreectrlgetimagelist"></a><a name="getimagelist"></a>CTreeCtrl::GetImageList  
 Appelez cette fonction pour récupérer le descripteur de la normale ou la liste d’images état associé au contrôle TreeView.  
...
- `TVSIL_NORMAL`Récupère la liste image normale, qui contient les images sélectionnés et pour l’élément d’arborescence.  
  
- `TVSIL_STATE`Récupère la liste d’images état, qui contient les images pour les articles de vue arborescence qui se trouvent dans un état défini par l’utilisateur.  
  
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers la liste de contrôle image en cas de réussite ; dans le cas contraire **NULL**.  
  
### <a name="remarks"></a>Remarques  
 Chaque élément dans un contrôle d’arborescence peut avoir une paire d’images bitmaps associées. Une image est affichée lorsque l'élément est sélectionné, et l'autre est affichée lorsque l'élément n'est pas sélectionné. Par exemple, un élément peut afficher un dossier ouvert lorsqu’il est sélectionné et un dossier fermé lorsqu’il ne l’est pas.  
...
### <a name="parameters"></a>Paramètres  
 `pItem`  
 Un pointeur vers un [structure TVITEM](http://msdn.microsoft.com/library/windows/desktop/bb773456) de structure, comme décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="a-namegetitemdataa--ctreectrlgetitemdata"></a><a name="getitemdata"></a>CTreeCtrl::GetItemData  
 Appelez cette fonction pour récupérer la valeur de spécifiques aux applications 32 bits associée à l’élément spécifié.  
...
##  <a name="a-namegetitemexpandedimageindexa--ctreectrlgetitemexpandedimageindex"></a><a name="getitemexpandedimageindex"></a>CTreeCtrl::GetItemExpandedImageIndex  
 Récupère l’index de l’image à afficher lorsque l’élément spécifié du contrôle de l’arborescence en cours est dans l’état développé.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [TVM_GETITEM](http://msdn.microsoft.com/library/windows/desktop/bb773596) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Que message renvoie la [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure qui décrit l’élément de contrôle de l’arborescence, puis cette méthode récupère le `iExpandedImage` membre de cette structure.  
...
##  <a name="a-namegetitemimagea--ctreectrlgetitemimage"></a><a name="getitemimage"></a>CTreeCtrl::GetItemImage  
 Chaque élément dans un contrôle d’arborescence peut avoir une paire d’images bitmaps associées.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `nImage`  
 Entier qui reçoit l’index d’image de l’élément dans la liste d’images du contrôle tree view.  
  
 `nSelectedImage`  
 Entier qui reçoit l’index d’image de l’élément sélectionné dans la liste d’images du contrôle tree view.  
...
### <a name="remarks"></a>Remarques  
 ...
 Appelez cette fonction pour récupérer l’index d’image de l’élément et son image sélectionnée dans la liste d’images du contrôle tree view.  
...
### <a name="remarks"></a>Notes  
 Chaque élément de l’arborescence est délimitée par un rectangle de graphiques. Chaque fois qu’un clic sur un point dans le rectangle, l’élément est dit *d’accès*. Cette méthode retourne le plus grand rectangle de sorte que lorsque vous cliquez sur un point dans le rectangle, l’élément identifié par le `hItem` paramètre est atteint.  
  
 Cette méthode envoie le `TVM_GETITEMPARTRECT` message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Pour plus d’informations, consultez la [TreeView_GetItemPartRect](http://msdn.microsoft.com/library/windows/desktop/bb773847) (macro).  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant utilise un identificateur d’accessibilité et la [CTreeCtrl::MapAccIdToItem](#mapaccidtoitem) méthode pour récupérer un handle vers l’élément d’arborescence racine. L’exemple utilise le handle et les [CTreeCtrl::GetItemPartRect](#getitempartrect) méthode pour dessiner un rectangle autour de cet élément 3D. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États. Nous avons utilisé le [CTreeCtrl::MapItemToAccID](#mapitemtoaccid) pour associer l’élément de l’arborescence racine avec un identificateur d’accessibilité.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `lpRect`  
 Pointeur vers un [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) structure qui reçoit le rectangle englobant. Les coordonnées sont exprimées par rapport au coin supérieur gauche du contrôle arborescence.  
  
 *bTextOnly*  
 Si ce paramètre est différent de zéro, le rectangle englobant inclut uniquement le texte de l’élément. Sinon, elle inclut la ligne entière qui occupe de l’élément dans le contrôle arborescence.  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si l’élément est visible, le rectangle englobant contenu `lpRect`. Sinon, 0 avec `lpRect` sans être initialisé.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `nStateMask`  
 Masque qui indique un ou plusieurs États à récupérer. Pour plus d’informations sur les valeurs possibles pour `nStateMask`, consultez la rubrique sur la **état** et **stateMask** les membres de la [structure TVITEM](http://msdn.microsoft.com/library/windows/desktop/bb773456) structure dans le [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="return-value"></a>Valeur de retour  
 A **UINT** qui contient l’opérateur de bits OR de valeurs spécifiées par nStateMask. Pour plus d’informations sur les valeurs possibles, consultez [CTreeCtrl::GetItem](#getitem). Pour rechercher la valeur d’un état spécifique, effectuez une opération AND au niveau du bit de la valeur d’état et la valeur de retour, comme illustré dans l’exemple suivant.  
...
### <a name="remarks"></a>Notes  
 Cette méthode envoie le [TVM_GETITEM](http://msdn.microsoft.com/library/windows/desktop/bb773596) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Que message renvoie la [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure qui décrit l’élément de contrôle d’arborescence et cette méthode récupère le `uStateEx` membre de cette structure.  
...
##  <a name="a-namegetlastvisibleitema--ctreectrlgetlastvisibleitem"></a><a name="getlastvisibleitem"></a>CTreeCtrl::GetLastVisibleItem  
 Récupère le dernier élément des nœuds dans le contrôle d’arborescence en cours.  
...
### <a name="return-value"></a>Valeur de retour  
 Le handle vers le dernier élément des nœuds si la méthode a réussi ; dans le cas contraire, `NULL`.  
  
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [TVM_GETNEXTITEM](http://msdn.microsoft.com/library/windows/desktop/bb773622) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Pour plus d’informations, consultez la `TVGN_LASTVISIBLE` indicateur dans le `flag` paramètre de ce message.  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant récupère un handle vers le dernier élément du nœud de l’arborescence des et puis dessine un rectangle 3D autour de cet élément. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États.  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTreeCtrl n °&19;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_22.cpp)]  
  
##  <a name="a-namegetnextitema--ctreectrlgetnextitem"></a><a name="getnextitem"></a>CTreeCtrl::GetNextItem  
 Appel de cette fonction pour récupérer l’arborescence de la vue item qui est la relation spécifiée, indiquée par le `nCode` paramètre, à `hItem`.  
...
### <a name="remarks"></a>Remarques  
 Cette fonction retourne **NULL** si l’élément en cours de récupération est le nœud racine de l’arborescence. Par exemple, si vous utilisez ce message avec le `TVGN_PARENT` indicateur sur un enfant de premier niveau du nœud racine de l’arborescence, renvoie le message **NULL**.  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTreeCtrl n °&21;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_24.cpp)]  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTreeCtrl n °&23;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_26.cpp)]  
  
##  <a name="a-namegetrootitema--ctreectrlgetrootitem"></a><a name="getrootitem"></a>CTreeCtrl::GetRootItem  
 Appelez cette fonction pour récupérer l’élément racine de l’arborescence.  
...
##  <a name="a-namegetscrolltimea--ctreectrlgetscrolltime"></a><a name="getscrolltime"></a>CTreeCtrl::GetScrollTime  
 Appelez cette fonction membre pour récupérer le temps maximal de défilement pour le contrôle TreeView.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [TVM_GETSELECTEDCOUNT](http://msdn.microsoft.com/library/windows/desktop/bb773629) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Remarques  
 L’implémentation MFC de `GetToolTips` renvoie un `CToolTipCtrl` objet, qui est utilisé par le contrôle d’arborescence, plutôt qu’un handle à un contrôle d’info-bulle.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `pHitTestInfo`  
 Adresse d’un [TVHITTESTINFO](http://msdn.microsoft.com/library/windows/desktop/bb773448) structure qui contient la position des correspondances test et qui reçoit des informations sur les résultats du test d’atteinte.  
...
|TVHT_NOWHERE|Dans la zone cliente, mais au-dessous du dernier élément.|  
...
|TVHT_ONITEMSTATEICON|Sur l’icône d’état d’un élément d’arborescence qui est dans un état défini par l’utilisateur.|  
 ...
 *lpInsertStruct*  
 ...
 `nImage`  
 Index de l’image de l’élément dans la liste d’images du contrôle tree view.  
  
 `nSelectedImage`  
 Index de l’image de l’élément sélectionné dans la liste d’images du contrôle tree view.  
 ...
 `nStateMask`  
 Spécifie les États doivent être définis. Consultez le `TVITEM` de la structure dans le [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Notes  
 L’exemple illustre les situations dans lesquelles vous souhaiterez utiliser chaque version de la fonction lors de l’insertion d’un élément de l’arborescence.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[27 NVC_MFC_CTreeCtrl](../../mfc/reference/codesnippet/cpp/ctreectrl-class_30.cpp)]  
...
### <a name="remarks"></a>Remarques  
 Si, par conséquent, vous pouvez ensuite utiliser [CTreeCtrl::GetChildItem](#getchilditem) pour récupérer les éléments enfants.  
...
##  <a name="a-namemapaccidtoitema--ctreectrlmapaccidtoitem"></a><a name="mapaccidtoitem"></a>CTreeCtrl::MapAccIdToItem  
 Mappe l’identificateur spécifié d’accessibilité pour le handle d’un élément d’arborescence dans le contrôle d’arborescence en cours.  
...
|Paramètre|Description|  
|---------------|-----------------|  
|[in] `uAccId`|Identificateur d’accessibilité d’un élément dans l’élément de l’arborescence.|  
  
### <a name="return-value"></a>Valeur de retour  
 Le handle à un élément de l’arborescence ( `HTREEITEM`) qui correspond à la `uAccId` paramètre. Pour plus d’informations, consultez la `hItem` membre de la [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure.  
  
### <a name="remarks"></a>Remarques  
 Outils d’accessibilité sont les applications qui aident les personnes handicapées utilisent des ordinateurs. Un identificateur d’accessibilité est utilisé par le `IAccessible` interface pour spécifier de manière unique un élément dans une fenêtre. Pour plus d’informations sur les identificateurs d’accessibilité, recherchez la rubrique « Sur Active Accessibility Support » à le [Microsoft Developer Network](http://go.microsoft.com/fwlink/linkid=56322).  
  
 Cette méthode envoie le [TVM_MAPACCIDTOHTREEITEM](http://msdn.microsoft.com/library/windows/desktop/bb773734) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
|Paramètre|Description|  
|---------------|-----------------|  
|[in] `hItem`|Handle d’un élément de l’arborescence du contrôle. Pour plus d’informations, consultez la `hItem` membre de la [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure.|  
...
### <a name="remarks"></a>Remarques  
 ...
 Cette méthode envoie le [TVM_MAPHTREEITEMTOACCID](http://msdn.microsoft.com/library/windows/desktop/bb773735) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant obtient un numéro d’identification d’un élément de contrôle d’arborescence. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États. Cet exemple de code obtienne un numéro d’identification unique pour le nœud de pays/région.  
...
##  <a name="a-nameselecta--ctreectrlselect"></a><a name="select"></a>CTreeCtrl::Select  
 Appelez cette fonction pour sélectionner l’élément donné de l’arborescence, faites défiler l’élément dans la vue ou redessiner l’élément dans le style utilisé pour indiquer la cible d’une opération de glisser-déplacer.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `nCode`  
 Le type d’action à exécuter. Ce paramètre peut être une des valeurs suivantes :  
  
- `TVGN_CARET`Définit la sélection à l’élément donné.  
  
- `TVGN_DROPHILITE`Dans le style utilisé pour indiquer la cible d’une opération de glisser-déplacer pour redessiner l’élément donné.  
  
- `TVGN_FIRSTVISIBLE`Fait défiler la vue d’arborescence verticalement afin que l’élément spécifié est le premier élément visible.  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Notes  
 Si `nCode` contient la valeur `TVGN_CARET`, la fenêtre parente reçoit le **TVN_SELCHANGING** et **TVN_SELCHANGED** des messages de notification. En outre, si l’élément spécifié est l’enfant d’un élément parent réduit, liste du parent des éléments enfants est développée pour afficher l’élément spécifié. Dans ce cas, la fenêtre parente reçoit le **TVN_ITEMEXPANDING** et **TVN_ITEMEXPANDED** messages de notification.  
...
### <a name="remarks"></a>Notes  
 Si `hItem` est **NULL**, cette fonction ne sélectionne aucun élément.  
...
##  <a name="a-nameselectsetfirstvisiblea--ctreectrlselectsetfirstvisible"></a><a name="selectsetfirstvisible"></a>CTreeCtrl::SelectSetFirstVisible  
 Appelez cette fonction pour faire défiler la vue d’arborescence verticalement afin que l’élément spécifié est le premier élément visible.  
...
### <a name="remarks"></a>Remarques  
 La fonction envoie un message dans la fenêtre avec le `TVM_SELECTITEM` et `TVGN_FIRSTVISIBLE` les paramètres de message.  
...
##  <a name="a-namesetautoscrollinfoa--ctreectrlsetautoscrollinfo"></a><a name="setautoscrollinfo"></a>CTreeCtrl::SetAutoscrollInfo  
 Définit la vitesse de défilement automatique du contrôle de l’arborescence en cours.  
...
### <a name="remarks"></a>Notes  
 Les paramètres de défilement automatique sont utilisés pour défiler dans l’affichage d’un élément qui n’est pas visible. Le contrôle d’arborescence doit avoir le `TVS_EX_AUTOHSCROLL` étendu de style, qui est décrit dans [Styles étendus de contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb759981).  
  
 Cette méthode envoie le [TVM_SETAUTOSCROLLINFO](http://msdn.microsoft.com/library/windows/desktop/bb773738) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant définit le comportement de défilement automatique du contrôle de l’arborescence en cours. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États. Nous avons intentionnellement apporté le contrôle d’arborescence étroit afin qu’elle doit faire défiler automatiquement pour afficher l’élément d’arborescence qui a le focus. L’exemple de code définit le contrôle d’arborescence pour défiler automatiquement 30 pixels par seconde toutes les 5 secondes jusqu'à ce que l’élément d’arborescence est affichée.  
...
### <a name="parameters"></a>Paramètres  
 `clr`  
 A **COLORREF** valeur qui contient la nouvelle couleur d’arrière-plan. Si cette valeur est -1, le contrôle revient à l’aide de la couleur système pour la couleur d’arrière-plan.  
...
##  <a name="a-namesetchecka--ctreectrlsetcheck"></a><a name="setcheck"></a>CTreeCtrl::SetCheck  
 Appelez cette fonction membre pour définir l’état d’activation d’un élément de l’arborescence.  
...
### <a name="remarks"></a>Remarques  
 Lorsque l’élément de l’arborescence est activée ( `fCheck` la valeur **TRUE**), l’élément s’affiche avec une coche adjacent.  
...
|Paramètre|Description|  
|---------------|-----------------|  
|[in] `dwExMask`|Masque de bits qui spécifie les styles dans le contrôle d’arborescence actuel sont affectés par cette méthode. Si ce paramètre est égal à zéro, elle est ignorée et la valeur de le `dwExStyles` paramètre est assigné au contrôle de l’arborescence.<br /><br /> Spécifiez zéro ou une combinaison au niveau du bit (ou) des styles décrites dans [Styles étendus de contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb759981).|  
|[in] `dwExStyles`|Masque de bits qui spécifie les styles dans l’arborescence actuelle du contrôle pour définir ou effacer.<br /><br /> Pour définir une combinaison de styles, spécifiez une combinaison au niveau du bit (ou) des styles décrites dans [Styles étendus de contrôle d’arborescence](http://msdn.microsoft.com/library/windows/desktop/bb759981). Pour supprimer un jeu de styles, indiquez la valeur zéro.|  
  
### <a name="return-value"></a>Valeur de retour  
 Une valeur qui contient la dernière étendue de styles de contrôle.  
  
### <a name="remarks"></a>Remarques  
 Cette méthode efface les styles spécifiés dans le `dwExMask` paramètre, puis définit les styles spécifiés dans le `dwExStyles` paramètre. Seuls les styles étendus qui correspondent aux bits de `dwExMask` modifier.  
  
 Cette méthode envoie le [TVM_SETEXTENDEDSTYLE](http://msdn.microsoft.com/library/windows/desktop/bb773744) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant ajoute le `TVS_EX_AUTOHSCROLL` étendu de style au contrôle d’arborescence en cours. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États. Nous avons intentionnellement apporté le contrôle d’arborescence étroit afin qu’elle doit faire défiler automatiquement pour afficher l’élément d’arborescence qui a le focus.  
...
##  <a name="a-namesetimagelista--ctreectrlsetimagelist"></a><a name="setimagelist"></a>CTreeCtrl::SetImageList  
 Appelez cette fonction pour définir la normale ou la liste d’images état d’une arborescence de contrôle de la vue et redessiner le contrôle à l’aide de nouvelles images.  
...
### <a name="parameters"></a>Paramètres  
 `pImageList`  
 Pointeur vers la liste d’images à affecter. Si `pImageList` est **NULL**, toutes les images sont supprimés dans le contrôle arborescence.  
  
 `nImageListType`  
 Type de liste d’images à définir. La liste d’images peut être une des valeurs suivantes :  
  
- `TVSIL_NORMAL`Définit la liste image normale, qui contient les images sélectionnés et pour l’élément d’arborescence. Vous devez utiliser cet état pour les images de superposition.  
  
- `TVSIL_STATE`Définit la liste d’images état, qui contient les images pour les articles de vue arborescence qui se trouvent dans un état défini par l’utilisateur.  
  
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers la liste d’images précédent, le cas échéant ; dans le cas contraire **NULL**.  
...
### <a name="parameters"></a>Paramètres  
 `hItem`  
 **HTREEITEM** qui spécifie à quel élément de la marque d’insertion est placée. Si cet argument est **NULL**, la marque d’insertion est supprimée.  
  
 *fAfter*  
 **BOOL** valeur qui spécifie si la marque d’insertion est placée avant ou après l’élément spécifié. Si cet argument est différente de zéro, la marque d’insertion est placée après l’élément. Si cet argument est zéro, la marque d’insertion est placée avant l’élément.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `hItem`  
 Handle de l’élément dont les attributs doivent être définis. Consultez le **hItem** membre de la `TVITEM` structure dans le [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
 ...
 `nImage`  
 Index de l’image de l’élément dans la liste d’images du contrôle tree view. Consultez le `iImage` membre de la `TVITEM` structure.  
  
 `nSelectedImage`  
 Index de l’image de l’élément sélectionné dans la liste d’images du contrôle tree view. Consultez le **iSelectedImage** membre de la `TVITEM` structure.  
  
 `nState`  
 Spécifie des valeurs pour les États de l’élément. Consultez le **état** membre de la `TVITEM` structure.  
  
 `nStateMask`  
 Spécifie les États doivent être définis. Consultez le **stateMask** membre de la `TVITEM` structure.  
...
### <a name="remarks"></a>Notes  
 Dans le `TVITEM` structure, la **hItem** membre identifie l’élément et le **masque** membre spécifie quels attributs à définir.  
  
 Si le **masque** membre ou le `nMask` paramètre spécifie le `TVIF_TEXT` valeur, le **pszText** membre ou le `lpszItem` est l’adresse d’une chaîne terminée par null et la **cchTextMax** membre est ignoré. Si **masque** (ou `nMask`) spécifie la `TVIF_STATE` valeur, le **stateMask** membre ou le `nStateMask` paramètre spécifie quel élément États à modifier et la **état** membre ou `nState` paramètre contient les valeurs de ces États.  
...
##  <a name="a-namesetitemdataa--ctreectrlsetitemdata"></a><a name="setitemdata"></a>CTreeCtrl::SetItemData  
 Appelez cette fonction pour que la valeur 32 bits spécifique à l’application associée à l’élément spécifié.  
...
##  <a name="a-namesetitemexpandedimageindexa--ctreectrlsetitemexpandedimageindex"></a><a name="setitemexpandedimageindex"></a>CTreeCtrl::SetItemExpandedImageIndex  
 Définit l’index de l’image à afficher lorsque l’élément spécifié du contrôle de l’arborescence en cours est dans l’état développé.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode envoie le [TVM_SETITEM](http://msdn.microsoft.com/library/windows/desktop/bb773758) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Cette méthode assigne le `iExpandedImage` paramètre à la `iExpandedImage` membre une [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure, puis utilise cette structure dans le message.  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant est un test simple pour déterminer si les [CTreeCtrl::GetItemExpandedImageIndex](#getitemexpandedimageindex) méthode retourne la valeur définie par le [CTreeCtrl::SetItemExpandedImageIndex](#setitemexpandedimageindex) (méthode). Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États.  
...
### <a name="parameters"></a>Paramètres  
 `cyHeight`  
 Spécifie la nouvelle hauteur de chaque élément dans l’arborescence, en pixels. Si cet argument est inférieure à la hauteur des images, elle sera définie à la hauteur des images. Si cet argument n’est pas encore, il est arrondi vers le plus proche même valeur. Si cet argument est -1, le contrôle revient à l’aide de sa hauteur par défaut de l’élément.  
...
### <a name="remarks"></a>Notes  
 Chaque élément dans un contrôle d’arborescence peut avoir une paire d’images bitmaps associées. Les images apparaissent à gauche de l'étiquette d'un élément. Une image est affichée lorsque l'élément est sélectionné, et l'autre est affichée lorsque l'élément n'est pas sélectionné. Par exemple, un élément peut afficher un dossier ouvert lorsqu'il est sélectionné et un dossier fermé lorsqu'il ne l'est pas.  
  
 Appelez cette fonction pour définir l’index de l’image de l’élément et son image sélectionnée dans la liste d’images du contrôle tree view.  
...
### <a name="parameters"></a>Paramètres  
 `hItem`  
 Handle de l’élément dont l’état doit être définie.  
  
 `nState`  
 Spécifie de nouveaux États de l’élément.  
...
### <a name="remarks"></a>Notes  
 Cette méthode envoie le [TVM_SETITEM](http://msdn.microsoft.com/library/windows/desktop/bb773758) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Cette méthode assigne le `uStateEx` paramètre à la `uStateEx` membre une [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure, puis utilise cette structure dans le message.  
...
### <a name="example"></a>Exemple  
 L’exemple de code suivant définit un élément de l’arborescence à l’état désactivé. Dans la section précédente de l’exemple de code qui n’est pas visible, nous avons créé une arborescence qui se compose d’un nœud de pays/région pour les États-Unis, les sous-nœuds pour les États de Washington et de Pennsylvanie et éléments de l’arborescence des villes dans ces États. Cet exemple de code définit le nœud de Pennsylvanie à l’état désactivé.  
...
##  <a name="a-namesetlinecolora--ctreectrlsetlinecolor"></a><a name="setlinecolor"></a>CTreeCtrl::SetLineColor  
 Appelez cette fonction membre pour définir la couleur de la ligne actuelle pour le contrôle TreeView.  
...
##  <a name="a-namesetscrolltimea--ctreectrlsetscrolltime"></a><a name="setscrolltime"></a>CTreeCtrl::SetScrollTime  
 Appelez cette fonction membre pour définir le délai maximal de défilement pour le contrôle TreeView.  
...
### <a name="parameters"></a>Paramètres  
 *uScrollTime*  
 Nouvelle heure de défilement maximal, en millisecondes. Si cette valeur est inférieure à 100, il sera arrondi à 100.  
  
### <a name="return-value"></a>Valeur de retour  
 Le défilement maximal antérieur, en millisecondes.  
...
### <a name="parameters"></a>Paramètres  
 `clr`  
 A **COLORREF** valeur qui contient la nouvelle couleur de texte. Si cet argument est -1, le contrôle revient à l’aide de la couleur système pour la couleur du texte.  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTreeCtrl n °&36;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_44.cpp)]  
...
### <a name="remarks"></a>Notes  
 Pour utiliser des info-bulles, indiquer la **TVS_NOTOOLTIPS** de style lorsque vous créez le `CTreeCtrl` objet.  
...
|Paramètre|Description|  
|---------------|-----------------|  
|[in] `hItem`|Un handle à un élément d’arborescence dans le contrôle. Pour plus d’informations, consultez la `hItem` membre de la [TVITEMEX](http://msdn.microsoft.com/library/windows/desktop/bb773459) structure.|  
  
### <a name="remarks"></a>Remarques  
 Pour plus d’informations sur la différence entre les info-bulles et les info-bulles, recherchez la rubrique « Info-bulles et info-bulles » à [Microsoft Developer Network](http://go.microsoft.com/fwlink/linkid=56322).  
  
 Cette méthode envoie le [TVM_SHOWINFOTIP](http://msdn.microsoft.com/library/windows/desktop/bb773779) message, qui est décrit dans la [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Remarques  
 `SortChildren`répète pas dans l’arborescence ; uniquement les enfants immédiats du `hItem` seront triées.  
...
### <a name="remarks"></a>Notes  
 ...
 Le `lParam1` et `lParam2` paramètres correspondent au **lParam** membre de la [structure TVITEM](http://msdn.microsoft.com/library/windows/desktop/bb773456) structure pour les deux éléments comparés. Le `lParamSort` paramètre correspond à la **lParam** membre de la `TV_SORTCB` structure.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTreeCtrl&#38;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_46.cpp)]  
  
 [!code-cpp[NVC_MFC_CTreeCtrl n °&39;](../../mfc/reference/codesnippet/cpp/ctreectrl-class_47.cpp)]  
