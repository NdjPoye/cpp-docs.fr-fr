...
Est utilisé uniquement dans une `ON_UPDATE_COMMAND_UI` gestionnaire dans un `CCmdTarget`-classe dérivée.  
...
## <a name="remarks"></a>Notes  
 ...
 Lorsque le menu est extrait vers le bas, l’infrastructure recherche et appelle chaque `ON_UPDATE_COMMAND_UI` gestionnaire, chaque gestionnaire appelle `CCmdUI` telles que les fonctions membres **activer** et **vérifier**, et l’infrastructure appropriée affiche ensuite chaque élément de menu.  
  
 Un élément de menu peut être remplacé par un bouton de barre de contrôle ou un autre objet d’interface utilisateur de commande sans modifier le code dans la `ON_UPDATE_COMMAND_UI` gestionnaire.  
...
|--------------------------|------------|--------------|--------------|-------------|  
|Menu Item|Active ou désactive|(×) Ou décoche|Vérifications à l’aide de point (•)|Texte d’élément de jeux|  
...
|Bouton normal`CDialogBar`|Active ou désactive|Active ou désactive la case à cocher|Identique à`SetCheck`|Texte du bouton de jeux|  
|Contrôle normal dans`CDialogBar`|Active ou désactive|(Non applicable)|(Non applicable)|Définit le texte de la fenêtre|  
  
 Pour plus d’informations sur l’utilisation de cette classe, consultez la page [la mise à jour des objets Interface utilisateur](../../mfc/how-to-update-user-interface-objects.md).  
...
### <a name="remarks"></a>Remarques  
 Il s’agit d’une fonction membre avancée qui doit être utilisée conjointement avec un `ON_COMMAND_EX` gestionnaire retourne **FALSE**. Pour plus d’informations, consultez [technique Remarque 6](../../mfc/tn006-message-maps.md).  
...
##  <a name="m_pother"></a>CCmdUI::m_pOther  
 Pointeur (de type `CWnd`) à l’objet de fenêtre, comme une barre d’outil ou d’état, qui envoyé la notification.  
...
### <a name="parameters"></a>Paramètres  
 `nCheck`  
 Spécifie l’état du contrôle à définir. Si 0, désactive ; Si 1, recherche ; et si vous entrez 2, définit indéterminé.  
...
### <a name="remarks"></a>Remarques  
 Cette fonction membre fonctionne comme `SetCheck`, à ceci près qu’il fonctionne sur les éléments d’interface utilisateur agissant en tant que partie d’un groupe de cases d’option. Désélectionnez les autres éléments dans le groupe n’est pas automatique, sauf si les éléments proprement dits conserver le comportement du groupe de cases d’option.  
