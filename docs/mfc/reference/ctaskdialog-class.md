...
|[CTaskDialog::GetCommonButtonFlag](#getcommonbuttonflag)|Convertit un standard associé à un bouton Windows vers le type de bouton commun la `CTaskDialog` classe.|  
|[CTaskDialog::GetCommonButtonId](#getcommonbuttonid)|Convertit un des types de bouton courantes liées à la `CTaskDialog` classe pour un bouton Windows standard.|  
...
|[CTaskDialog::IsCommandControlEnabled](#iscommandcontrolenabled)|Détermine si un bouton de commande ou un bouton commune est activée.|  
...
|[CTaskDialog::OnCommandControlClick](#oncommandcontrolclick)|L’infrastructure appelle cette méthode lorsque l’utilisateur clique sur un bouton de commande.|  
|[CTaskDialog::OnCreate](#oncreate)|Le framework appelle cette méthode après avoir créé le `CTaskDialog`.|  
|[CTaskDialog::OnDestroy](#ondestroy)|Le framework appelle cette méthode juste avant qu’il détruit la `CTaskDialog`.|  
...
|[CTaskDialog::OnInit](#oninit)|Le framework appelle cette méthode lorsque le `CTaskDialog` est initialisé.|  
|[CTaskDialog::OnNavigatePage](#onnavigatepage)|Le framework appelle cette méthode lorsque l’utilisateur déplace le focus en ce qui concerne les contrôles le `CTaskDialog`.|  
...
|[CTaskDialog::OnTimer](#ontimer)|L’infrastructure appelle cette méthode lorsque la temporisation expire.|  
...
|[CTaskDialog::RemoveAllRadioButtons](#removeallradiobuttons)|Supprime toutes les cases de la `CTaskDialog`.|  
...
|[CTaskDialog::SetCommonButtonOptions](#setcommonbuttonoptions)|Met à jour un sous-ensemble des boutons courants pour être activé et exigent une élévation UAC.|  
|[CTaskDialog::SetCommonButtons](#setcommonbuttons)|Ajoute les boutons courants à le `CTaskDialog`.|  
...
|[CTaskDialog::SetExpansionArea](#setexpansionarea)|Met à jour la zone d’expansion de le `CTaskDialog`.|  
|[CTaskDialog::SetFooterIcon](#setfootericon)|Met à jour l’icône de pied de page pour le `CTaskDialog`.|  
|[CTaskDialog::SetFooterText](#setfootertext)|Met à jour le texte de pied de page de la `CTaskDialog`.|  
...
|[CTaskDialog::SetProgressBarState](#setprogressbarstate)|Définit l’état de la barre de progression et l’affiche dans le `CTaskDialog`.|  
|[CTaskDialog::SetRadioButtonOptions](#setradiobuttonoptions)|Active ou désactive la case d’option.|  
|[CTaskDialog::SetVerificationCheckbox](#setverificationcheckbox)|Définit l’état d’activation de la case à cocher de vérification.|  
...
|[CTaskDialog::TaskDialogCallback](#taskdialogcallback)|Le framework appelle ceci en réponse à différents messages Windows.|  
...
|`m_bVerified`|`TRUE`Indique la case à cocher de vérification est activée ; `FALSE` indique qu’il n’est pas.|  
|`m_footerIcon`|L’icône dans le pied de page de la `CTaskDialog`.|  
|`m_hWnd`|Un handle vers la fenêtre de la `CTaskDialog`.|  
...
|`m_nCommonButton`|Masque qui indique les boutons courants sont affichés sur le `CTaskDialog`.|  
|`m_nDefaultCommandControl`|Contrôle de l’ID du bouton de commande qui est sélectionné lorsque le `CTaskDialog` s’affiche.|  
|`m_nDefaultRadioButton`|Contrôle de l’ID de la case d’option qui est sélectionné lorsque le `CTaskDialog` s’affiche.|  
...
|`m_nRadioId`|L’ID du contrôle de bouton radio sélectionnée.|  
...
|`m_strInformation`|Les informations de développé pour le `CTaskDialog`.|  
...
## <a name="remarks"></a>Remarques  
 La `CTaskDialog` classe remplace la boîte de message Windows standard et comporte des fonctionnalités supplémentaires telles que de nouveaux contrôles pour rassembler les informations de l’utilisateur. Cette classe est dans la bibliothèque MFC [!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)]. Le `CTaskDialog` est disponible avec [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]. Les versions antérieures de Windows ne peut pas afficher le `CTaskDialog` objet. Utilisez `CTaskDialog::IsSupported` pour déterminer, lors de l’exécution, si l’utilisateur actuel peut afficher la boîte de dialogue. La boîte de message Windows standard est toujours pris en charge dans [!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)].  
  
 Le `CTaskDialog` est disponible uniquement lorsque vous générez votre application à l’aide de la bibliothèque Unicode.  
  
 Le `CTaskDialog` a deux constructeurs différents. Un constructeur vous permet de spécifier les deux boutons de commande et un maximum de six contrôles bouton normal. Vous pouvez ajouter plusieurs boutons de commande après avoir créé le `CTaskDialog`. Le deuxième constructeur ne prend pas en charge les boutons de commande, mais vous pouvez ajouter un nombre illimité de contrôles de bouton normal. Pour plus d’informations sur les constructeurs, consultez [CTaskDialog::CTaskDialog](#ctaskdialog).  
  
 L’illustration suivante montre un exemple `CTaskDialog` pour montrer l’emplacement de certains contrôles.  
  
 ![Exemple de CTaskDialog](../../mfc/reference/media/ctaskdialogsample.png "ctaskdialogsample")  
Exemple de CTaskDialog  
  
## <a name="requirements"></a>Spécifications  
 **Système d’exploitation au minimum :**[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]  
...
### <a name="parameters"></a>Paramètres  
 ...
 [in] `strCaption`  
 La chaîne que le `CTaskDialog` affiche à l’utilisateur. Utiliser cette chaîne pour expliquer l’objectif de la commande.  
...
### <a name="remarks"></a>Remarques  
 ...
 Lorsque l’utilisateur sélectionne un contrôle de bouton de commande, le `CTaskDialog` se ferme. Si votre application affiche la boîte de dialogue à l’aide de [CTaskDialog::DoModal](#domodal), `DoModal` renvoie le `nCommandControlID` du contrôle de bouton de commande sélectionnée.  
...
### <a name="parameters"></a>Paramètres  
 ...
 [in] `bEnabled`  
 Un paramètre booléen qui indique si la case est activée.  
  
### <a name="remarks"></a>Notes  
 Boutons de la radio pour le [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) vous permettent de rassembler des informations de l’utilisateur. Utilisez la fonction [CTaskDialog::GetSelectedRadioButtonID](#getselectedradiobuttonid) pour déterminer quelle case d’option est sélectionnée.  
  
 Le `CTaskDialog` ne nécessite pas que le `nRadioButtonID` paramètres sont uniques pour chaque case d’option. Toutefois, vous pouvez rencontrer un comportement inattendu si vous n’utilisez pas un identificateur distinct pour chaque case d’option.  
...
### <a name="parameters"></a>Paramètres  
 [in] `nCommandControlID`  
 L’ID de commande du contrôle de cliquer.  
...
### <a name="parameters"></a>Paramètres  
 ...
 [in] `nCommonButtons`  
 Un masque des boutons courants à ajouter à la `CTaskDialog`.  
  
 [in] `nTaskDialogOptions`  
 Le jeu d’options à utiliser pour le `CTaskDialog`.  
  
 [in] `strFooter`  
 Chaîne à utiliser en tant que pied de page.  
...
### <a name="remarks"></a>Notes  
 Il existe deux méthodes que vous pouvez ajouter un `CTaskDialog` à votre application. La première consiste à utiliser un des constructeurs pour créer un `CTaskDialog` et l’afficher à l’aide de [CTaskDialog::DoModal](#domodal). La deuxième consiste à utiliser la fonction statique [CTaskDialog::ShowDialog](#showdialog), ce qui vous permet d’afficher un `CTaskDialog` sans créer explicitement un `CTaskDialog` objet.  
  
 Le deuxième constructeur crée des contrôles de bouton de commande à l’aide des données du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Cette méthode ajoute un contrôle de bouton de commande pour chaque entrée valide dans la table de chaînes entre `nIDCommandControlsFirst` et `nCommandControlsLast`, inclus. Pour ces contrôles de bouton de commande, la chaîne de la table de chaînes est la légende du contrôle et l’ID de chaîne est l’ID. du contrôle  
  
 Consultez la page [CTaskDialog::SetOptions](#setoptions) pour obtenir la liste des options valides.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTaskDialog&#7;](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_3.cpp)]  
  
##  <a name="domodal"></a>CTaskDialog::DoModal  
 Affiche la `CTaskDialog` et rend modal.  
...
### <a name="remarks"></a>Notes  
 Affiche cette instance de la [CTaskDialog](../../mfc/reference/ctaskdialog-class.md). L’application attend que l’utilisateur de fermer la boîte de dialogue.  
...
### <a name="remarks"></a>Remarques  
 Les boutons courants sont les boutons par défaut que vous fournissez à [CTaskDialog::CTaskDialog](#ctaskdialog). Le [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) affiche les boutons situés en bas de la boîte de dialogue.  
  
 La liste énumérée des boutons est fournie dans CommCtrl.h.  
  
##  <a name="getcommonbuttonflag"></a>CTaskDialog::GetCommonButtonFlag  
 Convertit un standard associé à un bouton Windows vers le type de bouton commun le [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md).  
...
### <a name="remarks"></a>Remarques  
 ...
 Si `nCommandControlID` n'est pas un identificateur valide pour une commune `CTaskDialog` bouton ou un bouton de commande, cette méthode lève une exception.  
...
### <a name="return-value"></a>Valeur de retour  
 `TRUE`Si l’ordinateur prend en charge les `CTaskDialog`; `FALSE` dans le cas contraire.  
  
### <a name="remarks"></a>Remarques  
 Cette fonction permet de déterminer à l’exécution si l’ordinateur qui exécute votre application prend en charge la `CTaskDialog Class`. Si l’ordinateur ne prend pas en charge les `CTaskDialog`, vous devez fournir une autre méthode pour communiquer des informations à l’utilisateur. Votre application se bloquera si elle tente d’utiliser un `CTaskDialog` sur un ordinateur qui ne prend pas en charge la `CTaskDialog` classe.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode crée des contrôles de bouton de commande à l’aide des données du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Nouveaux contrôles de bouton de commande ajoutés à l’aide de cette méthode utilisent la chaîne pour la légende du contrôle et l’ID de chaîne pour les ID d' un contrôle La plage de chaînes sélectionné est fournie par `nIDCommandControlsFirst` et `nCommandControlsLast`, inclus. S’il existe une entrée vide dans la plage, la méthode n’ajoute pas le contrôle de bouton de commande pour cette entrée.  
...
##  <a name="loadradiobuttons"></a>CTaskDialog::LoadRadioButtons  
 Ajoute des contrôles de bouton radio à l’aide de données à partir de la table de chaînes.  
...
### <a name="remarks"></a>Notes  
 Cette méthode crée des cases d’option à l’aide des données du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Nouveaux boutons radio ajoutés à l’aide de cette méthode utilisent la chaîne pour la légende du bouton de la case d’option et l’ID de chaîne pour ID de la case d’option La plage de chaînes sélectionné est fournie par `nIDRadioButtonsFirst` et `nRadioButtonsLast`, inclus. S’il existe une entrée vide dans la plage, la méthode n’ajoute pas d’un bouton radio pour cette entrée.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode masque actuel `CTaskDialog` lorsqu’il affiche la `oTaskDialog`. Le `oTaskDialog` s’affiche dans le même emplacement que l’actuel `CTaskDialog`.  
  
##  <a name="oncommandcontrolclick"></a>CTaskDialog::OnCommandControlClick  
 L’infrastructure appelle cette méthode lorsque l’utilisateur clique sur un bouton de commande.  
...
##  <a name="oncreate"></a>CTaskDialog::OnCreate  
 Le framework appelle cette méthode après avoir créé le `CTaskDialog`.  
...
##  <a name="ondestroy"></a>CTaskDialog::OnDestroy  
 Le framework appelle cette méthode juste avant qu’il détruit la `CTaskDialog`.  
...
##  <a name="oninit"></a>CTaskDialog::OnInit  
 Le framework appelle cette méthode lorsque le `CTaskDialog` est initialisé.  
...
##  <a name="onnavigatepage"></a>CTaskDialog::OnNavigatePage  
 Le framework appelle cette méthode en réponse à la [CTaskDialog::NavigateTo](#navigateto) (méthode).  
...
##  <a name="ontimer"></a>CTaskDialog::OnTimer  
 L’infrastructure appelle cette méthode lorsque la temporisation expire.  
...
### <a name="parameters"></a>Paramètres  
 [in] `bChecked`  
 `TRUE`Indique que la case à cocher de vérification est sélectionnée ; `FALSE` indique qu’il n’est pas.  
...
##  <a name="removeallradiobuttons"></a>CTaskDialog::RemoveAllRadioButtons  
 Supprime toutes les cases de la `CTaskDialog`.  
...
### <a name="remarks"></a>Notes  
 Utilisez cette méthode pour modifier un contrôle de bouton de commande est activé ou nécessite une élévation après qu’il a été ajouté à la `CTaskDialog Class`.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTaskDialog n °&2;](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_1.cpp)]  
  
##  <a name="setcommonbuttonoptions"></a>CTaskDialog::SetCommonButtonOptions  
 Met à jour un sous-ensemble des boutons courants pour être activé et exigent une élévation UAC.  
...
### <a name="parameters"></a>Paramètres  
 ...
 Un masque pour les boutons courants pour la désactiver.  
...
### <a name="remarks"></a>Remarques  
 ...
 Si vous utilisez cette méthode pour désactiver ou élever un bouton commun qui n’est pas disponible pour ce `CTaskDialog`, cette méthode lève une exception à l’aide de la [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro.  
  
 Cette méthode permet à n’importe quel bouton est disponible pour le `CTaskDialog` mais n’est pas dans le `nDisabledButtonMask`, même si elle a été désactivée. Cette méthode traite l’élévation d’une manière similaire : il enregistre les boutons courants comme ne nécessitant ne pas d’élévation si le bouton commun est disponible mais n’est pas inclus dans `nElevationButtonMask`.  
...
##  <a name="setcommonbuttons"></a>CTaskDialog::SetCommonButtons  
 Ajoute les boutons courants à le `CTaskDialog`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 [in] `nDisabledButtonMask`  
 Un masque des boutons pour les désactiver.  
...
### <a name="remarks"></a>Notes  
 ...
 Les boutons indiqué par `nButtonMask` remplacer les boutons courants précédemment ajoutés à la `CTaskDialog`. Seuls les boutons figurant `nButtonMask` sont disponibles.  
  
 Si le paramètre `nDisabledButtonMask` ou `nElevationButtonMask` contient un bouton qui n’est pas `nButtonMask`, cette méthode lève une exception à l’aide de la [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro.  
...
### <a name="parameters"></a>Paramètres  
 [in] `nCommandControlID`  
 L’ID du contrôle de bouton de commande par défaut.  
  
### <a name="remarks"></a>Remarques  
 Le contrôle de bouton de commande par défaut est le contrôle qui est sélectionné lorsque le `CTaskDialog` est affiché en premier à l’utilisateur.  
...
### <a name="remarks"></a>Remarques  
 La case d’option par défaut est le bouton est sélectionné lorsque le `CTaskDialog` est affiché en premier à l’utilisateur.  
...
##  <a name="setexpansionarea"></a>CTaskDialog::SetExpansionArea  
 Met à jour la zone d’expansion de le `CTaskDialog`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 La chaîne que le `CTaskDialog` s’affiche dans le corps principal de la boîte de dialogue lorsque l’utilisateur clique sur le bouton de développement.  
  
 [in] `strCollapsedLabel`  
 La chaîne que le `CTaskDialog` affiche en regard du bouton d’extension lorsque la zone étendue est réduite.  
  
 [in] `strExpandedLabel`  
 La chaîne que le `CTaskDialog` affiche en regard du bouton d’extension lorsque la zone de développé s’affiche.  
  
### <a name="remarks"></a>Remarques  
 La zone d’extension de la `CTaskDialog Class` vous permet de fournir des informations supplémentaires à l’utilisateur. La zone d’extension est dans la partie principale de la `CTaskDialog`, qui se trouve immédiatement en dessous de la chaîne de titre et contenu.  
  
 Lors de la `CTaskDialog` est d’abord affichée, elle n’affiche pas les informations étendues et place `strCollapsedLabel` en regard du bouton d’extension. Lorsque l’utilisateur clique sur le bouton de développement, le `CTaskDialog` affiche `strExpandedInformation` et modifie l’étiquette à `strExpandedLabel`.  
...
### <a name="remarks"></a>Remarques  
 L’icône de pied de page s’affiche en bas de la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md). Il peut avoir associé texte de pied de page. Vous pouvez modifier le texte de pied de page avec [CTaskDialog::SetFooterText](#setfootertext).  
  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si la `CTaskDialog` est affiché ou le paramètre d’entrée est `NULL`.  
  
 A `CTaskDialog` accepte uniquement un `HICON` ou `LPCWSTR` comme une icône de pied de page. Il est configuré en définissant l’option `TDF_USE_HICON_FOOTER` dans le constructeur ou [CTaskDialog::SetOptions](#setoptions). Par défaut, le `CTaskDialog` est configuré pour utiliser `LPCWSTR` comme type d’entrée pour l’icône de pied de page. Cette méthode génère une exception si vous essayez de définir l’icône de l’aide du type inapproprié.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTaskDialog&#7;](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_3.cpp)]  
  
##  <a name="setfootertext"></a>CTaskDialog::SetFooterText  
 Met à jour le texte de pied de page de la `CTaskDialog`.  
...
### <a name="remarks"></a>Remarques  
 L’icône de pied de page s’affiche en regard du texte de pied de page en bas de la `CTaskDialog`. Vous pouvez modifier l’icône de pied de page avec [CTaskDialog::SetFooterIcon](#setfootericon).  
...
### <a name="remarks"></a>Notes  
 L’instruction principale de la `CTaskDialog Class` est le texte affiché à l’utilisateur dans une grande taille de police en gras. Il se trouve dans la boîte de dialogue sous la barre de titre.  
...
### <a name="remarks"></a>Notes  
 Cette méthode efface toutes les options actuelles de la `CTaskDialog`. Pour conserver les options actuelles, vous devez les extraire tout d’abord avec [CTaskDialog::GetOptions](#getoptions) et de les combiner avec les options que vous souhaitez définir.  
 ...
 Permet à l’utilisateur de fermer la `CTaskDialog` à l’aide du clavier ou à l’aide de l’icône dans le coin supérieur droit de la boîte de dialogue, même si le **Annuler** bouton n’est pas activé. Si cet indicateur n’est pas défini et la **Annuler** bouton n’est pas activé, l’utilisateur ne peut pas fermer la boîte de dialogue à l’aide de Alt + F4, la touche ÉCHAP, ou bouton de fermeture de la barre de titre.  
 ...
 Indique que la case à cocher de vérification est sélectionnée.  
 ...
 Permet à l’utilisateur afin de réduire le `CTaskDialog`. Pour prendre en charge cette option, le `CTaskDialog` ne peut pas être modale. MFC ne prend pas en charge cette option, car les MFC ne prend pas en charge un non modal `CTaskDialog`.  
...
### <a name="parameters"></a>Paramètres  
 [in] `bEnabled`  
 `TRUE`Pour activer la barre de sélection ; `FALSE` pour désactiver la barre de sélection et le supprimer à partir de la `CTaskDialog`.  
  
 [in] `nMarqueeSpeed`  
 Entier qui indique la vitesse de la barre de sélection.  
  
### <a name="remarks"></a>Notes  
 La barre de sélection apparaît en dessous du texte principal de la `CTaskDialog Class`.  
  
 Utilisez `nMarqueeSpeed` pour définir la vitesse de la barre de sélection ; valeurs élevées indiquent une vitesse inférieure. La valeur 0 pour `nMarqueeSpeed` déplace la barre de sélection à la vitesse par défaut [!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)].  
  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si `nMarqueeSpeed` est inférieur à 0.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si `nProgressPos` n’est pas dans la plage de barre de progression. Vous pouvez modifier l’intervalle de barre de progression avec [CTaskDialog::SetProgressBarRange](#setprogressbarrange).  
...
### <a name="remarks"></a>Remarques  
 La position de la barre de progression est relatif à `nRangeMin` et `nRangeMax`. Par exemple, si `nRangeMin` est 50 et `nRangeMax` est égal à 100, une position de 75 se trouve à mi-chemin entre la barre de progression. Utilisez [CTaskDialog::SetProgressBarPosition](#setprogressbarposition) pour définir la position de la barre de progression.  
 ...
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si `nRangeMin` est au moins `nRangeMax`. Cette méthode lève également une exception si le `CTaskDialog` est déjà affichée et a une barre de progression de type texte défilant.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTaskDialog n °&4;](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_7.cpp)]  
  
##  <a name="setprogressbarstate"></a>CTaskDialog::SetProgressBarState  
 Définit l’état de la barre de progression et l’affiche dans le `CTaskDialog`.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si la `CTaskDialog` est déjà affichée et a une barre de progression de type texte défilant.  
  
 Le tableau suivant répertorie les valeurs possibles pour `nState`. Dans tous ces cas, la barre de progression est renseigné avec la couleur régulière jusqu'à atteindre la position d’arrêt désigné. À ce stade il change de couleur en fonction de l’état.  
  
 PBST_NORMAL  
 Après la progression de la barre soit remplie, le `CTaskDialog` ne modifie pas la couleur de la barre. Par défaut, la couleur standard est verte.  
  
 PBST_ERROR  
 Après la progression de la barre soit remplie, le `CTaskDialog` modifie la couleur de la barre de couleur de l’erreur. Par défaut, il est rouge.  
  
 PBST_PAUSED  
 Après la progression de la barre soit remplie, le `CTaskDialog` modifie la couleur de la barre de la couleur en pause. Par défaut, il s’agit jaune.  
...
### <a name="remarks"></a>Notes  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si `nRadioButtonID` n’est pas un ID valide pour un bouton radio.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CTaskDialog n °&3;](../../mfc/reference/codesnippet/cpp/ctaskdialog-class_2.cpp)]  
  
##  <a name="setverificationcheckbox"></a>CTaskDialog::SetVerificationCheckbox  
 Définit l’état d’activation de la case à cocher de vérification.  
...
### <a name="parameters"></a>Paramètres  
 [in] `bChecked`  
 `TRUE`Pour que la vérification de la case à cocher activée lorsque la `CTaskDialog` est affichée ; `FALSE` pour que la vérification de la case à cocher désactivée lorsque le `CTaskDialog` s’affiche.  
...
### <a name="remarks"></a>Remarques  
 Cette méthode lève une exception avec le [Vérifiez](http://msdn.microsoft.com/library/738c4ccf-c29c-4c04-8d6c-f126bedf6e91) macro si cette instance de la `CTaskDialog Class` est déjà affichée.  
...
### <a name="parameters"></a>Paramètres  
 [in] `strWindowTitle`  
 Le nouveau titre du `CTaskDialog`.  
...
##  <a name="taskdialogcallback"></a>CTaskDialog::TaskDialogCallback  
 Le framework appelle cette méthode en réponse à différents messages Windows.  
...
### <a name="remarks"></a>Remarques  
 ...
 Les valeurs de `wParam` et `lParam` dépendent du message généré spécifique. Il est possible pour les deux de ces valeurs est vide. Le tableau suivant répertorie les notifications par défaut qui sont prises en charge et que les valeurs de `wParam` et `lParam` représentent. Si vous substituez cette méthode dans une classe dérivée, vous devez implémenter le code de rappel pour chaque message dans le tableau suivant.  
...
|`TDN_EXPANDO_BUTTON_CLICKED`|0 si la zone d’expansion est réduite ; différent de zéro si le texte est affiché.|Non utilisé.|  
