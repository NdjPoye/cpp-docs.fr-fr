...
## <a name="members"></a>Membres  
 Les fonctions membres de `CMapStringToPtr` sont similaires aux fonctions membres de classe [CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md). Ainsi, vous pouvez utiliser la documentation de référence de `CMapStringToOb` pour connaître les spécificités des fonctions membres. Chaque fois que vous voyez un `CObject` pointeur comme un paramètre de fonction ou de la valeur de retour, remplacez par un pointeur vers `void`.  
...
## <a name="remarks"></a>Remarques  
 ...
 Les mappages de pointeur de chaîne ne peuvent pas être sérialisés.  
  
 Lorsqu’un `CMapStringToPtr` objet est supprimé, ou lorsque ses éléments sont supprimés, le `CString` les objets de clé et les mots sont supprimés.  
