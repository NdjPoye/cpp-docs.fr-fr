...
|[CImageList::Attach](#attach)|Associe une liste d’images à un `CImageList` objet.|  
...
|[CImageList::Create](#create)|Initialise une liste d’images et l’attache à une `CImageList` objet.|  
|[CImageList::DeleteImageList](#deleteimagelist)|Supprime une liste d’images.|  
|[CImageList::DeleteTempMap](#deletetempmap)|Appelée par le [CWinApp](../../mfc/reference/cwinapp-class.md) Gestionnaire de durée d’inactivité pour supprimer un fichier temporaire `CImageList` objet créé par `FromHandle`.|  
|[CImageList::Detach](#detach)|Détache un objet liste d’images d’un `CImageList` de l’objet et retourne un handle vers une liste d’images.|  
|[CImageList::DragEnter](#dragenter)|Verrouille les mises à jour pendant une opération glisser et affiche l’image à la position spécifiée.|  
|[CImageList::DragLeave](#dragleave)|Déverrouille la fenêtre et masque l’image afin que la fenêtre peut être mis à jour.|  
|[CImageList::DragMove](#dragmove)|Déplace l’image qui est glissé pendant une opération de glisser-déplacer.|  
|[CImageList::DragShowNolock](#dragshownolock)|Affiche ou masque l’image pendant une opération glisser, sans verrouillage de la fenêtre.|  
|[CImageList::Draw](#draw)|Dessine l’image qui est glissé pendant une opération de glisser-déplacer.|  
|[CImageList::DrawEx](#drawex)|Dessine un élément de liste d’image dans le contexte de périphérique spécifié. La fonction utilise le style de dessin spécifié, fusionne l’image avec la couleur spécifiée.|  
|[CImageList::DrawIndirect](#drawindirect)|Dessine une image à partir d’une liste d’images.|  
|[CImageList::EndDrag](#enddrag)|Termine une opération de glisser-déplacer.|  
...
|[CImageList::GetSafeHandle](#getsafehandle)|Récupère les **m_hImageList**.|  
...
|[CImageList::Replace](#replace)|Remplace une image d’une liste d’images avec une nouvelle image.|  
...
## <a name="remarks"></a>Notes  
 Une « liste d’images » est une collection d’images de même taille, chacun d'entre eux peut être désigné par son index de base zéro. Listes d’images sont utilisées pour gérer efficacement de grands ensembles d’icônes ou de bitmaps. Toutes les images dans une liste d’images sont contenues dans une seule grande bitmap au format de périphérique d’écran. Une liste d’images peut également inclure une bitmap monochrome contenant des masques utilisés pour dessiner des images en toute transparence (style icône). L’interface API de programmation d’applications Microsoft Win32 fournit des fonctions de liste image qui vous permettent de dessiner des images, créer et détruire des listes d’images, ajouter et supprimer des images, remplacer, fusionner des images et faire glisser des images.  
...
### <a name="parameters"></a>Paramètres  
 `pbmImage`  
 Pointeur vers l’image bitmap contenant l’ou les images. Le nombre d’images est déduit de la largeur de la bitmap.  
 ...
 `crMask`  
 Couleur utilisée pour générer le masque. Chaque pixel de cette couleur dans l’image bitmap donnée devient noir et le bit correspondant dans le masque est défini à un.  
...
### <a name="parameters"></a>Paramètres  
 `hImageList`  
 Handle vers un objet liste d’images.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `ptHotSpot`  
 Coordonnées de départ glisser (en général, la position du curseur). Les coordonnées sont exprimées par rapport à l’angle supérieur gauche de l’image.  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Remarques  
 Cette fonction crée une liste d’images temporaire qui est utilisée pour faire glisser. L’image associe l’image spécifiée et son masque le curseur actuel. En réponse à ultérieures `WM_MOUSEMOVE` messages, vous pouvez déplacer l’image à l’aide de la `DragMove` fonction membre. Pour terminer l’opération de glissement, vous pouvez utiliser la `EndDrag` fonction membre.  
 ...
 *iDst*  
 ...
 `uFlags`  
 La valeur d’indicateur de bits qui spécifie le type d’opération de copie doit être faite. Ce paramètre peut être une des valeurs suivantes :  
...
##  <a name="create"></a>CImageList::Create  
 Initialise une liste d’images et l’attache à une [CImageList](../../mfc/reference/cimagelist-class.md) objet.  
...
|`ILC_COLOR4`|Utiliser une section de bitmap indépendante du périphérique (DIB) 4 bits (16 couleurs) en tant que la bitmap de la liste d’images.|  
|`ILC_COLOR8`|Utiliser une section DIB 8 bits. Les couleurs utilisées pour la table des couleurs sont les mêmes couleurs que la palette de demi-teintes.|  
|`ILC_COLOR16`|Utiliser une application 16 bits (32 / 64k couleur) section DIB.|  
|`ILC_COLOR24`|Utilisation d’une section DIB 24 bits.|  
|`ILC_COLOR32`|Utilisation d’une section DIB 32 bits.|  
|`ILC_COLORDDB`|Utilisez une bitmap dépendant du périphérique.|  
|`ILC_MASK`|Utilise un masque. La liste d’images contient deux images, y compris un bitmap monochrome utilisé comme un masque. Si cette valeur n’est pas incluse, la liste d’images contient uniquement un bitmap. Consultez la page [dessin d’Images à partir d’une liste d’images](../../mfc/drawing-images-from-an-image-list.md) pour plus d’informations sur les images masquées.|  
  
 `nInitial`  
 Nombre d’images contenant la liste d’images initialement.  
  
 `nGrow`  
 Nombre d’images dont la liste d’images peut augmenter lorsque le système a besoin de redimensionner la liste pour faire place aux nouvelles images. Ce paramètre représente le nombre de nouvelles images de que la liste de l’image redimensionnée peut contenir.  
  
 `nBitmapID`  
 ID de ressource de la bitmap à associer à la liste d’images.  
  
 `crMask`  
 Couleur utilisée pour générer un masque. Chaque pixel de cette couleur dans l’image bitmap spécifiée devient noir, et le bit correspondant dans le masque est défini à un.  
...
### <a name="remarks"></a>Remarques  
 Vous construisez un `CImageList` en deux étapes. Tout d’abord, appelez le constructeur, puis `Create`, qui crée la liste d’images et l’attache à le `CImageList`objet.  
...
##  <a name="deletetempmap"></a>CImageList::DeleteTempMap  
 Appelée automatiquement par le `CWinApp` Gestionnaire de durée d’inactivité, `DeleteTempMap` supprime tout fichier temporaire `CImageList` les objets créés par [FromHandle](#fromhandle), mais ne détruit ne pas tous les handles ( `hImageList`) temporairement associé à la **ImageList** objets.  
...
##  <a name="dragenter"></a>CImageList::DragEnter  
 Pendant une opération glisser, verrouille les mises à jour de la fenêtre spécifiée par `pWndLock` et affiche l’image à la position spécifiée par `point`.  
...
### <a name="parameters"></a>Paramètres  
 `pWndLock`  
 Pointeur vers la fenêtre propriétaire de l’image.  
  
 `point`  
 Position d’affichage de l’image. Coordonnées sont exprimées par rapport à l’angle supérieur gauche de la fenêtre (et non à la zone client).  
...
### <a name="remarks"></a>Notes  
 Cette fonction est généralement appelée en réponse à une `WM_MOUSEMOVE` message. Pour commencer une opération de glisser-déplacer, utilisez la `BeginDrag` fonction membre.  
...
##  <a name="dragshownolock"></a>CImageList::DragShowNolock  
 Affiche ou masque l’image pendant une opération glisser, sans verrouillage de la fenêtre.  
...
### <a name="parameters"></a>Paramètres  
 `bShow`  
 Spécifie si l’image doit être indiqué.  
...
|Valeur|Signification|  
|-----------|-------------|  
|`ILD_BLEND25`, **ILD_FOCUS**|Dessine l’image de 25 pour cent avec la couleur de surbrillance du système de fusion. Cette valeur n’a aucun effet si la liste d’images ne contient pas un masque.|  
...
|`ILD_TRANSPARENT`|Dessine l’image en toute transparence avec le masque, quelle que soit la couleur d’arrière-plan.|  
...
### <a name="parameters"></a>Paramètres  
 ...
 `sz`  
 Taille de la partie de l’image à dessiner par rapport au coin supérieur gauche de l’image. Consultez la page `dx` et *dy* dans [ImageList_DrawEx](http://msdn.microsoft.com/library/windows/desktop/bb761536) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] *.*  
  
 *clrBk*  
 Couleur d’arrière-plan de l’image. Consultez la page *rgbBk* dans [ImageList_DrawEx](http://msdn.microsoft.com/library/windows/desktop/bb761536) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] *.*  
  
 *clrFg*  
 Couleur de premier plan de l’image. Consultez la page *rgbFg* dans [ImageList_DrawEx](http://msdn.microsoft.com/library/windows/desktop/bb761536) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] *.*  
  
 `nStyle`  
 Indicateur qui spécifie le style de dessin. Consultez la page *fStyle* dans [ImageList_DrawEx](http://msdn.microsoft.com/library/windows/desktop/bb761536) dans les [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] *.*  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Remarques  
 La fonction utilise le style de dessin spécifié, fusionne l’image avec la couleur spécifiée.  
 ...
 *pimldp*  
 ...
 `pt`  
 A [POINT](http://msdn.microsoft.com/library/windows/desktop/dd162805) structure contenant les – coordonnées x et y, où l’image sera dessiné.  
  
 `sz`  
 A [taille](http://msdn.microsoft.com/library/windows/desktop/dd145106) structure indiquant la taille de l’image à dessiner.  
  
 *ptOrigin*  
 A [POINT](http://msdn.microsoft.com/library/windows/desktop/dd162805) structure contenant les – coordonnées x et y, en spécifiant l’angle supérieur gauche de l’opération de dessin par rapport à l’image elle-même. Les pixels de l’image à gauche de la coordonnée x – et au-dessus de la coordonnée y – ne figurent pas.  
  
 `fStyle`  
 Indicateur qui spécifie le style de dessin et, éventuellement, l’image de superposition. Consultez la section Notes pour plus d’informations sur l’image de superposition. L’implémentation par défaut MFC, `ILD_NORMAL`, dessine l’image à l’aide de la couleur d’arrière-plan pour la liste d’images. Si la couleur d’arrière-plan est le `CLR_NONE` valeur, l’image est dessinée en toute transparence à l’aide d’un masque.  
 ...
 *dwRop*  
 Valeur qui spécifie un code d’opération de bitmap. Ces codes définissent comment combiner les données de couleur du rectangle source avec les données de couleur du rectangle de destination obtenir la couleur finale. Implémentation, par défaut de MFC **SRCCOPY**, copie le rectangle source directement vers le rectangle de destination. Ce paramètre est ignoré si la `fStyle` paramètre n’inclut pas le **ILD_ROP** indicateur.  
  
 Les autres valeurs possibles sont décrites dans le **dwRop** membre de la [IMAGELISTDRAWPARAMS](http://msdn.microsoft.com/library/windows/desktop/bb761395) structure.  
  
 *rgbBack*  
 La couleur d’arrière-plan d’image, par défaut `CLR_DEFAULT`. Ce paramètre peut être une valeur RVB définies par l’application ou une des valeurs suivantes :  
  
|Valeur|Signification|  
|-----------|-------------|  
|`CLR_DEFAULT`|Couleur d’arrière-plan par défaut. L’image est dessinée à l’aide de la couleur d’arrière-plan de liste image.|  
|`CLR_NONE`|Aucune couleur d’arrière-plan. L’image est dessinée en transparence.|  
  
 *rgbFore*  
 Couleur de premier plan, de l’image par défaut `CLR_DEFAULT`. Ce paramètre peut être une valeur RVB définies par l’application ou une des valeurs suivantes :  
 ...
 *fState*  
 Indicateur spécifiant l’état de dessin. Ce membre peut contenir un ou plusieurs indicateurs d’état de liste image.  
 ...
 *Frame*  
 ...
 Lorsqu’il est utilisé avec **ILS_APLHA**, ce membre contient la valeur pour le canal alpha. Cette valeur peut être comprise entre 0 et 255, 0 étant totalement transparent et 255 totalement opaque.  
  
 *crEffect*  
 A [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) valeur utilisée pour les effets de lumière et les clichés instantanés.  
...
### <a name="remarks"></a>Remarques  
 ...
 Une image de superposition est une image qui est dessinée sur l’image principale, spécifiée dans cette fonction membre à le `nImage` paramètre. Dessiner un masque de superposition à l’aide de la [dessiner](#draw) fonction membre avec l’index de base un du masque de superposition spécifié à l’aide de la [INDEXTOOVERLAYMASK](http://msdn.microsoft.com/library/windows/desktop/bb761408) macro.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CImageList&#11;](../../mfc/reference/codesnippet/cpp/cimagelist-class_10.cpp)]  
  
##  <a name="enddrag"></a>CImageList::EndDrag  
 Appelez cette fonction pour mettre fin à une opération de glisser-déplacer.  
...
### <a name="remarks"></a>Remarques  
 Pour commencer une opération de glisser-déplacer, utilisez la `BeginDrag` fonction membre.  
...
##  <a name="extracticon"></a>CImageList::ExtractIcon  
 Appelez cette fonction pour créer une icône basée sur une image et son masque dans une liste d’images associée.  
...
### <a name="remarks"></a>Remarques  
 Si un `CImageList` n’est pas déjà attaché au handle, temporaire `CImageList` objet est créé et attaché. Ce fichier temporaire `CImageList` objet est valide uniquement jusqu'à ce que la prochaine fois que l’application dispose de temps d’inactivité dans sa boucle d’événements, auquel cas tous les objets temporaires sont supprimés.  
 ...
 *pImageInfo*  
 Pointeur vers un [IMAGEINFO](http://msdn.microsoft.com/library/windows/desktop/bb761393) structure qui reçoit des informations sur l’image. Les informations contenues dans cette structure peuvent servir à manipuler directement les bitmaps de l’image.  
...
##  <a name="getsafehandle"></a>CImageList::GetSafeHandle  
 Appelez cette fonction pour récupérer la **m_hImageList** membre de données.  
...
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CImageList n °&23;](../../mfc/reference/codesnippet/cpp/cimagelist-class_16.cpp)]  
  
##  <a name="operator_himagelist"></a>CImageList::operator HIMAGELIST  
 Utilisez cet opérateur pour obtenir le handle de joint le `CImageList` objet.  
...
### <a name="return-value"></a>Valeur de retour  
 Si réussie, un handle vers la liste d’images représenté par le `CImageList` objet ; sinon **NULL**.  
...
### <a name="remarks"></a>Remarques  
 Tous les éléments qui suivent `nImage` maintenant déplacer vers le bas une position. Par exemple, si une liste d’images contient deux éléments, la suppression du premier élément provoquera l’élément restant à présent en première position. `nImage`=&0; pour l’élément dans la première position.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CImageList n °&19;](../../mfc/reference/codesnippet/cpp/cimagelist-class_19.cpp)]  
...
### <a name="parameters"></a>Paramètres  
 ...
 `hIcon`  
 Un handle de l’icône qui contient l’image bitmap et un masque pour la nouvelle image.  
  
### <a name="return-value"></a>Valeur de retour  
 La version retour **BOOL** retourne zéro si réussit ; sinon, 0.  
  
 La version retour `int` retourne l’index de base zéro de l’image si réussi ; sinon-1.  
  
### <a name="remarks"></a>Remarques  
 Appelez cette fonction membre après avoir appelé [fonction SetImageCount](#setimagecount) pour affecter le nouveau, des images valides pour l’espace réservé d’image numéros d’index.  
...
### <a name="parameters"></a>Paramètres  
 `cr`  
 Couleur d’arrière-plan à définir. Il peut être `CLR_NONE`. Dans ce cas, les images sont dessinés en toute transparence à l’aide du masque.  
...
##  <a name="setdragcursorimage"></a>CImageList::SetDragCursorImage  
 Crée une nouvelle image glisser en combinant l’image donnée (généralement une image de curseur de souris) avec l’image glisser en cours.  
...
### <a name="parameters"></a>Paramètres  
 *nDrag*  
 Index de la nouvelle image pour pouvoir être associé à l’image.  
...
### <a name="remarks"></a>Remarques  
 Étant donné que les fonctions de déplacement utilisent la nouvelle image pendant une opération glisser, vous devez utiliser les fenêtres [ShowCursor](http://msdn.microsoft.com/library/windows/desktop/ms648396) fonction permettant de masquer le curseur de la souris après avoir appelé `CImageList::SetDragCursorImage`. Sinon, le système peut sembler être composé de deux curseurs de souris pour la durée de l'opération Glisser-déplacer.  
...
### <a name="remarks"></a>Remarques  
 Si vous appelez cette fonction membre pour augmenter le nombre d’images de la liste, puis appelez [remplacer](#replace) pour chaque image supplémentaire à affecter les nouveaux index pour les images valides. Si vous ne parvenez pas à attribuer les index à images valides, les opérations de dessin qui créent les nouvelles images est imprévisibles.  
  
 Si vous réduisez la taille d’une liste d’images à l’aide de cette fonction, les images tronqués sont libérées.  
  
### <a name="example"></a>Exemple  
 [!code-cpp[NVC_MFC_CImageList n °&21;](../../mfc/reference/codesnippet/cpp/cimagelist-class_21.cpp)]  
