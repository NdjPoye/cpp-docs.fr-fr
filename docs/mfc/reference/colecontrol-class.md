...
|Nom|Description|  
|----------|-----------------|  
|[COleControl::AmbientAppearance](#ambientappearance)|Récupère l’apparence du contrôle actuel.|  
...
|[COleControl::AmbientUIDead](#ambientuidead)|Détermine si le contrôle doit répondre à des actions d’interface utilisateur.|  
|[COleControl::AmbientUserMode](#ambientusermode)|Détermine le mode du conteneur.|  
|[COleControl::BoundPropertyChanged](#boundpropertychanged)|Notifie au conteneur qu’une propriété liée a été modifiée.|  
...
|[COleControl::ClipCaretRect](#clipcaretrect)|Ajuste un rectangle du point d’insertion si elle est superposée par un contrôle.|  
|[COleControl::ControlInfoChanged](#controlinfochanged)|Appelez cette fonction après modification de l’ensemble des mnémoniques gérés par le contrôle.|  
...
|[COleControl::GetActivationPolicy](#getactivationpolicy)|Modifie le comportement d’activation par défaut d’un contrôle qui prend en charge la `IPointerInactive` interface.|  
...
|[COleControl::GetBackColor](#getbackcolor)|Retourne la valeur de la propriété BackColor stock.|  
|[COleControl::GetBorderStyle](#getborderstyle)|Retourne la valeur de la propriété stockée BorderStyle.|  
...
|[COleControl::GetClientOffset](#getclientoffset)|Récupère la différence entre l’angle supérieur gauche de la zone rectangulaire du contrôle et de l’angle supérieur gauche de sa zone cliente.|  
|[COleControl::GetClientRect](#getclientrect)|Récupère la taille de la zone du client du contrôle.|  
|[COleControl::GetClientSite](#getclientsite)|Interroge un objet pour le pointeur vers son site client actif dans son conteneur.|  
...
|[COleControl::GetDC](#getdc)|Fournit un moyen pour un contrôle sans fenêtre afin d’obtenir un contexte de périphérique de son conteneur.|  
...
|[COleControl::GetForeColor](#getforecolor)|Retourne la valeur de la propriété de couleur de premier plan de stock.|  
...
|[COleControl::GetWindowlessDropTarget](#getwindowlessdroptarget)|Substituez pour permettre un contrôle sans fenêtre soit la cible de glissement et les opérations de déplacement.|  
...
|[COleControl::InternalGetText](#internalgettext)|Récupère la propriété stock Caption ou Text.|  
...
|[COleControl::IsConvertingVBX](#isconvertingvbx)|Permet un chargement spécialisé d’un contrôle OLE.|  
...
|[COleControl::IsSubclassedControl](#issubclassedcontrol)|Appelée pour déterminer si le contrôle sous-classe un contrôle Windows.|  
...
|[COleControl::OnAppearanceChanged](#onappearancechanged)|Appelé lorsque la propriété d’apparence stock est modifiée.|  
|[COleControl::OnBackColorChanged](#onbackcolorchanged)|Appelé lorsque la propriété BackColor stock est modifiée.|  
|[COleControl::OnBorderStyleChanged](#onborderstylechanged)|Appelé lorsque la propriété stock BorderStyle est modifiée.|  
|[COleControl::OnClick](#onclick)|Appelée pour déclencher l’action, cliquez sur événements.|  
|[COleControl::OnClose](#onclose)|Notifie le contrôle que `IOleControl::Close` a été appelée.|  
|[COleControl::OnDoVerb](#ondoverb)|Appelé après l’exécution d’un verbe de contrôle.|  
|[COleControl::OnDraw](#ondraw)|Appelée lorsqu’un contrôle est demandé à se redessiner.|  
...
|[COleControl::OnEdit](#onedit)|Appelée par le conteneur pour activer l’interface utilisateur d’un contrôle OLE.|  
|[COleControl::OnEnabledChanged](#onenabledchanged)|Appelé lorsque la propriété Enabled du stock est modifié.|  
|[COleControl::OnEnumVerbs](#onenumverbs)|Appelée par le conteneur pour énumérer les verbes d’un contrôle.|  
|[COleControl::OnEventAdvise](#oneventadvise)|Appelé lorsque des gestionnaires d’événements soient ou non connectés à partir d’un contrôle.|  
...
|[COleControl::OnGetColorSet](#ongetcolorset)|Notifie le contrôle que `IOleObject::GetColorSet` a été appelée.|  
...
|[COleControl::OnGetNaturalExtent](#ongetnaturalextent)|Substituez pour récupérer la taille d’affichage du contrôle le plus proche de la taille et l’étendue en mode proposé.|  
...
|[COleControl::OnGetViewRect](#ongetviewrect)|Substituez pour convertir la taille du contrôle dans un rectangle commençant à une position spécifique.|  
|[COleControl::OnGetViewStatus](#ongetviewstatus)|Substituez pour récupérer l’état d’affichage du contrôle.|  
|[COleControl::OnHideToolBars](#onhidetoolbars)|Appelée par le conteneur lorsque le contrôle est l’interface utilisateur désactivée.|  
...
|[COleControl::OnKeyDownEvent](#onkeydownevent)|Appelée après le déclenchement de l’événement KeyDown de stock.|  
|[COleControl::OnKeyPressEvent](#onkeypressevent)|Appelée après le déclenchement de l’événement KeyPress stock.|  
...
|[COleControl::OnQueryHitPoint](#onqueryhitpoint)|Remplacer par la requête si un point donné chevauche l’affichage d’un contrôle.|  
|[COleControl::OnQueryHitRect](#onqueryhitrect)|Remplacer par la requête si l’affichage d’un contrôle empiète sur n’importe quel point dans un rectangle.|  
...
|[COleControl::OnRenderGlobalData](#onrenderglobaldata)|Appelé par l’infrastructure pour récupérer les données de la mémoire globale dans le format spécifié.|  
|[COleControl::OnResetState](#onresetstate)|Rétablit les valeurs par défaut les propriétés d’un contrôle.|  
|[COleControl::OnSetClientSite](#onsetclientsite)|Notifie le contrôle que `IOleControl::SetClientSite` a été appelée.|  
...
|[COleControl::OnSetObjectRects](#onsetobjectrects)|Appelé lorsque les dimensions du contrôle ont été modifiées.|  
...
|[COleControl::PostModalDialog](#postmodaldialog)|Notifie au conteneur qu’une boîte de dialogue modale a été fermée.|  
|[COleControl::PreModalDialog](#premodaldialog)|Notifie au conteneur qu’une boîte de dialogue modale est sur le point d’être affiché.|  
...
|[COleControl::ResetStockProps](#resetstockprops)|Initialise `COleControl` le stock de propriétés à leurs valeurs par défaut.|  
|[COleControl::ResetVersion](#resetversion)|Initialise le numéro de version à une valeur donnée.|  
|[COleControl::ScrollWindow](#scrollwindow)|Permet à un contrôle sans fenêtre faire défiler une zone au sein de son image active sur place sur l’écran.|  
...
|[COleControl::SetBackColor](#setbackcolor)|Définit la valeur de la propriété BackColor stock.|  
|[COleControl::SetBorderStyle](#setborderstyle)|Définit la valeur de la propriété stockée BorderStyle.|  
|[COleControl::SetCapture](#setcapture)|Provoque la fenêtre du contrôle conteneur à prendre possession de la capture de souris au nom du contrôle.|  
...
|[COleControl::SetFocus](#setfocus)|Provoque la fenêtre du contrôle conteneur à prendre possession du focus d’entrée au nom du contrôle.|  
|[COleControl::SetFont](#setfont)|Définit la valeur de la propriété stock Font.|  
|[COleControl::SetForeColor](#setforecolor)|Définit la valeur de la propriété de couleur de premier plan de stock.|  
...
|[COleControl::DrawMetafile](#drawmetafile)|Appelé par l’infrastructure lorsque le contexte de périphérique de métafichier est utilisé.|  
|[COleControl::IsInvokeAllowed](#isinvokeallowed)|Permet l’appel de méthode automation.|  
|[COleControl::SetInitialDataFormats](#setinitialdataformats)|Appelée par l’infrastructure pour initialiser la liste des formats de données pris en charge par le contrôle.|  
  
## <a name="remarks"></a>Remarques  
 Dérivée de `CWnd`, cette classe hérite de toutes les fonctionnalités d’un objet fenêtre Windows et des fonctionnalités supplémentaires spécifiques à OLE, telles que le déclenchement d’événements et de prendre en charge des méthodes et propriétés.  
  
 Contrôles OLE peuvent être insérées dans des applications conteneur OLE et communiquent avec le conteneur en utilisant un système bidirectionnels de déclenchement d’événement et exposition de méthodes et de propriétés pour le conteneur. Notez que les conteneurs OLE standards prennent uniquement en charge les fonctionnalités de base d’un contrôle OLE. Ils ne peuvent pas prendre en charge des fonctionnalités étendues d’un contrôle OLE. Déclenchement de l’événement se produit lorsque des événements sont envoyés au conteneur suite à certaines actions se produisent dans le contrôle. À son tour, le conteneur communique avec le contrôle à l’aide d’un ensemble de méthodes et propriétés analogues aux fonctions membres exposé et les données membres d’une classe C++. Cette approche permet au développeur de contrôler l’apparence du contrôle et de notifier au conteneur lorsque certaines actions se produisent.  
  
## <a name="windowless-controls"></a>Contrôles sans fenêtre  
 Contrôles OLE peuvent être utilisé actif en place sans fenêtre. Les contrôles sans fenêtre présentent des avantages significatifs :  
  
-   Les contrôles sans fenêtre peuvent être transparents et non rectangulaires  
  
-   Les contrôles sans fenêtre réduisent le temps de taille et la création d’instance de l’objet  
  
 Contrôles n’avez pas besoin d’une fenêtre. Services offerts par une fenêtre peuvent facilement être fournis via une fenêtre unique partagée (généralement du conteneur) et un peu de code de distribution. Une fenêtre est essentiellement une complication inutile sur l’objet.  
  
 Lors de l’activation sans fenêtre est utilisée, le conteneur (qui possède de fenêtre) est chargé de fournir des services qui auraient sinon été fournis par la fenêtre du contrôle. Par exemple, si votre contrôle doit interroger le focus clavier, interrogez la capture de la souris ou d’obtenir un contexte de périphérique, ces opérations sont gérées par le conteneur. Le `COleControl` [les fonctions membres de l’opération sans fenêtre](http://msdn.microsoft.com/en-us/e9e28f79-9a70-4ae4-a5aa-b3e92f1904df) appeler ces opérations sur le conteneur.  
  
 Lors de l’activation sans fenêtre est activée, les délégués de conteneur d’entrée de messages du contrôle `IOleInPlaceObjectWindowless` interface (une extension de [IOleInPlaceObject](http://msdn.microsoft.com/library/windows/desktop/ms692646) pour la prise en charge sans fenêtre). `COleControl`d’implémentation de cette interface distribue ces messages dans la table des messages de contrôle, après ajustement de la souris en coordonnées correctement. Vous pouvez traiter ces messages comme des messages de fenêtres ordinaires, en ajoutant les entrées correspondantes dans la table des messages.  
  
 Dans un contrôle sans fenêtre, vous devez toujours utiliser le `COleControl` au lieu de correspondants, les fonctions membres `CWnd` les fonctions membres ou leurs fonctions API Windows associées.  
  
 Objets de contrôle OLE peuvent également créer une fenêtre uniquement lorsqu’ils deviennent actifs, mais augmente la quantité de travail nécessaire pour la transition d’actif / inactif et la vitesse de la transition tombe en panne. Il existe des cas lorsqu’il s’agit d’un problème : par exemple, considérez une grille de zones de texte. Lorsque le curseur haut et bas dans la colonne, chaque contrôle doit être en place activées et désactivées puis. La vitesse de la transition active/inactive affecte directement la vitesse de défilement.  
...
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers l’interface de dispatch de police ambiante du conteneur. La valeur par défaut est **NULL**. Si la valeur de retour n’est pas égal à **NULL**, vous êtes chargé de libérer la police en appelant son [IUnknown::Release](http://msdn.microsoft.com/library/windows/desktop/ms682317) fonction membre.  
  
### <a name="remarks"></a>Remarques  
 La propriété de police ambiante est définie par le conteneur et disponibles pour tous les contrôles. Notez que le conteneur n’est pas requis pour prendre en charge cette propriété.  
...
### <a name="return-value"></a>Valeur de retour  
 La valeur actuelle de la propriété ambiante du conteneur couleur de premier plan, le cas échéant. Si non pris en charge, cette fonction retourne la couleur de texte définies par le système de Windows.  
  
### <a name="remarks"></a>Notes  
 La propriété de couleur de premier plan ambiante est disponible pour tous les contrôles et est définie par le conteneur. Notez que le conteneur n’est pas requis pour prendre en charge cette propriété.  
...
### <a name="remarks"></a>Notes  
 Le contrôle peut utiliser LocaleID d’adapter son interface utilisateur pour les paramètres régionaux spécifiques. Notez que le conteneur n’est pas requis pour prendre en charge cette propriété.  
...
### <a name="remarks"></a>Notes  
 Propriété des unités d’échelle ambiante du conteneur peut être utilisée pour afficher les positions ou dimensions, accompagnées de l’unité choisie, telles que twips ou en centimètres. Notez que le conteneur n’est pas requis pour prendre en charge cette propriété.  
...
##  <a name="a-nameambientshowgrabhandlesa--colecontrolambientshowgrabhandles"></a><a name="ambientshowgrabhandles"></a>COleControl::AmbientShowGrabHandles  
 Détermine si le conteneur autorise le contrôle afficher les poignées de manipulation d’elles-mêmes lorsqu’elles sont actives.  
...
|------------------|-------------|  
|0|Alignement standard (nombres pour le texte de droit à gauche).|  
|1|Aligner à gauche|  
|2|Centre|  
|3|Aligner à droite|  
  
### <a name="remarks"></a>Remarques  
 Cette propriété est disponible pour tous les contrôles intégrés et est définie par le conteneur. Notez que le conteneur n’est pas requis pour prendre en charge cette propriété.  
...
### <a name="remarks"></a>Notes  
 Par exemple, un conteneur peut définissez-la **TRUE** en mode Création.  
...
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si le conteneur est en mode utilisateur. 0 dans le cas contraire (en mode Création). Si cette propriété n’est pas pris en charge, cette fonction renvoie la valeur TRUE.  
  
### <a name="remarks"></a>Remarques  
 Par exemple, un conteneur peut définissez-la **FALSE** en mode Création.  
...
### <a name="parameters"></a>Paramètres  
 `dispid`  
 L’ID de dispatch d’une propriété du contrôle dépendante.  
...
### <a name="remarks"></a>Remarques  
 Si l’autorisation est refusée, le contrôle doit permettre pas la valeur de la modification de propriété. Pour cela, en ignorant ou en échec de l’action qui a tenté de modifier la valeur de propriété.  
...
### <a name="remarks"></a>Notes  
 Entrée `pPoint` est relatif à l’origine de la zone cliente du contrôle OLE (coin supérieur gauche de la zone cliente du contrôle). Sortie `pPoint` est relatif à l’origine du parent (coin supérieur gauche du conteneur).  
...
### <a name="parameters"></a>Paramètres  
 `lpRect`  
 Lors de l’entrée, un pointeur vers un [RECT](../../mfc/reference/rect-structure1.md) structure qui contient la zone du signe insertion pour être ajustée. En sortie, la zone du signe insertion ajustée, ou **NULL** si le rectangle du point d’insertion est complètement couvert.  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Remarques  
 Un point d’insertion est une ligne, un bloc ou bitmap qui indique généralement qu’où texte ou les graphiques seront insérés clignotant.  
  
 Un objet sans fenêtre ne peut pas afficher en toute sécurité un signe insertion sans vérifier au préalable si le signe insertion partiellement ou totalement masqué par le chevauchement des objets. Pour que cela soit possible, un objet peut utiliser `ClipCaretRect` pour obtenir le signe insertion ajusté (réduit) pour vous assurer qu’il tienne dans la zone de découpage.  
  
 Création d’un point d’insertion des objets doivent envoyer le rectangle de signe insertion pour `ClipCaretRect` et utiliser le rectangle pour le point d’insertion. Si le point d’insertion est entièrement masqué, cette méthode retournera **FALSE** et le signe insertion ne doit pas être affiché dans ce cas.  
...
### <a name="remarks"></a>Notes  
 Cette fonction n’est généralement pas appelée directement. À la place du contrôle OLE est généralement créé par la fabrique de classe.  
 ...
 *lpszSource*  
 Le nom du module génère l’erreur (en général, le nom du module de contrôle OLE).  
  
 `lpszHelpFile`  
 Le nom du fichier d’aide qui contient une description de l’erreur.  
  
 `nHelpID`  
 L’ID de contexte permettent de l’erreur signalée.  
...
### <a name="remarks"></a>Notes  
 Les paramètres substituables `COleControl::OnClick` fonction membre sera appelée et une action, cliquez sur événement est déclenché si le contrôle prend en charge.  
  
 Cette fonction est prise en charge par la `COleControl` classe de base comme une méthode stockée, appelée DoClick. Pour plus d’informations, consultez l’article [contrôles ActiveX : méthodes](../../mfc/mfc-activex-controls-methods.md).  
  
##  <a name="a-namedopropexchangea--colecontroldopropexchange"></a><a name="dopropexchange"></a>COleControl::DoPropExchange  
 Appelé par l’infrastructure lors du chargement ou le stockage d’un contrôle à partir d’une représentation de stockage permanent, tel qu’un ensemble de flux de données ou de la propriété.  
...
### <a name="parameters"></a>Paramètres  
 `pPX`  
 Un pointeur vers un `CPropExchange` objet. L’infrastructure fournit cet objet pour établir le contexte de l’échange de propriété, y compris sa direction.  
...
### <a name="remarks"></a>Remarques  
 Appelez cette fonction pour gérer correctement la peinture d’un contrôle OLE inactive. Cette fonction doit être utilisée uniquement si OLE contrôle sous-classe un contrôle Windows et doit être appelé le `OnDraw` fonction de votre contrôle.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `rc`  
 Zone rectangulaire dans.  
  
### <a name="remarks"></a>Remarques  
 Cette fonction appelle directement le substituable `OnDraw` (fonction).  
...
### <a name="remarks"></a>Remarques  
 Cette caractéristique permet un contrôle prendre en charge la relation contenant-contenu visuel d’autres contrôles, mais pas true imbrication OLE. Un exemple serait avec plusieurs contrôles à l’intérieur d’une zone de groupe. Ces contrôles ne sont pas OLE contenu, mais elles se trouvent dans la même zone de groupe.  
...
### <a name="parameters"></a>Paramètres  
 `pPX`  
 Un pointeur vers un [CPropExchange](../../mfc/reference/cpropexchange-class.md) objet. L’infrastructure fournit cet objet pour établir le contexte de l’échange de propriété, y compris sa direction.  
...
##  <a name="a-namefireclicka--colecontrolfireclick"></a><a name="fireclick"></a>COleControl::FireClick  
 Appelé par l’infrastructure lorsque l’utilisateur clique sur un contrôle actif.  
...
### <a name="remarks"></a>Remarques  
 ...
 Pour l’activation automatique de l’événement Click se produise, table d’événements du contrôle doit avoir une action, cliquez sur événement défini.  
  
##  <a name="a-namefiredblclicka--colecontrolfiredblclick"></a><a name="firedblclick"></a>COleControl::FireDblClick  
 Appelé par l’infrastructure lorsque la souris est double-cliqué sur un contrôle actif.  
...
### <a name="remarks"></a>Notes  
 ...
 Pour l’activation automatique d’un événement de double clic de se produire, table d’événements du contrôle doit avoir un événement de double clic stock défini.  
...
### <a name="remarks"></a>Remarques  
 Cet événement fournit un moyen de signalisation dans des lieux appropriés dans votre code, qu’une erreur s’est produite dans votre contrôle. Contrairement à d’autres événements stock, telles que le clic ou MouseMove, erreur n’est jamais déclenchée par l’infrastructure.  
 ...
 L’implémentation de l’erreur de Stock d’un contrôle OLE événement utilise un `SCODE` valeur. Si votre contrôle utilise cet événement et est destiné à être utilisée dans Visual Basic 4.0, vous recevrez des erreurs, car la `SCODE` valeur n’est pas pris en charge dans Visual Basic.  
  
 Pour résoudre ce problème, modifiez manuellement le `SCODE` paramètre du contrôle. ODL pour une **long**. En outre, un événement personnalisé, une méthode ou une propriété qui utilise un `SCODE` paramètre entraîne également le même problème.  
...
### <a name="parameters"></a>Paramètres  
 `dispid`  
 L’ID de dispatch de l’événement de déclenchement.  
  
 `pbParams`  
 Un descripteur pour les types de paramètre de l’événement.  
  
### <a name="remarks"></a>Notes  
 En général cette fonction ne doit pas appelée directement. Au lieu de cela, vous appelez les fonctions de déclenchement d’événement dans la section de mappage des événements de la déclaration de classe de votre contrôle.  
...
> [!NOTE]
>  Constantes variants supplémentaires ont été définis pour tous les types variants, à l’exception de **VTS_FONT** et **VTS_PICTURE**, qui fournit un pointeur vers la constante de données variant. Ces constantes sont nommées à l’aide de la **VTS_P** `constantname` convention. Par exemple, **VTS_PCOLOR** est un pointeur vers un **VTS_COLOR** constante.  
  
##  <a name="a-namefirekeydowna--colecontrolfirekeydown"></a><a name="firekeydown"></a>COleControl::FireKeyDown  
 Appelé par l’infrastructure lorsqu’une touche est enfoncée alors que le contrôle est actif de l’interface utilisateur.  
...
### <a name="parameters"></a>Paramètres  
 `pnChar`  
 Pointeur vers la valeur de code de touche virtuelle de la touche enfoncée. Pour une liste des codes de touches virtuels standards, consultez Winuser.h  
...
- **CTRL_MASK** le CTRL a été enfoncée lors de l’action.  
  
- **ALT_MASK** le ALT a été enfoncée lors de l’action.  
  
### <a name="remarks"></a>Remarques  
 Si cet événement est défini comme un événement personnalisé, vous déterminez lorsque l’événement est déclenché.  
  
 Pour l’activation automatique d’un événement KeyDown se produise, table d’événements du contrôle doit avoir un événement KeyDown stock défini.  
  
##  <a name="a-namefirekeypressa--colecontrolfirekeypress"></a><a name="firekeypress"></a>COleControl::FireKeyPress  
 Appelé par l’infrastructure lorsqu’une touche est enfoncée et relâchée pendant que le contrôle personnalisé est Active l’interface utilisateur dans le conteneur.  
...
### <a name="remarks"></a>Remarques  
 ...
 Pour l’activation automatique d’un événement KeyPress, table d’événements du contrôle doit avoir un événement KeyPress stock défini.  
  
##  <a name="a-namefirekeyupa--colecontrolfirekeyup"></a><a name="firekeyup"></a>COleControl::FireKeyUp  
 Appelé par l’infrastructure lorsqu’une touche est relâchée alors que le contrôle personnalisé est Active l’interface utilisateur dans le conteneur.  
...
### <a name="parameters"></a>Paramètres  
 `pnChar`  
 Pointeur vers la valeur de code de touche virtuelle de la touche finale. Pour une liste des codes de touches virtuels standards, consultez Winuser.h  
...
### <a name="remarks"></a>Remarques  
 ...
 Pour l’activation automatique d’un événement KeyUp se produise, table d’événements du contrôle doit avoir un événement KeyUp stock défini.  
  
##  <a name="a-namefiremousedowna--colecontrolfiremousedown"></a><a name="firemousedown"></a>COleControl::FireMouseDown  
 Appelé par l’infrastructure lorsqu’un bouton de la souris est enfoncé sur actif d’un contrôle personnalisé.  
...
### <a name="remarks"></a>Remarques  
 ...
 Pour l’activation automatique d’un événement MouseDown, table d’événements du contrôle doit avoir un événement MouseDown stock défini.  
  
##  <a name="a-namefiremousemovea--colecontrolfiremousemove"></a><a name="firemousemove"></a>COleControl::FireMouseMove  
 Appelé par l’infrastructure lorsque le curseur est placé sur un contrôle personnalisé actif.  
...
### <a name="parameters"></a>Paramètres  
 `nButton`  
 La valeur numérique de boutons de la souris enfoncé. Contient une combinaison des valeurs suivantes :  
...
### <a name="remarks"></a>Notes  
 ...
 Pour l’activation automatique d’un événement MouseMove, table d’événements du contrôle doit avoir un événement MouseMove stock défini.  
...
### <a name="parameters"></a>Paramètres  
 `nButton`  
 La valeur numérique de la souris du bouton. Il peut avoir une des valeurs suivantes :  
  
- **LEFT_BUTTON** le bouton gauche de la souris a été relâché.  
  
- **MIDDLE_BUTTON** le bouton central de la souris a été relâché.  
  
- **RIGHT_BUTTON** le bouton droit de la souris a été publié.  
...
### <a name="remarks"></a>Remarques  
 ...
 Pour l’activation automatique d’un événement MouseUp, table d’événements du contrôle doit avoir un événement MouseUp stock défini.  
 ...
 **READYSTATE_UNINITIALIZED**  
 État d’initialisation par défaut  
 ...
 **READYSTATE_INTERACTIVE**  
 Contrôle a suffisamment de données pour être interactives, mais pas tous asynchrone des données sont encore chargées  
...
##  <a name="a-namegetactivationpolicya--colecontrolgetactivationpolicy"></a><a name="getactivationpolicy"></a>COleControl::GetActivationPolicy  
 Modifie le comportement d’activation par défaut d’un contrôle qui prend en charge la `IPointerInactive` interface.  
 ...
 **POINTERINACTIVE_ACTIVATEONDRAG**  
 L’objet doit être activé lorsque la souris est glissée au cours d’une opération de glissement sur place et opération de déplacement.  
  
### <a name="remarks"></a>Notes  
 Lors de la `IPointerInactive` interface est activée, le conteneur délègue les `WM_SETCURSOR` et `WM_MOUSEMOVE` messages. `COleControl`d’implémentation de cette interface distribue ces messages dans la table des messages de contrôle, après ajustement de la souris en coordonnées correctement.  
  
 Lorsque le conteneur reçoit un `WM_SETCURSOR` ou `WM_MOUSEMOVE` message avec le pointeur de la souris sur un objet inactif prenant en charge `IPointerInactive`, il doit appeler `GetActivationPolicy` sur l’interface et les indicateurs de retour à partir de la **POINTERINACTIVE** énumération.  
  
 Vous pouvez traiter ces messages comme des messages de fenêtres ordinaires, en ajoutant les entrées correspondantes dans la table des messages. Dans les gestionnaires, évitez d’utiliser le `m_hWnd` variable membre (ou toutes les fonctions de membre qui l’utilise) sans d’abord vérifier que sa valeur est non - **NULL**.  
  
 N’importe quel objet destiné à de plus de définir le curseur de la souris et/ou de déclencher un événement de déplacement de la souris, tels que des commentaires spéciaux visual, doivent retourner le **POINTERINACTIVE_ACTIVATEONENTRY** indicateur et dessiner les commentaires uniquement lorsqu’elles sont actives. Si l’objet retourné cet indicateur, le conteneur doit activer sur place immédiatement et transmet le message même qui a déclenché l’appel à `GetActivationPolicy`.  
  
 Si les deux le **POINTERINACTIVE_ACTIVATEONENTRY** et **POINTERINACTIVE_DEACTIVATEONLEAVE** indicateurs sont retournés, puis l’objet sera activé uniquement lorsque la souris est au-dessus de l’objet. Si seul le **POINTERINACTIVE_ACTIVATEONENTRY** indicateur est retourné, puis l’objet n’est activée une fois lorsque la souris pénètre dans l’objet.  
  
 Vous pouvez également être la cible d’un glisser et déplacer un contrôle inactif. Cela nécessite l’activation du contrôle au moment où que l’utilisateur fait glisser un objet, afin que la fenêtre de contrôle peut être enregistrée comme une cible de déplacement. Pour que l’activation se produisent pendant une opération de glissement, retourner le **POINTERINACTIVE_ACTIVATEONDRAG** indicateur :  
  
 [!code-cpp[NVC_MFCAxCtl n °&1;](../../mfc/reference/codesnippet/cpp/colecontrol-class_1.cpp)]  
  
 Les informations communiquées par `GetActivationPolicy` ne doit pas être mise en cache par un conteneur. Au lieu de cela, cette méthode doit être appelée chaque fois que la souris entre dans un objet inactif.  
  
 Si un objet inactif ne demande pas à être activé lorsque la souris est placé dessus sur place, son conteneur doit distribuer ultérieures `WM_SETCURSOR` messages à cet objet en appelant [OnInactiveSetCursor](#oninactivesetcursor) tant que le pointeur de la souris reste sur l’objet.  
  
 L’activation de la `IPointerInactive` interface généralement indique que le contrôle doit être capable de traiter les messages de la souris à tout moment. Pour obtenir ce comportement dans un conteneur qui ne prend en charge les `IPointerInactive` interface, vous devez avoir toujours activé quand il est visible, ce qui signifie que le contrôle doit avoir la **OLEMISC_ACTIVATEWHENVISIBLE** indicateur parmi ses indicateurs divers. Toutefois, pour empêcher cet indicateur de prendre effet dans un conteneur qui prend en charge `IPointerInactive`, vous pouvez également spécifier le **OLEMISC_IGNOREACTIVATEWHENVISIBLE** indicateur :  
...
### <a name="remarks"></a>Notes  
 ...
 Notez que les fonctions ont déjà été prévues pour les propriétés ambiantes courantes, telles que [AmbientBackColor](#ambientbackcolor) et [AmbientFont](#ambientfont).  
...
##  <a name="a-namegetborderstylea--colecontrolgetborderstyle"></a><a name="getborderstyle"></a>COleControl::GetBorderStyle  
 Implémente la fonction Get de propriété BorderStyle stockée de votre contrôle.  
...
### <a name="return-value"></a>Valeur de retour  
 Si le contrôle est activé et sans fenêtre, retourne **cela** si le contrôle a actuellement la capture de la souris (comme déterminé par le conteneur du contrôle), ou **NULL** si elle n’a pas la capture.  
  
 Sinon, retourne la `CWnd` objet qui possède la capture de la souris (même en tant que `CWnd::GetCapture`).  
  
### <a name="remarks"></a>Remarques  
 Un contrôle sans fenêtre activé reçoit la souris capture lorsque [SetCapture](#setcapture) est appelée.  
...
### <a name="remarks"></a>Remarques  
 En règle générale implémenté par le [IMPLEMENT_OLECREATE_EX](class-factories-and-licensing.md#implement_olecreate_ex).  
  
##  <a name="a-namegetclientoffseta--colecontrolgetclientoffset"></a><a name="getclientoffset"></a>COleControl::GetClientOffset  
 Récupère la différence entre l’angle supérieur gauche de la zone rectangulaire du contrôle et de l’angle supérieur gauche de sa zone cliente.  
...
### <a name="remarks"></a>Remarques  
 Le contrôle OLE a une zone rectangulaire dans son conteneur. La zone cliente du contrôle est la zone de contrôle à l’exception des bordures et des barres de défilement. Le décalage est récupérée par `GetClientOffset` est la différence entre l’angle supérieur gauche de la zone rectangulaire du contrôle et de l’angle supérieur gauche de sa zone cliente. Si votre contrôle a des éléments non clients autres que les bordures standards et les barres de défilement, remplacer cette fonction membre pour spécifier le décalage.  
  
##  <a name="a-namegetclientrecta--colecontrolgetclientrect"></a><a name="getclientrect"></a>COleControl::GetClientRect  
 Récupère la taille de la zone du client du contrôle.  
...
### <a name="parameters"></a>Paramètres  
 `lpRect`  
 Pointeur vers un `RECT` structure contenant les dimensions de la zone du client du contrôle sans fenêtre, c'est-à-dire la taille du contrôle moins les bordures de fenêtre, cadres, barres de défilement et ainsi de suite. Le `lpRect` paramètre indique la taille du rectangle client du contrôle pas sa position.  
  
##  <a name="a-namegetclientsitea--colecontrolgetclientsite"></a><a name="getclientsite"></a>COleControl::GetClientSite  
 Interroge un objet pour le pointeur vers son site client actif dans son conteneur.  
...
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers le site client actif du contrôle dans son conteneur.  
  
### <a name="remarks"></a>Remarques  
 Le pointeur retourné pointe vers une instance de `IOleClientSite`. Le `IOleClientSite` interface, implémentée par les conteneurs, est de vue l’objet de son contexte : où elle est ancrée dans le document où il obtient son stockage, interface utilisateur et autres ressources.  
...
### <a name="remarks"></a>Remarques  
 Par défaut, `GetControlFlags` renvoie `fastBeginPaint | clipPaintDC`.  
  
 `fastBeginPaint`  
 Si la valeur, utilise une peinture de début (fonction) adapté aux contrôles OLE au lieu de la [BeginPaint](http://msdn.microsoft.com/library/windows/desktop/dd183362) API (la valeur par défaut).  
  
 `clipPaintDC`  
 Si ce n’est pas définie, désactive l’appel à `IntersectClipRect` effectuées par `COleControl` et un gain de vitesse limité. Si vous utilisez l’activation sans fenêtre, l’indicateur est sans effet.  
  
 `pointerInactive`  
 Si défini, fournit l’interaction souris pendant que le contrôle est inactif en activant `COleControl`d’implémentation de la `IPointerInactive` interface, qui est désactivée par défaut.  
  
 `noFlickerActivate`  
 Si défini, élimine les opérations de dessin supplémentaires et le scintillement qui accompagne cet article. À utiliser lorsque votre contrôle se dessine identique dans les états actifs et inactifs. Si vous utilisez l’activation sans fenêtre, l’indicateur est sans effet.  
  
 `windowlessActivate`  
 Si défini, il indique à votre contrôle utilise l’activation sans fenêtre.  
  
 `canOptimizeDraw`  
 Si défini, il indique que le contrôle effectue un dessin optimisé, si le conteneur prend en charge.  
...
### <a name="remarks"></a>Notes  
 Notez que toutes les coordonnées de fenêtres de contrôle par rapport au coin supérieur gauche du contrôle.  
  
##  <a name="a-namegetdca--colecontrolgetdc"></a><a name="getdc"></a>COleControl::GetDC  
 Fournit un objet sans fenêtres pour un écran (ou compatible) contexte périphérique à partir de son conteneur.  
 ...
 *lprcRect*  
 ...
 `dwFlags`  
 Attributs du contexte de périphérique de dessin. Choix possibles sont :  
  
- **OLEDC_NODRAW** indique que l’objet ne sont pas utiliser le contexte de périphérique pour effectuer aucun dessin, mais simplement pour obtenir des informations sur le périphérique d’affichage. Le conteneur doit simplement transmettre le contrôleur de domaine de la fenêtre sans traitement supplémentaire.  
  
- **OLEDC_PAINTBKGND** demande que le conteneur peindre l’arrière-plan avant de retourner le contrôleur de domaine. Un objet doit utiliser cet indicateur si elle demande un contrôleur de domaine pour redessiner une zone avec un arrière-plan transparent.  
  
- **OLEDC_OFFSCREEN** informe le conteneur de l’objet souhaite à restituer dans une bitmap hors écran doit alors être copiée sur l’écran. Un objet doit utiliser cet indicateur lorsqu’il doit effectuer l’opération de dessin génère un grand nombre de scintillement. Le conteneur est libre d’honorer cette demande ou non. Toutefois, si cet indicateur n’est pas défini, le conteneur doit obtenir un écran contrôleur de domaine. Cela permet aux objets effectuer des opérations d’écran direct telles que l’affichage d’une sélection (via une **XOR** opération).  
...
### <a name="remarks"></a>Remarques  
 Le [ReleaseDC](#releasedc) fonction membre doit être appelée pour libérer le contexte après la peinture. Lors de l’appel `GetDC`, les objets passent le rectangle qu’ils souhaitent introduire dans leurs coordonnées clientes. `GetDC`traduit ces coordonnées de la zone cliente de conteneur. L’objet ne doit pas demander un rectangle de dessin souhaité dépasse sa propre rectangle de zone client, dont la taille peut être récupérée avec [GetClientRect](#getclientrect). Cela empêche les objets par inadvertance de dessin où ils ne devraient pas.  
  
##  <a name="a-namegetenableda--colecontrolgetenabled"></a><a name="getenabled"></a>COleControl::GetEnabled  
 Implémente la fonction Get de propriété Enabled de stockage de votre contrôle.  
...
### <a name="remarks"></a>Notes  
 La fonction qui appelle cette fonction est chargée de libérer le pointeur après avoir terminé l’objet. Notez que le conteneur n’est pas requis pour prendre en charge cet objet.  
...
### <a name="return-value"></a>Valeur de retour  
 ...
 Sinon, retourne la `CWnd` objet qui a le focus (même en tant que `CWnd::GetFocus`).  
...
##  <a name="a-namegetfonttextmetricsa--colecontrolgetfonttextmetrics"></a><a name="getfonttextmetrics"></a>COleControl::GetFontTextMetrics  
 Mesure les mesures de texte pour les `CFontHolder` objet détenu par le contrôle.  
...
### <a name="remarks"></a>Notes  
 Une telle police peut être sélectionnée avec la [COleControl::SelectFontObject](#selectfontobject) (fonction). `GetFontTextMetrics`Initialise le `TEXTMETRIC` structure vers laquelle pointe `lptm` avec des informations de métriques valide sur `fontHolder`de police en cas de réussite, ou remplir la structure avec des zéros en cas d’échec. Vous devez utiliser cette fonction au lieu de [GetTextMetrics](http://msdn.microsoft.com/library/windows/desktop/dd144941) lorsque peinture de votre contrôle, car les contrôles, comme les incorporant OLE, peuvent être requises pour se rendre dans un métafichier.  
  
 Le `TEXTMETRIC` de la structure de la police par défaut est actualisé lorsque le [SelectFontObject](#selectfontobject) fonction est appelée. Vous devez appeler `GetFontTextMetrics` uniquement après que la sélection de la propriété stock Font pour assurer les informations qu’elle fournit est valide.  
...
##  <a name="a-namegetmessagestringa--colecontrolgetmessagestring"></a><a name="getmessagestring"></a>COleControl::GetMessageString  
 Appelé par l’infrastructure pour obtenir une chaîne courte qui décrit l’objectif de l’élément identifié par `nID`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `rMessage`  
 Une référence à un [CString](../../atl-mfc-shared/reference/cstringt-class.md) via lesquels une chaîne est retournée.  
  
### <a name="remarks"></a>Notes  
 Cela permet d’obtenir un message à afficher dans la barre d’état lorsque l’élément de menu est en surbrillance. L’implémentation par défaut tente de charger une ressource chaîne identifiée par `nID`.  
...
### <a name="remarks"></a>Remarques  
 Appelez cette fonction à la place de la fonction Get d’une propriété où la récupération de la propriété par l’utilisateur du contrôle n’est pas pris en charge. Un exemple serait une propriété écriture seule.  
...
### <a name="return-value"></a>Valeur de retour  
 L’état de préparation du contrôle, une des valeurs suivantes :  
...
### <a name="remarks"></a>Remarques  
 La plupart des contrôles simples jamais besoin de faire la différence entre **LOADED** et `INTERACTIVE`. Toutefois, les contrôles qui prennent en charge les propriétés de chemin d’accès de données peut-être pas prêts à être interactives jusqu'à réception d’au moins certaines données de façon asynchrone. Un contrôle doit tenter de devenir interactives dès que possible.  
...
##  <a name="a-namegetrectincontainera--colecontrolgetrectincontainer"></a><a name="getrectincontainer"></a>COleControl::GetRectInContainer  
 Obtient les coordonnées du rectangle du contrôle relatif du conteneur, exprimé en unités de périphérique.  
...
##  <a name="a-namegetstocktextmetricsa--colecontrolgetstocktextmetrics"></a><a name="getstocktextmetrics"></a>COleControl::GetStockTextMetrics  
 Mesure les métriques de texte pour la propriété du contrôle stockée Font, qui peut être sélectionnée avec le [SelectStockFont](#selectstockfont) (fonction).  
...
### <a name="remarks"></a>Remarques  
 Le `GetStockTextMetrics` fonction initialisera le `TEXTMETRIC` structure vers laquelle pointe `lptm` avec les informations des métriques valide en cas de réussite ou la structure avec des zéros non significatifs en cas d’échec de remplissage. Utilisez cette fonction au lieu de [GetTextMetrics](http://msdn.microsoft.com/library/windows/desktop/dd144941) lorsque peinture de votre contrôle, car les contrôles, comme les incorporant OLE, peuvent être requises pour se rendre dans un métafichier.  
  
 Le `TEXTMETRIC` de la structure de la police par défaut est actualisé lorsque le `SelectStockFont` fonction est appelée. Vous devez appeler cette fonction uniquement après que la sélection de la police pour vous assurer qu’il fournit les informations de stock est valide.  
...
### <a name="return-value"></a>Valeur de retour  
 La valeur actuelle de la chaîne de texte de contrôle ou une chaîne de longueur zéro si aucune chaîne n’est présent.  
  
> [!NOTE]
>  Pour plus d’informations sur la `BSTR` type de données, voir [des Types de données](../../mfc/reference/data-types-mfc.md) dans la section Macros and Globals.  
  
### <a name="remarks"></a>Notes  
 Notez que l’appelant de cette fonction doit appeler `SysFreeString` sur la chaîne retournée pour gratuitement la ressource. Dans l’implémentation du contrôle, utilisez `InternalGetText` pour accéder aux propriétés stock de texte ou de la légende du contrôle.  
...
### <a name="return-value"></a>Valeur de retour  
 Pointeur vers l’objet `IDropTarget` interface. Dans la mesure où il n’est pas une fenêtre, un objet sans fenêtre ne peut pas inscrire un `IDropTarget` interface. Toutefois, pour participer au glisser -déplacer, un objet sans fenêtre peut toujours implémenter l’interface et retourner dans `GetWindowlessDropTarget`.  
  
### <a name="remarks"></a>Remarques  
 Normalement, cela suppose que la fenêtre de contrôle soit stockée en tant que cible de dépôt. Mais puisque le contrôle n’a aucune fenêtre propre, le conteneur utilise sa propre fenêtre comme cible de déplacement. Le contrôle doit simplement fournir une implémentation de la `IDropTarget` interface à laquelle le conteneur peut déléguer des appels au moment opportun. Exemple :  
...
### <a name="parameters"></a>Paramètres  
 *piidPrimary*  
 Pointeur vers l’ID de l’interface de répartition principale du contrôle.  
...
##  <a name="a-nameinternalgetfonta--colecontrolinternalgetfont"></a><a name="internalgetfont"></a>COleControl::InternalGetFont  
 Accède à la propriété stock Font du contrôle  
...
### <a name="return-value"></a>Valeur de retour  
 Une référence à un [CFontHolder](../../mfc/reference/cfontholder-class.md) objet qui contient l’objet de police stock.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `bErase`  
 Spécifie si l’arrière-plan dans la zone de mise à jour doit être effacé lorsque la zone de mise à jour est traitée.  
  
### <a name="remarks"></a>Remarques  
 Si `lpRect` a un **NULL** valeur, l’ensemble du contrôle est redessiné. Si `lpRect` n’est pas **NULL**, cela indique la partie de la zone rectangulaire du contrôle à invalider. Dans les cas où le contrôle n’a aucune fenêtre, ou n’est pas actif actuellement, le rectangle est ignoré, et un appel est effectué sur le site client [IAdviseSink::OnViewChange](http://msdn.microsoft.com/library/windows/desktop/ms694337) fonction membre. Utilisez cette fonction au lieu de `CWnd::InvalidateRect` ou `InvalidateRect`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `bErase`  
 Spécifie si l’arrière-plan d’une région non valide est à effacer. Si **TRUE**, l’arrière-plan est effacé. Si **FALSE**, l’arrière-plan reste inchangé.  
...
### <a name="remarks"></a>Remarques  
 ...
 Si `bErase` est **TRUE** pour tout ou partie de la région de mise à jour de l’arrière-plan de la région entière, pas seulement dans la partie donnée, est effacé.  
  
##  <a name="a-nameisconvertingvbxa--colecontrolisconvertingvbx"></a><a name="isconvertingvbx"></a>COleControl::IsConvertingVBX  
 Permet un chargement spécialisé d’un contrôle OLE.  
...
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si le contrôle est converti ; sinon 0.  
  
### <a name="remarks"></a>Remarques  
 Lors de la conversion d’un formulaire qui utilise VBX de contrôles à l’autre qui utilise des contrôles OLE, le code de chargement spéciales pour les contrôles OLE peut être nécessaire. Par exemple, si vous chargez une instance de votre contrôle OLE, vous pouvez avoir un appel à [PX_Font](persistence-of-ole-controls.md#px_font) dans votre `DoPropExchange`:  
  
 [!code-cpp[NVC_MFCAxCtl n °&3;](../../mfc/reference/codesnippet/cpp/colecontrol-class_4.cpp)]  
  
 Toutefois, les contrôles VBX n’ont pas un objet police ; chaque propriété de la police a été enregistrée individuellement. Dans ce cas, vous utiliseriez `IsConvertingVBX` pour faire la distinction entre ces deux cas :  
  
 [!code-cpp[NVC_MFCAxCtl n °&4;](../../mfc/reference/codesnippet/cpp/colecontrol-class_5.cpp)]  
  
 Un autre cas serait si votre contrôle VBX enregistré des données binaires (dans son **VBM_SAVEPROPERTY** le Gestionnaire de messages), et votre contrôle OLE enregistre ses données binaires dans un format différent. Si vous souhaitez que votre contrôle OLE pour être compatible avec le contrôle VBX, vous pouvez lire les deux formats anciens et nouveaux à l’aide de la `IsConvertingVBX` fonction en distinguant si le contrôle VBX ou du contrôle OLE a été chargé.  
  
 Dans votre contrôle `DoPropExchange` (fonction), vous pouvez vérifier cette condition et si la valeur est true, exécuter du code de charge spécifique pour cette conversion (tels que les exemples précédents). Si le contrôle n’est pas converti, vous pouvez exécuter le code de chargement normal. Cette possibilité s’applique uniquement aux contrôles équivalents et VBX converties.  
...
### <a name="remarks"></a>Notes  
 Implémentation de l’infrastructure de **IDispatch::Invoke** appelle **IsInvokeAllowed** pour déterminer si une fonction donnée (identifié par `dispid`) peuvent être appelées. Le comportement par défaut pour un contrôle OLE est de permettre des méthodes automation d’être appelée uniquement si le contrôle a été initialisé ; Toutefois, **IsInvokeAllowed** est une fonction virtuelle et peut être remplacé si nécessaire (par exemple, lorsque le contrôle est utilisé en tant que serveur automation). Pour plus d’informations, consultez l’article de la Base de connaissances Q166472, « comment faire : utiliser un contrôle OLE en tant que serveur Automation. » Articles de la Base de connaissances sont disponibles dans la documentation Visual Studio de MSDN Library ou à [http://support.microsoft.com](http://support.microsoft.com/).  
...
##  <a name="a-nameisoptimizeddrawa--colecontrolisoptimizeddraw"></a><a name="isoptimizeddraw"></a>Fonction membre COleControl::IsOptimizedDraw  
...
### <a name="remarks"></a>Remarques  
 Si le dessin optimisé est pris en charge, le contrôle ne peut-être pas puis anciens objets (stylets, pinceaux, polices, etc.) dans le contexte de périphérique après le dessin.  
...
### <a name="parameters"></a>Paramètres  
 *strNewPath*  
 Pointeur vers une chaîne contenant le chemin d’accès qui fait référence à l’emplacement absolu de la propriété de contrôle asynchrones.  
  
 *Prop*  
 A [CDataPathProperty](../../mfc/reference/cdatapathproperty-class.md) objet qui implémente une propriété de contrôle asynchrones.  
...
### <a name="remarks"></a>Remarques  
 Notez que chaque verrouillage du contrôle doit être associé avec un déverrouillage du contrôle une fois. Vous devez uniquement verrouiller votre contrôle sur de courtes périodes, telles que lors de l’activation d’un événement.  
...
##  <a name="a-nameonappearancechangeda--colecontrolonappearancechanged"></a><a name="onappearancechanged"></a>COleControl::OnAppearanceChanged  
 Appelé par l’infrastructure lorsque la valeur de propriété d’apparence stockée a été modifiée.  
...
### <a name="remarks"></a>Notes  
 Remplacez cette fonction si vous souhaitez que la notification après cette modification de propriété. L’implémentation par défaut appelle `InvalidateControl`.  
...
##  <a name="a-nameonbackcolorchangeda--colecontrolonbackcolorchanged"></a><a name="onbackcolorchanged"></a>COleControl::OnBackColorChanged  
 Appelé par l’infrastructure lorsque la valeur de la propriété BackColor stockée a été modifiée.  
...
##  <a name="a-nameonborderstylechangeda--colecontrolonborderstylechanged"></a><a name="onborderstylechanged"></a>COleControl::OnBorderStyleChanged  
 Appelé par l’infrastructure lorsque la valeur de propriété stockée BorderStyle a été modifiée.  
...
### <a name="remarks"></a>Remarques  
 ...
 Remplacez cette fonction si vous souhaitez que la notification après cette modification de propriété.  
  
##  <a name="a-nameonclicka--colecontrolonclick"></a><a name="onclick"></a>COleControl::OnClick  
 Appelé par l’infrastructure lorsqu’un bouton de la souris a été utilisé ou la méthode stock DoClick a été appelée.  
...
##  <a name="a-nameonclosea--colecontrolonclose"></a><a name="onclose"></a>COleControl::OnClose  
 Appelé par l’infrastructure lorsque le conteneur a appelé du contrôle **IOleControl::Close** (fonction).  
...
##  <a name="a-nameondoverba--colecontrolondoverb"></a><a name="ondoverb"></a>COleControl::OnDoVerb  
 Appelé par l’infrastructure lorsque le conteneur appelle le **IOleObject::DoVerb** fonction membre.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `hWndParent`  
 Handle de la fenêtre parent du contrôle. Si l’exécution du verbe crée une fenêtre (ou windows), `hWndParent` doit être utilisé comme parent.  
...
### <a name="remarks"></a>Remarques  
 ...
 Cette fonction est généralement utilisée en tant que la fonction gestionnaire d’une `ON_OLEVERB` message d’entrée de mappage. Cela permet un verbe « Edit » dans le menu du contrôle « Objet ». Exemple :  
...
##  <a name="a-nameonenabledchangeda--colecontrolonenabledchanged"></a><a name="onenabledchanged"></a>COleControl::OnEnabledChanged  
 Appelé par l’infrastructure lorsque la valeur de la propriété Enabled du stock a été modifiée.  
...
### <a name="remarks"></a>Remarques  
 Remplacez cette fonction si vous souhaitez que la notification après cette modification de propriété. L’implémentation par défaut appelle [InvalidateControl](#invalidatecontrol).  
  
##  <a name="a-nameonenumverbsa--colecontrolonenumverbs"></a><a name="onenumverbs"></a>COleControl::OnEnumVerbs  
 Appelé par l’infrastructure lorsque le conteneur appelle le **IOleObject::EnumVerbs** fonction membre.  
...
##  <a name="a-nameoneventadvisea--colecontroloneventadvise"></a><a name="oneventadvise"></a>COleControl::OnEventAdvise  
 Appelé par l’infrastructure lorsqu’un gestionnaire d’événements est connecté ou non à partir d’un contrôle OLE.  
...
### <a name="parameters"></a>Paramètres  
 `bAdvise`  
 **TRUE** indique qu’un gestionnaire d’événements a été connecté au contrôle. **FALSE** indique qu’un gestionnaire d’événements a été déconnecté à partir du contrôle.  
  
##  <a name="a-nameonfontchangeda--colecontrolonfontchanged"></a><a name="onfontchanged"></a>COleControl::OnFontChanged  
 Appelé par l’infrastructure lorsque la valeur de propriété stockée Font a été modifiée.  
...
##  <a name="a-nameonforecolorchangeda--colecontrolonforecolorchanged"></a><a name="onforecolorchanged"></a>COleControl::OnForeColorChanged  
 Appelé par l’infrastructure lorsque la valeur de propriété stockée ForeColor a été modifiée.  
...
### <a name="remarks"></a>Remarques  
 ...
 Remplacez cette fonction si vous souhaitez un comportement supplémentaire lorsque la gestion des événements sont figée ou non figées.  
  
##  <a name="a-nameongetcolorseta--colecontrolongetcolorset"></a><a name="ongetcolorset"></a>COleControl::OnGetColorSet  
 Appelé par l’infrastructure lorsque le conteneur appelle le **IViewObject::GetColorSet** fonction membre.  
...
### <a name="parameters"></a>Paramètres  
 `ptd`  
 Pointe vers le périphérique cible pour lequel l’image doit être restitué. Si cette valeur est **NULL**, l’image doit être restituée pour un appareil cible par défaut, généralement un périphérique d’affichage.  
...
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si un jeu de couleurs valides est retourné ; sinon 0.  
  
### <a name="remarks"></a>Remarques  
 Le conteneur appelle cette fonction pour obtenir toutes les couleurs nécessaires pour dessiner le contrôle OLE. Le conteneur peut utiliser les jeux de couleurs obtenus en conjonction avec les couleurs que nécessaires à la palette de couleurs globale. L’implémentation par défaut retourne **FALSE**.  
...
##  <a name="a-nameongetcontrolinfoa--colecontrolongetcontrolinfo"></a><a name="ongetcontrolinfo"></a>COleControl::OnGetControlInfo  
 Appelé par l’infrastructure lorsque le conteneur du contrôle a demandé des informations à propos du contrôle.  
...
### <a name="remarks"></a>Notes  
 Remplacez cette fonction si votre contrôle a une propriété dont la valeur ne peut pas être directement convertie en une chaîne et la valeur de propriété à afficher dans la fenêtre Propriétés de conteneur fourni.  
  
##  <a name="a-nameongetinplacemenua--colecontrolongetinplacemenu"></a><a name="ongetinplacemenu"></a>COleControl::OnGetInPlaceMenu  
 Appelé par l’infrastructure lorsque le contrôle est activée pour obtenir le menu à fusionner dans le menu du conteneur existant UI.  
...
### <a name="return-value"></a>Valeur de retour  
 Le handle de menu du contrôle, ou **NULL** si le contrôle n’a aucun. L’implémentation par défaut retourne **NULL**.  
...
### <a name="parameters"></a>Paramètres  
 `dwAspect`  
 Spécifie la représentation de l'objet. Représentations incluent le contenu, une icône, une miniature ou un document imprimé. Valeurs valides sont issues de l’énumération [DVASPECT](http://msdn.microsoft.com/library/windows/desktop/ms690318) ou **DVASPECT2**.  
 ...
 *lIndex Valeur de type*  
 ...
 `hicTargetDev`  
 Spécifie le contexte d’informations pour le périphérique cible indiqué par le `ptd` paramètre à partir duquel l’objet peut extraire les métriques du périphérique et tester les fonctions du périphérique. Si `ptd` est **NULL**, l’objet doit ignorer la valeur de le `hicTargetDev` paramètre.  
...
- **DVEXTENT_CONTENT** rechercher la taille du contrôle doit être pour qu’elle corresponde à contenu (composant logiciel enfichable à la taille)  
  
- **DVEXTENT_INTEGRAL** lors du redimensionnement, passer taille proposée au contrôle  
  
 `psizel`  
 Pointe vers les données retournées par le contrôle de dimensionnement. Les données de dimensionnement retourné a la valeur -1 pour n’importe quelle dimension qui n’est pas réglée.  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si elle retourne correctement ou que vous ajuste la taille ; sinon 0.  
  
### <a name="remarks"></a>Remarques  
 Remplacez cette fonction pour retourner la taille d’affichage de l’objet le plus proche pour le mode de taille et de mesure proposé dans le **DVEXTENTINFO** structure. L’implémentation par défaut retourne **FALSE** et ajuste pas à la taille.  
...
##  <a name="a-nameongetpredefinedstringsa--colecontrolongetpredefinedstrings"></a><a name="ongetpredefinedstrings"></a>COleControl::OnGetPredefinedStrings  
 Appelée par l’infrastructure pour obtenir un ensemble de chaînes prédéfinies qui représente les valeurs possibles pour une propriété.  
...
##  <a name="a-nameongetpredefinedvaluea--colecontrolongetpredefinedvalue"></a><a name="ongetpredefinedvalue"></a>COleControl::OnGetPredefinedValue  
 Appelé par l’infrastructure pour obtenir la valeur correspondant à l’une des chaînes prédéfinies précédemment retournés par une substitution de `COleControl::OnGetPredefinedStrings`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `lpvarOut`  
 Pointeur vers un **variante** de la structure dans laquelle une valeur de propriété sera retournée.  
...
### <a name="parameters"></a>Paramètres  
 *dwDrawAspect*  
 `DWORD`qui décrit quelle forme, ou aspect, d’un objet doit être affichée. Valeurs valides sont issues de l’énumération [DVASPECT](http://msdn.microsoft.com/library/windows/desktop/ms690318) ou **DVASPECT2**.  
...
### <a name="remarks"></a>Notes  
 Remplacez cette fonction si votre contrôle utilise deux passes dessin et de ses parties opaques et transparentes ont des dimensions différentes.  
...
- `DVASPECT_CONTENT`Rectangle englobant de l’objet entier. Coin supérieur gauche à l’origine et de taille égale à celui retourné par l’objet **GetViewExtent***.*  
  
- **DVASPECT_OPAQUE** objets avec une zone rectangulaire opaque retournent ce rectangle. D’autres échouent.  
  
- **DVASPECT_TRANSPARENT** Rectangle couvrant toutes les parties ou irréguliers.  
...
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si le rectangle en taille réelle de l’objet est retourné avec succès ; sinon 0.  
  
### <a name="remarks"></a>Remarques  
 Taille de l’objet est convertie en `OnGetViewRect` dans un rectangle commençant à une position spécifique (la valeur par défaut est le coin supérieur gauche de l’affichage). Remplacez cette fonction si votre contrôle utilise deux passes dessin et de ses parties opaques et transparentes ont des dimensions différentes.  
...
### <a name="return-value"></a>Valeur de retour  
 Une des valeurs de la **double** énumération en cas de réussite ; sinon, 0. Les valeurs possibles sont n’importe quelle combinaison des éléments suivants :  
  
 **VIEWSTATUS_OPAQUE**  
 L’objet est complètement opaque. Si ce bit n’est pas défini, l’objet contient des zones transparentes. Ce bit s’applique uniquement aux aspects liés au contenu et non `DVASPECT_ICON` ou `DVASPECT_DOCPRINT`.  
  
 **VIEWSTATUS_SOLIDBKGND**  
 Objet a un arrière-plan uni (composé dans une couleur unie, pas un motif de tracé). Ce bit est significatif uniquement si **VIEWSTATUS_OPAQUE** est défini et s’applique uniquement aux aspects liés au contenu et non `DVASPECT_ICON` ou `DVASPECT_DOCPRINT`.  
  
 **VIEWSTATUS_DVASPECTOPAQUE**  
 Prend en charge l’objet **DVASPECT_OPAQUE**. Tous les **IViewObjectEx** méthodes qui prennent un aspect de dessin comme un paramètre peut être appelé avec cet aspect.  
  
 **VIEWSTATUS_DVASPECTTRANSPARENT**  
 Prend en charge l’objet **DVASPECT_TRANSPARENT**. Tous les **IViewObjectEx** méthodes qui prennent un aspect de dessin comme un paramètre peut être appelé avec cet aspect.  
...
##  <a name="a-nameonhidetoolbarsa--colecontrolonhidetoolbars"></a><a name="onhidetoolbars"></a>COleControl::OnHideToolBars  
 Appelé par l’infrastructure lorsque le contrôle est l’interface utilisateur désactivée.  
...
##  <a name="a-nameoninactivemousemovea--colecontroloninactivemousemove"></a><a name="oninactivemousemove"></a>COleControl::OnInactiveMouseMove  
 Appelée par le conteneur pour l’objet sous le pointeur de la souris sur la réception du inactive une `WM_MOUSEMOVE` message.  
 ...
 *x*  
 Coordonnée x de l’emplacement de la souris dans les coordonnées clientes de la fenêtre.  
  
 *y*  
 Coordonnée y de l’emplacement de la souris dans les coordonnées clientes de la fenêtre.  
  
 `dwKeyState`  
 Identifie l’état actuel des touches de modification sur le raccourci clavier. Valeurs valides peuvent être une combinaison des indicateurs de **MK_CONTROL**, **MK_SHIFT**, **MK_ALT**, **MK_BUTTON**, **MK_LBUTTON**, **MK_MBUTTON**, et **MK_RBUTTON**.  
  
### <a name="remarks"></a>Remarques  
 Notez que les coordonnées clientes de fenêtre (pixels) sont utilisées pour passer de la position du curseur de la souris. Ceci est rendu possible en passant également le rectangle englobant de l’objet dans le même système de coordonnées.  
...
##  <a name="a-nameoninactivesetcursora--colecontroloninactivesetcursor"></a><a name="oninactivesetcursor"></a>COleControl::OnInactiveSetCursor  
 Appelée par le conteneur pour l’objet sous le pointeur de la souris sur la réception du inactive une `WM_SETCURSOR` message.  
...
### <a name="parameters"></a>Paramètres  
 `nChar`  
 La valeur de code de touche virtuelle de la touche enfoncée. Pour une liste des codes de touches virtuels standards, consultez Winuser.h  
...
##  <a name="a-nameonkeypresseventa--colecontrolonkeypressevent"></a><a name="onkeypressevent"></a>COleControl::OnKeyPressEvent  
 Appelé par l’infrastructure après le stock KeyPress (événement) a été déclenché.  
...
### <a name="parameters"></a>Paramètres  
 `nChar`  
 Contient la valeur de code de touche virtuelle de la touche enfoncée. Pour une liste des codes de touches virtuels standards, consultez Winuser.h  
  
### <a name="remarks"></a>Notes  
 Notez que le `nChar` valeur a été modifiée par le conteneur.  
 ...
 *pbPageOptional*  
 Retourne un indicateur si l’utilisation de la page de propriété spécifiée est facultatif.  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si un ID de classe a été renvoyé dans `lpclsid`; sinon, 0.  
...
##  <a name="a-nameonmnemonica--colecontrolonmnemonic"></a><a name="onmnemonic"></a>COleControl::OnMnemonic  
 Appelé par l’infrastructure lorsque le conteneur a détecté qu’une touche mnémonique du contrôle OLE a été enfoncée.  
...
##  <a name="a-nameonpropertiesa--colecontrolonproperties"></a><a name="onproperties"></a>COleControl::OnProperties  
 Appelé par l’infrastructure lorsque le verbe de propriétés du contrôle a été appelée par le conteneur.  
...
### <a name="remarks"></a>Remarques  
 ...
 Vous pouvez également utiliser cette fonction pour provoquer l’affichage des pages de propriétés de votre contrôle. Effectuer un appel à la `OnProperties` fonction, en passant le handle du parent du contrôle dans le `hWndParent` paramètre. Dans ce cas, les valeurs de la `lpMsg` et `lpRect` sont ignorés.  
...
### <a name="parameters"></a>Paramètres  
 `dwAspect`  
 Spécifie la façon dont l’objet est représenté. Valeurs valides sont issues de l’énumération [DVASPECT](http://msdn.microsoft.com/library/windows/desktop/ms690318) ou **DVASPECT2**.  
  
 `pRectBounds`  
 Pointeur vers un `RECT` structure qui spécifie le rectangle englobant de la zone cliente du contrôle OLE.  
  
 `ptlLoc`  
 Pointeur vers le **POINT** structure qui spécifie le point à vérifier pour effectuer un test. Le point est spécifié dans les coordonnées de la zone client OLE.  
...
- **HITRESULT_OUTSIDE** `ptlLoc` est en dehors de l’OLE de l’objet et le ne ferme pas.  
  
- **HITRESULT_TRANSPARENT** *ptlLoc* se trouve dans les limites de l’objet OLE, mais pas proche de l’image. Par exemple, un point au milieu d’un cercle transparent peut être **HITRESULT_TRANSPARENT**.  
  
- **HITRESULT_CLOSE** `ptlLoc` est à l’intérieur ou à l’extérieur de l’objet OLE, mais suffisamment près à l’objet considéré à l’intérieur. Objets petits, légers ou détaillées peuvent utiliser cette valeur. Même si un point est à l’extérieur du rectangle englobant le rectangle d’un objet, il peut encore être proche (cela est nécessaire pour atteindre les petits objets).  
  
- **HITRESULT_HIT** `ptlLoc` se trouve dans l’image de l’objet.  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si un résultat de recherche est retourné avec succès ; sinon 0. Un accès est un chevauchement avec la zone d’affichage de contrôle OLE.  
  
### <a name="remarks"></a>Remarques  
 Interroge si le rectangle d’affichage d’un objet chevauche du point donné (atteint le point). `QueryHitPoint`peut être substituée pour tester l’accès pour les objets non rectangulaires.  
...
### <a name="parameters"></a>Paramètres  
 `dwAspect`  
 Spécifie la représentation de l'objet. Valeurs valides sont issues de l’énumération [DVASPECT](http://msdn.microsoft.com/library/windows/desktop/ms690318) ou **DVASPECT2**.  
...
### <a name="remarks"></a>Remarques  
 Interroge si le rectangle d’affichage d’un objet chevauche n’importe quel point dans le rectangle donné (atteint le rectangle). `QueryHitRect`peut être substituée pour tester l’accès pour les objets non rectangulaires.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `lpStgMedium`  
 Pointe vers une [STGMEDIUM](http://msdn.microsoft.com/library/windows/desktop/ms683812) structure dans laquelle les données doit être retournée.  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Notes  
 Le format spécifié est un été placé dans l’objet de contrôle à l’aide de la [DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata) ou [DelayRenderFileData](../../mfc/reference/coledatasource-class.md#delayrenderfiledata) les fonctions membres pour le rendu retardé. L’implémentation par défaut de cette fonction appelle `OnRenderFileData` ou `OnRenderGlobalData`, respectivement, si le support de stockage fourni est un fichier ou la mémoire. Si le format demandé est `CF_METAFILEPICT` ou format de jeu de la propriété persistante, l’implémentation par défaut restitue les données appropriées et retourne zéro. Sinon, elle retourne 0 et n’a aucun effet.  
...
##  <a name="a-nameonrenderfiledataa--colecontrolonrenderfiledata"></a><a name="onrenderfiledata"></a>COleControl::OnRenderFileData  
 Appelée par l’infrastructure pour récupérer des données dans le format spécifié lorsque le support de stockage est un fichier.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `pFile`  
 Pointe vers une [CFile](../../mfc/reference/cfile-class.md) objet dans lequel les données doivent être rendus.  
  
### <a name="return-value"></a>Valeur de retour  
 Valeur différente de zéro cas de réussite ; sinon, 0.  
  
### <a name="remarks"></a>Remarques  
 Le format spécifié est un été placé dans l’objet de contrôle à l’aide de la [DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata) fonction membre pour le rendu retardé. L’implémentation par défaut de cette fonction renvoie simplement **FALSE**.  
  
 Remplacez cette fonction pour fournir vos données dans le format demandé et le support. En fonction de vos données, vous souhaiterez substituer l’une des autres versions de cette fonction à la place. Si vous voulez gérer plusieurs supports de stockage, remplacez `OnRenderData`. Si vos données sont dans un fichier, ou de taille variable, remplacer `OnRenderFileData`.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `phGlobal`  
 Pointe vers un handle de mémoire globale dans laquelle les données doit être retournée. Si aucune mémoire n’a été alloué, ce paramètre peut être **NULL**.  
...
##  <a name="a-nameonsetclientsitea--colecontrolonsetclientsite"></a><a name="onsetclientsite"></a>COleControl::OnSetClientSite  
 Appelé par l’infrastructure lorsque le conteneur a appelé du contrôle **IOleControl::SetClientSite** (fonction).  
...
### <a name="remarks"></a>Notes  
 Si les données sont définies dans la propriété persistent le format, l’implémentation par défaut modifie l’état du contrôle en conséquence. Dans le cas contraire, l’implémentation par défaut ne fait rien. Si `bRelease` est **TRUE**, puis un appel à **ReleaseStgMedium** est effectuée ; sinon non.  
...
##  <a name="a-nameonsetextenta--colecontrolonsetextent"></a><a name="onsetextent"></a>COleControl::OnSetExtent  
 Appelé par l’infrastructure lorsque l’étendue du contrôle doit être modifiée, à la suite d’un appel à [IOleObject::SetExtent](http://msdn.microsoft.com/library/windows/desktop/ms694330).  
...
### <a name="remarks"></a>Remarques  
 L’implémentation par défaut gère le redimensionnement de l’étendue du contrôle. Si le contrôle est actif sur place, un appel à du conteneur **OnPosRectChanged** est alors effectuée.  
...
### <a name="parameters"></a>Paramètres  
 *lpRectPos*  
 Pointeur vers une structure RECT indiquant la nouvelle position et la taille relatifs au conteneur du contrôle.  
...
##  <a name="a-nameonshowtoolbarsa--colecontrolonshowtoolbars"></a><a name="onshowtoolbars"></a>COleControl::OnShowToolBars  
 Appelé par l’infrastructure lorsque le contrôle a été activée UI.  
...
##  <a name="a-nameontextchangeda--colecontrolontextchanged"></a><a name="ontextchanged"></a>COleControl::OnTextChanged  
 Appelé par l’infrastructure lorsque la valeur de propriété stockée Caption ou Text a été modifiée.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `bHitTest`  
 Spécifie si le test de positionnement est sur le point.  
  
### <a name="return-value"></a>Valeur de retour  
 Si `bHitTest` est **FALSE**, retourne **HTNOWHERE**. Si `bHitTest` est **TRUE**est un de la souris en suivante les valeurs de test de positionnement et renvoie l’emplacement dans lequel le parent (conteneur) point atterri dans la zone cliente du contrôle OLE :  
...
- **HTERROR** sur l’arrière-plan de l’écran ou sur une ligne de démarcation entre windows (même en tant que **HTNOWHERE** , sauf que les **DefWndProc** fonction Windows génère un signal sonore pour indiquer une erreur).  
...
- **HTZOOM** de maximiser le bouton.  
  
### <a name="remarks"></a>Remarques  
 Entrée `pPoint` est relatif à l’origine du parent (coin supérieur gauche du conteneur). Sortie `pPoint` est relatif à l’origine de la zone cliente du contrôle OLE (coin supérieur gauche de la zone cliente du contrôle).  
  
##  <a name="a-namepostmodaldialoga--colecontrolpostmodaldialog"></a><a name="postmodaldialog"></a>COleControl::PostModalDialog  
 Notifie au conteneur qu’une boîte de dialogue modale a été fermée.  
...
##  <a name="a-namepremodaldialoga--colecontrolpremodaldialog"></a><a name="premodaldialog"></a>COleControl::PreModalDialog  
 Notifie au conteneur qu’une boîte de dialogue modale est sur le point d’être affiché.  
...
##  <a name="a-nameresetstockpropsa--colecontrolresetstockprops"></a><a name="resetstockprops"></a>COleControl::ResetStockProps  
 Initialise l’état de la `COleControl` le stock de propriétés à leurs valeurs par défaut.  
...
### <a name="remarks"></a>Notes  
 ...
 Vous pouvez améliorer les performances de l’initialisation binaire d’un contrôle à l’aide de `ResetStockProps` et `ResetVersion` de remplacement `COleControl::OnResetState`. Lisez l'exemple ci-dessous. Pour plus d’informations sur l’optimisation de l’initialisation, consultez [contrôles ActiveX : optimisation](../../mfc/mfc-activex-controls-optimization.md).  
...
### <a name="remarks"></a>Notes  
 Vous pouvez améliorer les performances de l’initialisation binaire d’un contrôle à l’aide de `ResetVersion` et `ResetStockProps` de remplacement `COleControl::OnResetState`. Consultez l’exemple [ResetStockProps](#resetstockprops). Pour plus d’informations sur l’optimisation de l’initialisation, consultez [contrôles ActiveX : optimisation](../../mfc/mfc-activex-controls-optimization.md).  
  
##  <a name="a-namescrollwindowa--colecontrolscrollwindow"></a><a name="scrollwindow"></a>COleControl::ScrollWindow  
 Permet à un objet OLE sans fenêtre faire défiler une zone au sein de son image active sur place à l’écran.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `lpRect`  
 Pointe vers une [CRect](../../atl-mfc-shared/reference/crect-class.md) objet ou une structure RECT qui spécifie la partie de la zone cliente de l’objet OLE à faire défiler, en coordonnées clientes de la fenêtre. Si `lpRect` est **NULL**, défile de la zone cliente de l’objet OLE entier.  
  
 `lpClipRect`  
 Pointe vers une `CRect` objet ou `RECT` structure qui spécifie le rectangle de découpage pour. Seuls les pixels à l’intérieur du rectangle défile. Bits à l’extérieur du rectangle ne sont pas affectées même si elles se trouvent dans le `lpRect` rectangle. Si `lpClipRect` est **NULL**, aucun découpage n’est effectuée sur le rectangle de défilement.  
...
### <a name="return-value"></a>Valeur de retour  
 Un pointeur vers le sélectionné précédemment `CFont` objet. Vous devez utiliser [CDC::SelectObject](../../mfc/reference/cdc-class.md#selectobject) de sélectionner cette police dans le contexte de périphérique lorsque vous avez terminé.  
...
##  <a name="a-nameserializestockpropsa--colecontrolserializestockprops"></a><a name="serializestockprops"></a>COleControl::SerializeStockProps  
 Sérialise ou initialise l’état de le `COleControl` propriétés stock : apparence, BackColor, BorderStyle, légende, Enabled, police, couleur de premier plan et texte.  
...
### <a name="parameters"></a>Paramètres  
 ...
 `bConvert`  
 Indique si les données persistantes doivent être converties au format plus récent lorsqu’il est enregistré ou mis à jour dans le même format qu’il avait lorsqu’il a été chargé.  
  
### <a name="return-value"></a>Valeur de retour  
 Le numéro de version du contrôle. Si la charge de l’archive spécifiée, `SerializeVersion` renvoie la version chargée à partir de cette archive. Sinon, elle retourne la version actuellement chargée.  
...
### <a name="parameters"></a>Paramètres  
 *sAppearance*  
 A **court** ( `VT_I2`) la valeur à utiliser pour l’apparence de votre contrôle. La valeur zéro définit l’apparence du contrôle à plat et la valeur 1 définit l’apparence du contrôle en 3D.  
...
##  <a name="a-namesetbackcolora--colecontrolsetbackcolor"></a><a name="setbackcolor"></a>COleControl::SetBackColor  
 Définit la valeur de propriété stockée BackColor de votre contrôle.  
...
### <a name="parameters"></a>Paramètres  
 *dwBackColor*  
 Un **OLE_COLOR** valeur à utiliser pour l’arrière-plan du contrôle de dessin.  
...
##  <a name="a-namesetborderstylea--colecontrolsetborderstyle"></a><a name="setborderstyle"></a>COleControl::SetBorderStyle  
 Définit la valeur de propriété stockée BorderStyle de votre contrôle.  
...
### <a name="remarks"></a>Notes  
 La fenêtre de contrôle sera ensuite possible de re-création et `OnBorderStyleChanged` appelée.  
  
##  <a name="a-namesetcapturea--colecontrolsetcapture"></a><a name="setcapture"></a>COleControl::SetCapture  
 Provoque la fenêtre du contrôle conteneur à prendre possession de la capture de souris au nom du contrôle.  
...
### <a name="return-value"></a>Valeur de retour  
 Un pointeur vers le **CWnd** objet fenêtre précédemment reçu entrée de la souris.  
  
### <a name="remarks"></a>Remarques  
 Si le contrôle est activé et sans fenêtre, cette fonction provoque la fenêtre du contrôle conteneur à prendre possession de la capture de la souris, au nom du contrôle. Dans le cas contraire, cette fonction provoque le contrôle proprement dit pour prendre possession de la capture de la souris (même en tant que `CWnd::SetCapture`).  
  
##  <a name="a-namesetcontrolsizea--colecontrolsetcontrolsize"></a><a name="setcontrolsize"></a>COleControl::SetControlSize  
 Définit la taille de la fenêtre de contrôle OLE et notifie au conteneur que le site du contrôle change.  
...
##  <a name="a-namesetenableda--colecontrolsetenabled"></a><a name="setenabled"></a>COleControl::SetEnabled  
 Définit la valeur de la propriété Enabled du contrôle du stock.  
...
##  <a name="a-namesetfocusa--colecontrolsetfocus"></a><a name="setfocus"></a>COleControl::SetFocus  
 Provoque la fenêtre du contrôle conteneur à prendre possession du focus d’entrée au nom du contrôle.  
...
### <a name="remarks"></a>Remarques  
 Si le contrôle est activé et sans fenêtre, cette fonction provoque la fenêtre du contrôle conteneur à prendre possession du focus d’entrée, au nom du contrôle. Le focus d’entrée dirige l’entrée au clavier pour la fenêtre du conteneur et le conteneur envoie tous les messages de clavier suivants pour l’objet OLE qui appelle `SetFocus`. Les fenêtres qui avaient le focus d’entrée le perd.  
  
 Si le contrôle n’est pas sans fenêtre, cette fonction provoque le contrôle lui-même pour s’approprier le focus d’entrée (identique à `CWnd::SetFocus`).  
  
##  <a name="a-namesetfonta--colecontrolsetfont"></a><a name="setfont"></a>COleControl::SetFont  
 Définit la propriété stock Font du contrôle.  
...
##  <a name="a-namesetforecolora--colecontrolsetforecolor"></a><a name="setforecolor"></a>COleControl::SetForeColor  
 Définit la valeur de propriété stockée couleur de premier plan du contrôle.  
...
### <a name="parameters"></a>Paramètres  
 *dwForeColor*  
 Un **OLE_COLOR** valeur à utiliser pour le premier plan du contrôle de dessin.  
...
##  <a name="a-namesetinitialdataformatsa--colecontrolsetinitialdataformats"></a><a name="setinitialdataformats"></a>COleControl::SetInitialDataFormats  
 Appelée par l’infrastructure pour initialiser la liste des formats de données pris en charge par le contrôle.  
...
### <a name="remarks"></a>Notes  
 Appelez cette fonction dans votre constructeur pour définir la taille initiale de votre contrôle. La taille initiale est mesurée en unités de périphérique, ou en pixels. Il est recommandé que cet appel soit effectué dans le constructeur de votre contrôle.  
...
### <a name="parameters"></a>Paramètres  
 `bModified`  
 La nouvelle valeur pour le contrôle de modification indicateur. **TRUE** indique que l’état du contrôle a été modifié ; **FALSE** indique que l’état du contrôle a simplement été enregistrée.  
  
### <a name="remarks"></a>Remarques  
 Appel de cette fonction chaque fois qu’une modification produit qui affecterait l’état permanent de votre contrôle. Par exemple, si la valeur d’une propriété persistante change, appelez cette fonction avec `bModified` **TRUE**.  
...
### <a name="remarks"></a>Remarques  
 Appelez cette fonction lorsque `BoundPropertyRequestEdit` échoue. Cette fonction génère une exception de type **COleDispScodeException** pour indiquer que l’opération de définition n’était pas autorisée.  
  
##  <a name="a-namesetnotsupporteda--colecontrolsetnotsupported"></a><a name="setnotsupported"></a>Courants  
...
### <a name="remarks"></a>Remarques  
 Appelez cette fonction à la place de la fonction Set d’une propriété où la modification de la valeur de propriété par l’utilisateur du contrôle n’est pas pris en charge. Un exemple serait une propriété qui est en lecture seule.  
...
##  <a name="a-namesetrectincontainera--colecontrolsetrectincontainer"></a><a name="setrectincontainer"></a>COleControl::SetRectInContainer  
 Définit les coordonnées du rectangle du contrôle relatif du conteneur, exprimé en unités de périphérique.  
...
### <a name="parameters"></a>Paramètres  
 `lpRect`  
 Pointeur vers un rectangle contenant les nouvelles coordonnées du contrôle relatif du conteneur.  
  
### <a name="return-value"></a>Valeur de retour  
 Différent de zéro si l’appel a réussi ; sinon 0.  
  
### <a name="remarks"></a>Notes  
 Si le contrôle est ouvert, il est redimensionné ; dans le cas contraire du conteneur **OnPosRectChanged** fonction est appelée.  
  
##  <a name="a-namesettexta--colecontrolsettext"></a><a name="settext"></a>COleControl::SetText  
 Définit la valeur de la propriété stock de légende ou le texte de votre contrôle.  
...
### <a name="remarks"></a>Remarques  
 Notez que les propriétés stock de légende et le texte sont mappées à la même valeur. Cela signifie que toute modification apportée à des propriétés change automatiquement les deux propriétés. En général, un contrôle doit prendre en charge le stock Caption ou Text (propriété), mais pas les deux.  
...
### <a name="parameters"></a>Paramètres  
 `sc`  
 La valeur de code d’état doit être signalée. Pour obtenir une liste complète des codes de possibles, consultez l’article [contrôles ActiveX : rubriques avancées](../../mfc/mfc-activex-controls-advanced-topics.md).  
 ...
 `pszDescription`  
 Chaîne contenant une description de l’exception doit être signalée.  
  
### <a name="remarks"></a>Remarques  
 Cette fonction doit uniquement être appelée à partir d’une fonction Get ou Set pour une propriété OLE, ou de l’implémentation d’une méthode d’automatisation OLE. Si vous avez besoin de signaler les erreurs qui se produisent à d’autres moments, vous devez déclencher l’événement d’erreur stock.  
  
##  <a name="a-nametransformcoordsa--colecontroltransformcoords"></a><a name="transformcoords"></a>COleControl::TransformCoords  
 Transformations de coordonnent les valeurs comprises entre **HIMETRIC** unités et les unités native du conteneur.  
...
- **XFORMCOORDS_HIMETRICTOCONTAINER** transformation **HIMETRIC** unités à des unités du conteneur.  
  
- **XFORMCOORDS_CONTAINERTOHIMETRIC** transformer unités du conteneur à **HIMETRIC** unités.  
  
### <a name="remarks"></a>Remarques  
 Les deux premiers indicateurs, **XFORMCOORDS_POSITION** et **XFORMCOORDS_SIZE**, indiquer si les coordonnées doivent être traitées comme une position ou une taille. Les deux indicateurs restantes indiquent le sens de la transformation.  
  
##  <a name="a-nametranslatecolora--colecontroltranslatecolor"></a><a name="translatecolor"></a>Fonction membre COleControl::TranslateColor  
...
### <a name="parameters"></a>Paramètres  
 `clrColor`  
 A **OLE_COLOR** type de données. Pour plus d’informations, consultez Windows [OleTranslateColor](http://msdn.microsoft.com/library/windows/desktop/ms694353) (fonction).  
  
 `hpal`  
 Un handle vers une palette facultatif ; peut être **NULL**.  
  
### <a name="return-value"></a>Valeur de retour  
 Une valeur de 32 bits couleur RVB (rouge, vert, bleu) qui définit le solide de couleur le plus proche de la `clrColor` valeur représentant l’appareil.  
...
##  <a name="a-namewillambientsbevalidduringloada--colecontrolwillambientsbevalidduringload"></a><a name="willambientsbevalidduringload"></a>COleControl::WillAmbientsBeValidDuringLoad  
 Détermine si votre contrôle doit utiliser les valeurs de propriété ambiante en tant que valeurs par défaut, lorsqu’il est chargé par la suite de son état persistant.  
...
### <a name="return-value"></a>Valeur de retour  
 NonZero indique que les propriétés ambiantes est valides ; Sinon, les propriétés ambiantes ne sera pas valides.  
  
### <a name="remarks"></a>Notes  
 Dans certains conteneurs, votre contrôle aient pas accès à ses propriétés ambiantes lors de l’appel initial à la substitution de `COleControl::DoPropExchange`. C’est le cas si le conteneur appelle [IPersistStreamInit::Load](http://msdn.microsoft.com/library/windows/desktop/ms680730) ou [IPersistStorage::Load](http://msdn.microsoft.com/library/windows/desktop/ms680557) avant d’appeler [IOleObject::SetClientSite](http://msdn.microsoft.com/library/windows/desktop/ms684013) (autrement dit, s’il ne respecte pas la **OLEMISC_SETCLIENTSITEFIRST** bit d’état).  
...
### <a name="remarks"></a>Remarques  
 Appelez cette fonction pour distribuer des messages spécifiques à la table des messages du contrôle.  
