...
Les outils Visual C++ pour CMake est installé par défaut dans le cadre du développement de bureau avec une charge de travail de C++.
...
1. Sélectionnez la cible dans le menu déroulant de débogage et appuyez sur **F5** ou cliquez sur le bouton « Exécuter ». Le projet sera automatiquement build tout d’abord, tout comme avec les solutions Visual Studio.
2.  Cliquez avec le bouton droit sur le CMakeLists.txt et sélectionnez la Build dans le menu contextuel. Si vous avez plusieurs cibles dans votre structure de dossiers, vous pouvez choisir de générer toutes les ou qu’une seule cible spécifique, ou
  
3. Dans le menu principal, sélectionnez **générer | Générez la Solution** (**F7** ou **Ctrl + Maj + B**) (Assurez-vous qu’une cible de CMake est déjà sélectionnée dans le **élément de démarrage** liste déroulante dans le **Générales** barre d’outils).

![Commande de menu de génération de CMake](media/cmake-build-menu.png) 
...
![Erreurs de build CMake](media/cmake-build-errors.png)

Dans un dossier avec plusieurs cibles de génération, vous pouvez choisir l’élément de Build sur le menu de CMake ou le menu contextuel de CMakeLists.txt pour spécifier quelle cible CMake à générer. En appuyant sur **Ctrl + Maj + B** dans un CMake projet est généré dans le document actif. 

## <a name="debugging-the-project"></a>Débogage du projet
Pour déboguer un projet de CMake, choisissez la configuration souhaitée et appuyez sur **F5**, ou appuyez sur le bouton exécuter dans la barre d’outils. Si le bouton Exécuter indique « Sélectionner un élément démarrage », cliquez sur la flèche vers le bas et choisissez la cible que vous souhaitez exécuter. (Dans un projet de CMake, le « document actif » option est uniquement valide pour les fichiers .cpp.) 

 ![Bouton d’exécution de CMake](media/cmake-run-button.png)

 En appuyant sur **F5** sera première génération du projet si les modifications ont été apportées depuis la dernière build.

## <a name="configuring-cmake-debugging-sessions"></a>Configuration des sessions de débogage de CMake
Toutes les cibles de CMake exécutables sont affichés dans la liste déroulante d’élément de démarrage dans la barre d’outils général. Pour démarrer une session de débogage, sélectionnez une et lancer le débogueur.

 ![Liste déroulante élément de démarrage de CMake](media/cmake-startup-item-dropdown.png)
...
```
Dès que vous enregistrez le fichier launch.vs.json, une entrée est créée dans la liste déroulante d’élément de démarrage avec le nouveau nom. En modifiant le fichier launch.vs.json, vous pouvez créer que de nombreuses configurations de débogage que vous le souhaitez pour n’importe quel nombre de cibles de CMake.

**Visual Studio 2017 version 15.4**: Launch.vs.json prend en charge les variables qui sont déclarées dans CMakeSettings.json (voir ci-dessous). Elle comporte également un concept de « currentDir », qui sera défini en tant que le répertoire actuel de l’application de lancement :
...
```
Lorsque vous exécutez l’application, la valeur de `currentDir` est 
...
Pour modifier un fichier CMakeLists.txt, avec le bouton droit sur le fichier dans **l’Explorateur de solutions** et choisissez **ouvrir**. Si vous apportez des modifications au fichier, une barre d’état jaune s’affiche et vous informe que IntelliSense mettra à jour et vous donne la possibilité d’annuler l’opération de mise à jour. Pour plus d’informations sur CMakeLists.txt, consultez la [CMake documentation](https://cmake.org/documentation/).
  ...
  ![Modification de fichiers CMakeLists.txt](media/cmake-cmakelists.png)

Dès que vous enregistrez le fichier, l’étape de configuration sera automatiquement exécuter à nouveau et afficher des informations dans la fenêtre Sortie. Erreurs et avertissements sont affichés dans la liste d’erreurs ou la fenêtre Sortie. Double-cliquez sur une erreur dans la liste d’erreurs pour accéder à la ligne concernée dans CMakeLists.txt.

  ![Erreurs de fichier CMakeLists.txt](media/cmake-cmakelists-error.png)
 
## <a name="cmake_settings"></a>Paramètres de CMake et les configurations personnalisées
Par défaut, Visual Studio fournit six configurations de CMake par défaut (« x86-Debug », « x86-version », « x64-Debug », « x64-version finale », « Linux-Debug » et « Linux-version »). Ces configurations définissent la façon dont CMake.exe est appelée pour créer le cache de CMake pour un projet donné. Pour modifier ces configurations, ou créer une configuration personnalisée, choisissez **CMake | Modifier le paramètre de CMake**, puis choisissez le fichier CMakeLists.txt qui les paramètres seront appliquent à. Modifier les paramètres CMake est également disponible sur le menu contextuel du fichier **l’Explorateur de solutions**. Cette commande crée un fichier CMakeSettings.json dans le dossier du projet. Ce fichier est utilisé pour recréer le fichier de cache CMake, par exemple après une opération « Propre ». 
...
JSON IntelliSense vous permet de modifier le fichier CMakeSettings.json :
   ...
   ![JSON de CMake IntelliSense](media/cmake-json-intellisense.png)
...
```
1.  nom : le nom qui apparaîtra dans la liste déroulante de configuration C++. Cette valeur de propriété peut également être utilisée comme une macro `${name}` pour spécifier d’autres valeurs de propriété. Pour obtenir un exemple, consultez la définition de « buildRoot » dans CMakeSettings.json.
2.  Générateur : correspond au commutateur -G et spécifie le Générateur à utiliser. Cette propriété peut également être utilisée comme une macro `${generator}` pour aider à spécifier d’autres valeurs de propriété. Visual Studio prend actuellement en charge les générateurs de CMake suivantes :
    - « Ninja »
...
3.  buildRoot : correspond à - commutateur DCMAKE_BINARY_DIR et indique où le cache de CMake sera créé. Si le dossier n’existe pas, il sera créé.
4.  variables : contient une paire nom + valeur des variables de CMake seront passés en tant que - Dname = valeur à CMake. Si vos instructions de génération de projet CMake spécifient de l’ajout des variables directement dans le fichier de cache de CMake, il est recommandé de les ajouter ici à la place.
5.  cmakeCommandArgs : Spécifie les commutateurs supplémentaires que vous souhaitez passer à CMake.exe.
6.  type de configuration : définit le type de configuration de build pour le Générateur de sélectionné. Valeurs actuellement prises en charge sont « Debug », « MinSizeRel », « Release » et « RelWithDebInfo ».
...
|   j - N     | exécuter des tâches N en parallèle [par défaut = 14, dérivée de processeurs disponibles]| 
|   k - N     | Poursuivez la lecture jusqu'à ce que N tâches échouent [par défaut = 1]| 
...
CmakeSettings.json prend désormais en charge les environnements d’inherted. Cette fonctionnalité vous permet de créer une variable personnalisée qui :

```json
  "inheritEnvironments": [ "msvc_x64_x64" ]
```

Ce champ définit les variables qui sont automatiquement transmis à CMake.exe lorsqu’il s’exécute. L’exemple ci-dessus est identique à le « Invite de commandes développeur pour VS 2017 » en cours d’exécution avec le `-arch=amd64 -host_arch=amd64` arguments.

Le tableau suivant montre les valeurs prises en charge et leurs équivalents de ligne de commande :
...
Lorsque plusieurs projets CMake utilisent le même nom de configuration de CMake (par exemple, x86-Debug), tous les sont configurés et générés (dans son propre dossier racine de la build) lorsque cette configuration est sélectionnée. Vous pouvez déboguer les cibles à partir de tous les projets de CMake qui participent à cette configuration de CMake.
   ...
   ![Élément de menu CMake générer uniquement](media/cmake-build-only.png)

Pour limiter les builds et les sessions à un sous-ensemble des projets dans l’espace de travail de débogage, créer une configuration avec un nom unique dans le fichier CMakeSettings.json et l’appliquer à ces projets uniquement. Lorsque cette configuration est sélectionnée, IntelliSense, création, et le débogage sera activé uniquement ceux spécifiée pour les projets.

## <a name="troubleshooting-cmake-cache-errors"></a>Dépannage des erreurs de cache CMake
Si vous avez besoin de plus d’informations sur l’état du cache CMake pour diagnostiquer un problème, ouvrez le menu principal de CMake ou le menu contextuel CMakeLists.txt **l’Explorateur de solutions** pour exécuter une des commandes suivantes :
