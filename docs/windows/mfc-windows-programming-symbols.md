...
Ces symboles font également partie des fichiers de prise en charge MFC, mais ils prennent en charge les fonctions et les actions standard des applications Windows.  Ces symboles sont utilisés principalement avec les menus.  Lorsque vous utilisez des menus dans l'Éditeur de menus, ces symboles s'affichent dans la fenêtre Propriétés associée aux commandes de menu courantes.  Par exemple, si votre application contient un menu Fichier avec une commande Quitter, cette commande est associée au symbole ID\_APP\_EXIT dans la [fenêtre Propriétés](../Topic/Properties%20Window.md).  
