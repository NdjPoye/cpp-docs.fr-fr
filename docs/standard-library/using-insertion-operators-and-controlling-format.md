...
```  
 ...
 L'indicateur d'alignement à gauche est défini à l'aide du manipulateur [setiosflags](../Topic/setiosflags.md) avec l'énumérateur [left](../Topic/left.md).  Cet énumérateur est défini dans la classe [ios](../Topic/ios.md). Sa référence doit donc inclure le préfixe **ios::**.  Le manipulateur [resetiosflags](../Topic/resetiosflags.md) désactive l'indicateur d'alignement à gauche.  Contrairement à **width** et `setw`, l'effet de `setiosflags` et `resetiosflags` est permanent.  
  
##  <a name="vclrfprecisionanchor5"></a> Précision  
 La valeur par défaut pour la précision de virgule flottante est six.  Par exemple, le nombre 3466,9768 est imprimé comme 3466,98.  Pour modifier comment cette valeur est imprimée, utilisez le manipulateur [setprecision](../Topic/setprecision.md).  Le manipulateur a deux indicateurs : [fixed](../Topic/fixed.md) et [scientific](../Topic/scientific.md).  Si [fixe](../Topic/fixed.md) est défini, le nombre imprimé est 3466,976800.  Si **scientific**est défini, le nombre imprimé est 3,4669773\+003.  
...
```  
 ...
 Si vous remplacez l'indicateur **ios::fixed** par **ios::scientific**, le programme imprime ceci :  
...
```  
 ...
 Les délimiteurs sont présents uniquement dans l'objet de flux. Ils ne sont pas présents dans la chaîne extraite, mais ils le sont dans la chaîne retournée par [basic\_stringstream::str](../Topic/basic_stringstream::str.md)\(\).  
 ...
 Pour plus d'informations et pour obtenir des exemples de code complets, consultez [quoted](../Topic/quoted.md).  
