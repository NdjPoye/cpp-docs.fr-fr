...
-   Le nom de fichier ou le chemin d’accès spécifié sur la ligne de commande ou dans la directive #pragma lib est incorrecte, ou le chemin d’accès a une spécification de lecteur non valide. Vérifiez l’orthographe et que le fichier existe à l’emplacement spécifié.  
