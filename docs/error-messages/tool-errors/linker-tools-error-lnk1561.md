...
-   Avoir défini le point d’entrée à l’aide de la signature de fonction incorrect ; par exemple, vous pouvez avoir mal orthographié ou utilisé la casse correcte pour le nom de fonction ou spécifié de manière incorrecte le type de retour ou les types de paramètre.  
