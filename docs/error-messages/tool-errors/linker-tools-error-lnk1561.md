...
L’éditeur de liens n’a pas trouvé un point d’entrée. Cette erreur peut avoir plusieurs causes :  
...
-   Avoir défini le point d’entrée à l’aide de la signature de fonction incorrect ; par exemple, vous pouvez ont mal orthographié ou utilisé la casse correcte pour le nom de fonction ou spécifié de manière incorrecte le type de retour ou les types de paramètre. Par défaut, l’éditeur de liens recherche un `main` ou `wmain` fonction d’une application console, un `WinMain` ou `wWinMain` fonction pour une application Windows, ou `DllMain` pour une DLL qui nécessite l’initialisation.  
