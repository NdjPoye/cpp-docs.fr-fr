...
-   Réorganisez les déclarations des membres par taille, de la plus grande à la plus petite.  Dans l'extrait de code, inversez l'ordre des membres de structure de sorte que le membre `long long` précède `int`, et que `int` précède `char`.  
 ...
 Vous pouvez utiliser [\#pragma pack](../../preprocessor/pack.md) ou [\/Zp](../../build/reference/zp-struct-member-alignment.md) pour spécifier l'alignement de la structure. \(Le compilateur ne génère pas cet avertissement lorsque **\/Zp1** est spécifié.\)