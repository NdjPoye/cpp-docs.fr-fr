...
Cet avertissement ou erreur possède plusieurs significations possibles, selon le contexte.  
  
L’erreur C4996 se produit lorsque le compilateur rencontre une fonction ou une variable qui est marqué comme étant [déconseillée](../../cpp/deprecated-cpp.md) à l’aide un `__declspec(deprecated)` modificateur. Cet avertissement est également émis lorsque vous tentez d’accéder à une fonction, un membre de classe ou un typedef qui a C ++ 14 `[[deprecated]]` attribut. Pour plus d’informations, consultez [attributs Standard C++](../../cpp/attributes2.md). Vous pouvez utiliser cet attribut vous-même dans vos bibliothèques pour informer vos clients sur les typedefs, les membres ou les fonctions déconseillées.  
  
Plusieurs fonctions, des fonctions membres, des fonctions de modèle et des variables globales des bibliothèques de Visual Studio sont marquées comme déconseillées. Ces fonctions peuvent avoir un autre nom préféré, peuvent être risquées ou avoir une variante plus sécurisée, ou bien être obsolètes. Nombreux messages d’erreur incluent une suggestion de remplacement pour la fonction déconseillée ou une variable globale.  
...
Vous pouvez désactiver l’avertissement pour une ligne de code spécifique à l’aide de la [avertissement](../../preprocessor/warning.md) pragma `#pragma warning(suppress : 4996)`. Vous pouvez le désactiver dans un fichier à l’aide du pragma warning `#pragma warning(disable : 4996)`. Vous pouvez désactiver globalement dans les versions de ligne de commande à l’aide de la **/wd4996** l’option de ligne de commande. Pour désactiver l’avertissement pour un projet dans l’IDE de Visual Studio, ouvrez le **Pages de propriétés** boîte de dialogue, sélectionnez le **propriétés de Configuration**, **C/C++**, **avancé** page et de modifier le **désactivation des avertissements spécifiques** propriété à ajouter `4996`.  Vous pouvez également utiliser des macros de préprocesseur pour désactiver certaines classes spécifiques d’avertissements de désapprobation utilisés dans les bibliothèques. Ces macros sont décrits ci-dessous.  
  
Voici certaines des sources de bibliothèque de l’erreur C4996.  
...
Pour plus d’informations sur la façon de supprimer ces avertissements, consultez [_AFX_SECURE_NO_WARNINGS](../../mfc/reference/diagnostic-services.md#afx_secure_no_warnings) et [_ATL_SECURE_NO_WARNINGS](http://msdn.microsoft.com/Library/587d29d8-a75a-44a3-bec8-f724087e5e73).  
...
Pour désactiver les avertissements déconseillant ces éléments, définissez **_CRT_OBSOLETE_NO_WARNINGS**. Pour plus d’informations, consultez la documentation pour la fonction ou la variable déconseillée.  
  
## <a name="example-marshalling-errors-in-clr-code"></a>Exemple : Marshaling des erreurs dans le code CLR  
