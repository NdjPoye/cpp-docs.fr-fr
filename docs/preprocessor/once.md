...
## Notes  
 L'utilisation de `#pragma once` peut réduire les durées de génération étant donné que le compilateur n'ouvrira pas le fichier et ne le lira pas après le premier élément \#include du fichier dans l'unité de traduction.  Cette opération porte le nom d'*optimisation de plusieurs inclusions*.  Son effet est semblable à l'idiome *\#include guard*, qui utilise les définitions de macro de préprocesseur pour empêcher plusieurs inclusions du contenu du fichier.  Cela permet également d'empêcher les violations de la *règle de définitions*, qui exige que tous les modèles, types, fonctions et objets aient une seule définition dans votre code.  
...
```  
 ...
 Veillez à ne pas utiliser `#pragma once` ni l'idiome \#include guard dans les fichiers d'en\-tête destinés à être inclus plusieurs fois, avec des symboles de préprocesseur pour contrôler leurs effets.  Pour obtenir un exemple de cette conception, consultez le fichier d'en\-tête \<assert.h\>.  Veillez également à gérer les chemins d'accès Include pour éviter de créer plusieurs chemins d'accès aux fichiers inclus, ce qui peut anéantir l'optimisation de plusieurs inclusions à la fois pour l'idiome \#include guard et `#pragma once`.  
