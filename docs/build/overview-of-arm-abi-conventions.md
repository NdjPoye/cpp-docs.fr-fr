---
title: "Vue d&#39;ensemble des conventions ABI ARM | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
### Phase B : pré\-remplissage et extension des arguments  
...
### Phase C : assignation d'arguments aux registres et à la pile  
...
## Exploration de pile  
 Le code Windows est compilé avec les pointeurs de frame activés \([\/Oy \(Omission du pointeur frame\)](../build/reference/oy-frame-pointer-omission.md)\) pour permettre une exploration de pile rapide.  En général, le registre r11 pointe vers le lien suivant dans la chaîne, qui correspond à une paire {r11, lr} qui spécifie le pointeur vers le frame précédent de la pile et l'adresse de retour.  Nous recommandons aussi l'activation des pointeurs de frame dans votre code pour un profilage et un traçage améliorés.  
  
## Déroulement d'exception  
 ...
 L'interface EABI ARM spécifie un modèle de déroulement d'exception qui utilise des codes de déroulement.  Cependant, cette spécification ne suffit pas pour un déroulement dans Windows, qui doit gérer des cas où le processeur se situe au milieu du prologue ou de l'épilogue d'une fonction.  Pour plus d'informations sur les données et le déroulement d'exception Windows on ARM, consultez [Gestion des exceptions ARM](../build/arm-exception-handling.md).  
