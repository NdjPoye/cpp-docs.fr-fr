---
title: "/DEPENDENTS | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "/dependents"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "/DEPENDENTS (option Dumpbin)"
  - "DEPENDENTS (option Dumpbin)"
  - "-DEPENDENTS (option Dumpbin)"
ms.assetid: 9b31da2a-75ac-4bbf-a3f1-adf8b0ecbbb4
caps.latest.revision: 7
author: "corob-msft"
ms.author: "corob"
manager: "ghogen"
caps.handback.revision: 7
---
# /DEPENDENTS
[!INCLUDE[vs2017banner](../../assembler/inline/includes/vs2017banner.md)]

```  
/DEPENDENTS  
```  
  
## Notes  
 Cette option vide les noms des DLL à partir desquelles l'image importe des fonctions.  Elle ne vide pas les noms des fonctions importées.  
  
 Seule l'option [\/HEADERS](../../build/reference/headers.md) de DUMPBIN est disponible pour les fichiers générés à l'aide de l'option de compilation [\/GL](../../build/reference/gl-whole-program-optimization.md).  
  
## Voir aussi  
 [Options DUMPBIN](../../build/reference/dumpbin-options.md)