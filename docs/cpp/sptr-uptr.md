...
## Section spécifique à Microsoft  
 ...
 Le modificateur `__sptr`, ou pointeur signé, spécifie qu'une conversion de pointeur transforme les bits les plus significatifs d'un pointeur 64 bits en bit de signe du pointeur 32 bits.  Le modificateur `__uptr`, ou pointeur non signé, spécifie qu'une conversion définit les bits les plus significatifs à zéro.  Les déclarations suivantes illustrent les modificateurs `__sptr` et `__uptr` utilisés avec deux pointeurs non qualifiés, deux pointeurs qualifiés avec le type [\_\_ptr32](../cpp/ptr32-ptr64.md), et un paramètre de fonction.  
...
```  
 ...
 Utilisez les modificateurs `__sptr` et `__uptr` avec les déclarations de pointeur.  Utilisez les modificateurs dans la position d'un [qualificateur de type pointeur](../c-language/pointer-declarations.md), qui signifie que le modificateur doit suivre l'astérisque.  Vous ne pouvez pas utiliser de modificateurs avec des [pointeurs vers des membres](../cpp/pointers-to-members.md).  Les modificateurs n'ont pas d'incidence sur les déclarations non pointeur.  
  
 Si vous n'utilisez pas le modificateur `__sptr` ou `__uptr` et que vous activez [Avertissement du compilateur \(niveau 2\) C4826](../error-messages/compiler-warnings/compiler-warning-level-2-c4826.md), le compilateur émet un avertissement lorsqu'un pointeur 32 bits est converti en 64 bits.  
...
```  
  ...
  **Afficher chaque pointeur 32 bits \(en tant que pointeur 64 bits non signé\):**  
**p32d:       0000000087654321**  
**p32s:       0000000087654321**  
**p32u:       0000000087654321**  
**Afficher le pointeur 64 bits créé à partir de chaque pointeur 32 bits:**  
